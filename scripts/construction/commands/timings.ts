...
export default {
  summary:
    '[--skip-browser] [--skip-cargo] — time native compile (cargo run, direct binary, warm process), Vite, Chromium, page load, composition and one capture',
  ...
  async run({ root, id, has }) {
    ...
    const compile: Record<string, number | null> = {},
    ...
    const run = (command: string, args: string[]) =>
      new Promise<string>((resolve, reject) => {
        ...
        child.on('close', (code) => (code === 0 ? resolve(out) : reject(new Error(err.slice(-2000)))));
      });
    let live = false,
      outputs = 'not compared';
    try {
      await timed(compile, 'writeInputsMs', async () => {
      ...
      const cargo = compiler.cargoCommand(),
        build = ['--quiet', '--locked', '--profile', 'wasm-dev', '-p', 'naval-sim', '--example', 'compile_construction'];
      let reference = '';
      ...
      if (binary) {
        ...
        try {
          ...
          const stable =
            identity === (await compiler.compilerBinaryIdentity(binary).catch(() => '')) &&
            !!(await compiler.freshCompilerBinary(root, false));
          outputs = !stable
            ? 'not compared: compiler inputs changed during measurement'
            : (reference && direct !== reference) || first.result + '\n' !== direct || second.result + '\n' !== direct
              ? 'DIFFERENT'
              : 'identical';
      ...
      await timed(compile, 'libraryCacheHitMs', async () => {
        JSON.parse(
          await compiler.compileConstructionJson(root, source, undefined, {
            used: (value) => {
              path = value;
            },
          }),
      ...
      await timed(compile, 'libraryUncachedMs', async () => {
        JSON.parse(await compiler.compileConstructionJson(root, source, undefined, { paths: ['binary', 'cargo'] }));
      });
      if (!has('--skip-browser')) {
        ...
        const previous = process.env.CONSTRUCTION_SESSION;
        ...
        try {
          ...
          try {
            chrome = await timed(browser, 'chromiumLaunchMs', () =>
              chromium.launch({
                headless: true,
            ...
            const page = await chrome.newPage({ viewport: { width: 1600, height: 1000 } });
            page.setDefaultTimeout(120_000);
            await timed(browser, 'pageLoadMs', async () => {
              await page.goto(serverUrl(server) + '/tools/construction/review.html', { waitUntil: 'domcontentloaded' });
              await page.waitForFunction(() => !!window.constructionReviewModule);
            });
            await timed(browser, 'composeModelMs', () =>
              page.evaluate(async () => {
                window.constructionReviewInput = await (await fetch('/__review_input.json')).json();
                window.constructionReview = await window.constructionReviewModule!.openReview(window.constructionReviewInput!);
              }),
            );
            await timed(browser, 'firstViewMs', () =>
              page.evaluate(() => window.constructionReview!.render('profile', {}).then((frame) => frame.png.length)),
            );
            await timed(browser, 'secondViewMs', () =>
              page.evaluate(() => window.constructionReview!.render('quarter', {}).then((frame) => frame.png.length)),
            );
            await timed(browser, 'inspectMs', () =>
              page.evaluate(() => {
                window.constructionReview!.inspect();
              }),
            );
          } finally {
            await timed(browser, 'closeMs', async () => {
              await chrome?.close();
          ...
          if (previous === undefined) delete process.env.CONSTRUCTION_SESSION;
          else process.env.CONSTRUCTION_SESSION = previous;
        }
        if (live)
          for (const key of ['browserOneViewMs', 'browserOneViewRepeatMs'])
            await timed(warm, key, () =>
              withConstructionBrowser(root, input, (page) =>
                page.evaluate(() => window.constructionReview!.render('profile', {}).then((frame) => frame.png.length)),
              ),
            );
      }
    ...
    const sum = (...values: (number | null | undefined)[]) =>
      values.some((v) => v == null) ? null : Math.round(values.reduce<number>((a, v) => a + v!, 0));
    ...
    return {
      ...
      equipment: source.construction.equipment.length,
      session: live,
      cargoRunVsBinaryVsServedOutputs: outputs,
      compile,
      browser,
      warm,
      equivalents: {
        ...
        renderQuickViaCargoRunMs: sum(
          compile.writeInputsMs,
          compile.cargoRunMs,
          browser.viteStartMs,
          browser.chromiumLaunchMs,
          browser.pageLoadMs,
          browser.composeModelMs,
          views,
          browser.closeMs,
        ),
        renderQuickViaDirectBinaryMs: sum(
          compile.writeInputsMs,
          compile.freshnessCheckMs,
          compile.directBinaryMs,
          browser.viteStartMs,
          browser.chromiumLaunchMs,
          browser.pageLoadMs,
          browser.composeModelMs,
        ...
        renderQuickRepeatedRevisionViaSessionMs: sum(
          compile.libraryCacheHitMs,
          warm.browserOneViewRepeatMs,
          fourMoreViews,
          browser.inspectMs,
        ),
