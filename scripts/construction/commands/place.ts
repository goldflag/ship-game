...
export default {
  summary:
    ...
    '[--repeat n --step dx,dz] [--out batch.json] — seat new equipment on the native hull under (x,z): topmost ' +
    'support by default, the one nearest --y, or the named piece; wall fittings need --y and seat along the wall ' +
    'normal; proposes a revision-guarded batch validated by a native dry-run; never saves',
  ...
  async run(ctx) {
    ...
    const items = placementItems(source, catalog, {
      ...
      repeat: numbers('--repeat', 1)?.[0],
      step: numbers('--step', 2) as [number, number] | undefined,
    });
    ...
    emit(
      await proposeBatch(
        ...
        current,
        'Place ' + partId,
        failed ? [] : placementCommands(items, report.placements),
        report,
        {},
        ctx.option('--y') !== undefined,
      ),
    );
  },
