...
export default {
  ...
  async run(ctx) {
    ...
    const blocking = ctx.has('--all')
      ? {
          ...report,
          diagnostics: report.diagnostics.map((d) => (d.code === 'placement-support' ? { ...d, severity: 'warning' as const } : d)),
        }
      : report;
    emit(
      await proposeBatch(ctx, current, 'Reseat equipment', reseatCommands(placements), blocking, {
        ...
        ...(!ctx.has('--slide') && report.placements.some((p) => p.status === 'unsupported' && !p.message?.includes('within'))
          ? {
              hint: 'Nothing lies along the attachment direction of an unsupported record. --slide moves it sideways onto the closest support; otherwise move it explicitly.',
            }
          : {}),
      }),
    );
