...
export interface PlacementReport { placements: Placement[]; diagnostics: ConstructionDiagnostic[] }
...
export async function resolvePlacement(root: string, source: ConstructionSource, items: PlacementItem[]): Promise<PlacementReport> {
  ...
  try {
    const files = { source, catalog, request: { items } }, paths = Object.keys(files).map((name) => join(directory, name + '.json'));
    ...
    const child = spawn(existsSync(localCargo) ? localCargo : 'cargo', ['run', '--quiet', '--locked', '--profile', 'wasm-dev', '-p', 'naval-sim', '--example', 'place_construction', '--', ...paths], { cwd: root, stdio: ['ignore', 'pipe', 'pipe'] });
...
const brief = (p: Placement, measured: boolean) => ({
  id: p.id, partId: p.partId, status: p.status, position: p.position, bearingDeg: p.bearingDeg,
  ...
  ...(p.support ? { support: { kind: p.support.kind, id: p.support.primitiveId ?? p.support.id, face: p.support.face, panelId: p.support.panelId, point: p.support.point, slopeDeg: Number(((Math.acos(Math.min(1, Math.abs(p.support.normal[1]))) * 180) / Math.PI).toFixed(2)) } } : {}),
  ...(p.message ? { message: p.message } : {}),
});

/** Never saves. Builds the guarded batch, applies it in memory and compiles exactly that candidate natively. */
export async function proposeBatch(ctx: CommandContext, current: { source: ConstructionSource; hash: string }, label: string, commands: ConstructionCommand[], report: PlacementReport, extra: Record<string, unknown> = {}, measured = true) {
  ...
  return {
    id: ctx.id, revision: source.revision, fileRevision: current.hash, saved: false,
    placements: report.placements.map((p) => brief(p, measured)), ...extra,
    diagnostics: report.diagnostics,
    candidate: result
      ? { launchable: !!result.definition, diagnostics: result.diagnostics, ...(unrelated ? { unverified: 'The candidate stops at an error on ' + blocker!.sourceId + ', which this batch does not touch; fix it, then dry-run this batch again.' } : {}) }
      : { compiled: false, reason: failed ? 'No candidate: at least one record has no seat.' : 'Nothing to change: every record already sits on its support.' },
