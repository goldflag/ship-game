...
export function inspectConstructionGlb(bytes: Buffer, definition: ShipDefinition) {
  if (
    ...
    throw new Error('Invalid construction GLB.');
  const gltf = JSON.parse(bytes.subarray(20, 20 + bytes.readUInt32LE(12)).toString());
  if (gltf.scenes?.[gltf.scene ?? 0]?.extras?.definitionHash !== definition.contentHash)
    throw new Error('Construction GLB/definition hash mismatch.');
  const nodes = new Map<string, number>();
  gltf.nodes.forEach(
    (
      node: { extras?: { nodeId?: string }; matrix?: number[]; translation?: number[]; rotation?: number[]; scale?: number[] },
      i: number,
    ) => {
      const id = node.extras?.nodeId;
      if (id) {
        if (nodes.has(id)) throw new Error('Duplicate construction node ' + id);
        nodes.set(id, i);
      }
      if (
        [...(node.matrix ?? []), ...(node.translation ?? []), ...(node.rotation ?? []), ...(node.scale ?? [])].some(
          (n) => !Number.isFinite(n),
        )
      )
        throw new Error('Nonfinite construction transform.');
    },
  );
  const required = (id: string) => {
    const index = nodes.get(id);
    if (index === undefined) throw new Error('Missing construction joint/socket ' + id);
    return index;
  };
  for (const mount of definition.mounts) {
    ...
    for (const barrel of barrelIds(mount.weapon)) {
      ...
      for (let i = 1; i < chain.length; i++)
        if (!gltf.nodes[chain[i - 1]].children?.includes(chain[i])) throw new Error('Broken construction articulation chain: ' + mount.id);
    }
  }
  if (!gltf.meshes?.length || gltf.buffers?.some((b: { uri?: string }) => b.uri))
    throw new Error('Construction models must contain embedded mesh buffers.');
  return { nodes: nodes.size, meshes: gltf.meshes.length };
}

export async function constructionPipeline(
  root: string,
  action: string,
  id: string,
  force = false,
  ...
  const stage = join(root, '.build/ships', id),
    sourceDir = join(root, 'assets/ships', id),
    output = join(root, 'public/models');
  if (!result.definition || result.diagnostics.some((d) => d.severity === 'error'))
  ...
  for (const partId of publishedPartIds(source)) {
    ...
    if (manifest.contentHash !== part.contentHash || manifest.modelSha256 !== digest(await readFile(path)))
      throw new Error('Published component integrity failure: ' + partId);
  }
  ...
  const paths = {
    model: join(output, id + '.glb'),
    definition: join(output, id + '.json'),
    thumbnail: join(output, id + '-thumbnail.png'),
    manifest: join(sourceDir, 'generated/build.json'),
  };
  const readManifest = () => readFile(paths.manifest, 'utf8').then(JSON.parse);
  const imageInput = (payloadHash: string) =>
    hash([payloadHash, inputs.presentation, source.revision, definition.mounts.map((m) => [m.id, m.initialElevationDeg ?? 0])]);
  const intact = async (path: string, expected?: string) =>
    !!expected &&
    (await readFile(path).then(
      (bytes) => digest(bytes) === expected,
      () => false,
    ));
  const checkImages = async (manifest: any, review = true) => {
    if (
      manifest.imageInput !== imageInput(manifest.payloadHash) ||
      !(await intact(paths.thumbnail, manifest.thumbnailHash)) ||
      !(await intact(join(sourceDir, 'generated/thumbnail/render.json'), manifest.thumbnailMetadataHash))
    )
      throw new Error('Constructed thumbnail is stale. Run bun run ship:thumbnail ' + id);
    if (
      review &&
      (manifest.reviewInput !== imageInput(manifest.payloadHash) ||
        !(await intact(join(sourceDir, 'generated/review/cameras.json'), manifest.reviewMetadataHash)) ||
        !(
          await Promise.all(
            REVIEW_VIEWS.map((name) => intact(join(sourceDir, 'generated/review', name + '.png'), manifest.reviewHashes?.[name])),
          )
        ).every(Boolean))
    )
      throw new Error('Constructed review is stale. Run bun run ship:review ' + id);
  };
  const checkModel = async () => {
    ...
    if (
      manifest.contentHash !== definition.contentHash ||
      manifest.payloadHash !== modelPayloadHash(model) ||
      def.toString() !== canonical(definition) + '\n'
    )
  ...
  if (action === 'check') {
    const manifest = await checkModel();
    await checkImages(manifest);
    return { id, action, contentHash: definition.contentHash };
  }
  ...
  try {
    ...
    throw new Error('Another pipeline owns ' + id + '. Wait for it to finish.');
  }
  try {
    ...
    try {
      ...
      if (action !== 'build') throw error;
    }
    const previous = await readManifest().catch(() => undefined);
    if (action === 'build' && modelValid && !force) {
      try {
        await checkImages(previous);
        return { id, action, reused: true, contentHash: definition.contentHash };
      } catch {
        /* Refresh only stale image stages. */
      }
    }
    ...
    const assertCurrent = async () => {
      ...
      if (
        current.hash !== sourceFile.hash ||
        canonical(await readCatalog(root, source.construction.catalogRevision)) !== canonical(catalog) ||
        canonical(await constructionFingerprints(root, current.source, await services.compileConstruction(root, current.source))) !==
          canonical(inputs)
      )
      ...
      for (const partId of publishedPartIds(source)) {
        ...
        const manifest = JSON.parse(await readFile(join(dirname(path), 'manifest.json'), 'utf8'));
        if (manifest.contentHash !== part.contentHash || manifest.modelSha256 !== digest(await readFile(path)))
          throw new Error('Published component changed during build: ' + partId);
      }
    };
    ...
    const captured = await services.withConstructionBrowser(
      root,
      exportNeeded ? input : { ...input, modelUrl: definition.modelUrl },
      async (page) => {
        if (exportNeeded) {
          ...
          await page.evaluate(async (stageUrl) => {
            ...
            window.constructionReview = await window.constructionReviewModule!.openReview({
              ...window.constructionReviewInput!,
              definition,
              modelUrl: stageUrl + '/model.glb',
            });
          }, '/@fs' + stage);
        }
        const articulation = await page.evaluate(() => window.constructionReview!.sweep());
        if (articulation.maxMuzzleErrorM > 0.025) throw new Error('Exported articulation diverges from CPU poses.');
        ...
        const thumbnailNeeded =
          ...
          (force ||
            action === 'thumbnail' ||
            !imagesMatch ||
            !(await intact(paths.thumbnail, previous?.thumbnailHash)) ||
            !(await intact(join(sourceDir, 'generated/thumbnail/render.json'), previous?.thumbnailMetadataHash)));
        const reviewNeeded =
          action !== 'thumbnail' &&
          (force ||
            action === 'review' ||
            previous?.reviewInput !== imageInput(payloadHash) ||
            !(await intact(join(sourceDir, 'generated/review/cameras.json'), previous?.reviewMetadataHash)) ||
            !(
              await Promise.all(
                REVIEW_VIEWS.map((name) => intact(join(sourceDir, 'generated/review', name + '.png'), previous?.reviewHashes?.[name])),
              )
            ).every(Boolean));
        const thumbnail = thumbnailNeeded
          ? await page.evaluate(() => window.constructionReview!.render('profile', { width: 600, height: 180, transparent: true }))
          : undefined;
        const views = [];
        if (reviewNeeded)
          for (const name of REVIEW_VIEWS) views.push(await page.evaluate((name) => window.constructionReview!.render(name), name));
    ...
    if (captured.thumbnail) {
      ...
      await atomic(
        join(sourceDir, 'generated/thumbnail/render.json'),
        json({ contentHash, imageHash: digest(png(captured.thumbnail.png)), camera: captured.thumbnail.camera }),
      );
    ...
    const manifest = {
      format: 'construction-v2',
      inputs: { definition: inputs.definition, model: inputs.model },
      contentHash,
      ...
      modelHash: digest(await readFile(paths.model)),
      definitionHash: digest(await readFile(paths.definition)),
      thumbnailHash: captured.thumbnail ? digest(png(captured.thumbnail.png)) : previous?.thumbnailHash,
      thumbnailMetadataHash:
        action !== 'review' ? digest(await readFile(join(sourceDir, 'generated/thumbnail/render.json'))) : previous?.thumbnailMetadataHash,
      reviewMetadataHash:
        action !== 'thumbnail' ? digest(await readFile(join(sourceDir, 'generated/review/cameras.json'))) : previous?.reviewMetadataHash,
      reviewHashes: captured.views.length
        ? Object.fromEntries(captured.views.map((view) => [view.camera.name, digest(png(view.png))]))
        : previous?.reviewHashes,
    };
    ...
    return {
      ...
      action,
      contentHash,
      exported: exportNeeded,
      renderedThumbnail: !!captured.thumbnail,
      renderedViews: captured.views.length,
      diagnostics: result.diagnostics,
      articulation: {
        samples: captured.articulation.samples,
        maxMuzzleErrorM: captured.articulation.maxMuzzleErrorM,
        blockedSamples: captured.articulation.blocked.length,
      },
      model: paths.model,
    };
  } finally {
