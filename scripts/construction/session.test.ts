...
import { createSessionServer, probeSession, liveSession, sessionCompile, sessionBrowser, sessionPath, stopSession, SESSION_VERSION, type RunningSession, type SessionServices } from './session';

// Protocol only: fake services stand in for the native compiler and Chromium.
const roots: string[] = [], sessions: RunningSession[] = [];
afterEach(async () => {
  await Promise.all(sessions.splice(0).map(session => session.close()));
  await Promise.all(roots.splice(0).map(root => rm(root, { recursive: true, force: true })));
});
const root = async () => { const path = await mkdtemp(join(tmpdir(), 'construction-session-')); roots.push(path); return path; };
function fakes(overrides: Partial<SessionServices> = {}) {
  const log: string[] = [];
  const services: SessionServices = {
    async compile({ source, parts }) { log.push('start ' + source); await new Promise(resolve => setTimeout(resolve, 20)); log.push('end ' + source); return { result: JSON.stringify({ echo: source, parts: parts ?? null }), reused: 3 }; },
    async browser(inputs) {
      return { async open(lease) { log.push('open ' + inputs.get(lease)); return { evaluate: async (expression, isFunction, arg) => isFunction ? (0, eval)('(' + expression + ')')(arg) : (0, eval)(expression), close: async () => { log.push('close ' + lease); } }; } };
    },
    async close() { log.push('services closed'); },
    ...overrides,
  };
  return { services, log };
}
async function start(path: string, options: Partial<Parameters<typeof createSessionServer>[0]> = {}, overrides: Partial<SessionServices> = {}) {
  const fake = fakes(overrides), session = await createSessionServer({ root: path, services: fake.services, sweepMs: 20, ...options });
  sessions.push(session);
  return { session, ...fake };
}
const request = (session: RunningSession, path: string, init: RequestInit = {}) => fetch('http://127.0.0.1:' + session.record.port + path, init);

test('the session file is private, names this process and answers only its token', async () => {
  const path = await root(), { session } = await start(path);
  ...
  expect((await request(session, '/status', { headers: { authorization: 'Bearer ' + session.record.token, origin: 'http://127.0.0.1:5173' } })).status).toBe(403);
  expect((await request(session, '/status', { headers: { authorization: 'Bearer ' + session.record.token, host: 'example.test' } })).status).toBe(403);
...
test('compile returns the exact text and concurrent requests never interleave', async () => {
  const path = await root(), { session, log } = await start(path);
  const answers = await Promise.all(['a', 'b', 'c'].map(source => sessionCompile(path, source, 'r'.repeat(64), source === 'b' ? ['x'] : undefined)));
...
test('an unavailable or failing warm compiler means compile cold, never an error', async () => {
  ...
  await start(path, {}, { compile: async ({ source }) => { if (source === 'unsure') return undefined; throw new Error('panicked'); } });
...
test('stale session files fall back quickly: dead pid, closed port, other version, other worktree, changed sources, env off', async () => {
  ...
  const { session } = await start(path), record = { ...session.record };
  expect((await liveSession(path))?.pid).toBe(process.pid);
  process.env.CONSTRUCTION_SESSION = 'off';
  try { expect(await liveSession(path)).toBeUndefined(); } finally { delete process.env.CONSTRUCTION_SESSION; }
  await session.close();
  ...
  const hung = createServer(() => {}); await new Promise<void>(resolve => hung.listen(0, '127.0.0.1', resolve));
  try {
    ...
    for (const [value, reason] of cases) {
      await write(value);
      const from = performance.now(), probe = await probeSession(path, 300);
...
test('browser leases are exclusive, forward evaluate, and a dead holder is revoked', async () => {
  ...
  const response = await request(session, '/browser/acquire?pid=' + dead, { method: 'POST', headers: { authorization: 'Bearer ' + session.record.token }, body: '"crashed"' });
...
test('a browser that cannot start or open means render cold', async () => {
  const path = await root();
  await start(path, {}, { browser: async () => { throw new Error('no chromium'); } });
  expect(await sessionBrowser(path, '{}')).toBeUndefined();
  const probe = await probeSession(path);
...
test('the daemon ends itself when idle, when its sources change, and on stop', async () => {
  const idle = await root(), { session, log } = await start(idle, { idleTimeoutMs: 60 });
  ...
  const changed = await root(); let fingerprint = 'one';
  ...
  const stopped = await root(), running = await start(stopped);
  ...
  expect((await request(running.session, '/stop', { method: 'POST', headers: { authorization: 'Bearer ' + running.session.record.token } })).status).toBe(200);
