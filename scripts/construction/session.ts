...
export interface SessionStatus {
  ...
  browser: {
    state: 'disabled' | 'idle' | 'starting' | 'ready' | 'failed';
    requests: number;
    waiting: number;
    leased: boolean;
    error?: string;
...
export interface SessionPage {
  evaluate(expression: string, isFunction: boolean, arg: unknown): Promise<unknown>;
  close(): Promise<void>;
}
export interface SessionBrowser {
  open(lease: string): Promise<SessionPage>;
}
...
const DAEMON_SOURCES = [
  'scripts/construction/session.ts',
  'scripts/construction/session-daemon.ts',
  'scripts/construction/compiler.ts',
  'scripts/construction/browser.ts',
  'scripts/construction/server.ts',
  'scripts/construction/files.ts',
  'package.json',
  'bun.lock',
];
...
export async function sessionFingerprint(root: string): Promise<string> {
  const parts = await Promise.all(
    DAEMON_SOURCES.map((path) =>
      stat(join(root, path)).then(
        (s) => path + ':' + s.mtimeMs + ':' + s.size,
        () => path + ':absent',
      ),
    ),
  );
  return createHash('sha256')
    .update([SESSION_VERSION, process.versions.bun ?? process.version, ...parts].join('\n'))
    .digest('hex')
    .slice(0, 32);
}
...
export async function readSession(root: string): Promise<SessionRecord | undefined> {
  const record = await readFile(sessionPath(root), 'utf8')
    .then((text) => JSON.parse(text) as SessionRecord)
    .catch(() => undefined);
  return record && typeof record.port === 'number' && typeof record.token === 'string' && typeof record.pid === 'number'
    ? record
    : undefined;
}
const call = (record: SessionRecord, path: string, init: RequestInit & { timeoutMs: number }) =>
  fetch('http://127.0.0.1:' + record.port + path, {
    ...init,
    headers: { ...init.headers, authorization: 'Bearer ' + record.token },
    signal: AbortSignal.timeout(init.timeoutMs),
  });
export type SessionProbe =
  { live: true; record: SessionRecord; status: SessionStatus } | { live: false; reason: string; record?: SessionRecord };
...
export async function probeSession(root: string, timeoutMs = 500): Promise<SessionProbe> {
  ...
  if (record.fingerprint !== (await sessionFingerprint(root)))
    return { live: false, reason: 'Session was started from different daemon sources.', record };
  try {
    ...
    if (status.pid !== record.pid || status.fingerprint !== record.fingerprint)
      return { live: false, reason: 'Another process answers on the session port.', record };
    return { live: true, record, status };
  } catch {
    return { live: false, reason: 'Session did not answer within ' + timeoutMs + ' ms.', record };
  }
}
...
export async function sessionCompile(root: string, source: string, catalogRevision: string, parts?: string[]): Promise<string | undefined> {
  ...
  try {
    const response = await call(record, '/compile', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ source, catalogRevision, parts }),
      timeoutMs: 660_000,
    });
...
export async function sessionBrowser(root: string, input: string): Promise<BrowserLease | undefined> {
  ...
  if (!record) return undefined;
  try {
    const response = await call(record, '/browser/acquire?pid=' + process.pid, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: input,
      timeoutMs: 900_000,
    ...
    const evaluate = async (work: unknown, arg?: unknown) => {
      const reply = await call(record, '/browser/evaluate?lease=' + lease, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ expression: String(work), isFunction: typeof work === 'function', arg }),
        timeoutMs: 1_800_000,
      });
    ...
    const page = new Proxy(
      {},
      {
        get(_, key) {
          ...
          throw new Error(
            'A warm session page supports page.evaluate only, not page.' +
              key +
              '. Run with CONSTRUCTION_SESSION=off, or extend scripts/construction/session.ts.',
          );
        },
      },
    ...
    return {
      page,
      release: async () => {
        await call(record, '/browser/release?lease=' + lease, { method: 'POST', timeoutMs: 5_000 }).catch(() => {});
      },
    };
  } catch {
    return undefined;
...
export async function createSessionServer(options: SessionServerOptions): Promise<RunningSession> {
  const { root, services } = options,
    ...
    leaseTimeoutMs = options.leaseTimeoutMs ?? 20 * 60_000;
  const fingerprintNow = options.fingerprint ?? (() => sessionFingerprint(root)),
    fingerprint = await fingerprintNow();
  ...
  let lastActivity = Date.now(),
    ...
    reused = 0,
    browserRequests = 0;
  let browserState: SessionStatus['browser']['state'] = options.browser === false || !services.browser ? 'disabled' : 'idle',
  ...
  const startBrowser = () => {
    ...
    if (!endpoint) {
      ...
      endpoint = services.browser!(inputs).then((value) => {
        browserState = 'ready';
      ...
      endpoint.catch((error) => {
        browserState = 'failed';
        browserError = String(error?.message ?? error).slice(0, 500);
        endpoint = undefined;
      });
    }
    return endpoint;
  ...
  const body = async (request: IncomingMessage, limit: number) => {
    ...
    for await (const chunk of request) {
      size += chunk.length;
      if (size > limit) throw Object.assign(new Error('Request body exceeds its limit.'), { status: 413 });
      chunks.push(chunk);
    }
  ...
  const handle = async (request: IncomingMessage, response: ServerResponse) => {
    ...
    if (
      ...
      return send(403, { error: 'Sessions accept loopback connections only.' });
    ...
    try {
      ...
      if (url.pathname === '/compile') {
        ...
        if (typeof input.source !== 'string' || typeof input.catalogRevision !== 'string' || (input.parts && !Array.isArray(input.parts)))
          return send(400, { error: 'Expected source text, catalogRevision and optional parts.' });
        compileRequests++;
      ...
      if (url.pathname === '/browser/acquire') {
        ...
        const input = await body(request, 512 * 1024 * 1024),
        ...
        const held = (lease = { id, pid, at: Date.now() } as NonNullable<typeof lease>);
        inputs.set(id, input);
        browserRequests++;
        try {
          held.page = await ready.open(id);
        } catch (error) {
          // The cold path reproduces a genuine page failure with its own message.
          releaseLease(id);
          return send(503, { error: String((error as Error)?.message ?? error), fallback: true });
        }
        if (lease !== held) {
          void held.page.close().catch(() => {});
          return send(503, { error: 'Lease was revoked while its page loaded.', fallback: true });
        }
      ...
      if (url.pathname === '/browser/evaluate') {
        ...
        held.at = Date.now();
        try {
          return send(200, { value: await held.page.evaluate(String(input.expression), !!input.isFunction, input.arg) });
        } catch (error) {
          return send(422, { error: String((error as Error)?.message ?? error) });
        }
      }
      if (url.pathname === '/browser/release') {
        releaseLease(url.searchParams.get('lease') ?? '');
...
export function nativeServices(root: string): SessionServices {
  let compiler: { process: CompilerProcess; identity: string } | undefined,
    restarts = -1;
  ...
  return {
    ...
    compile(request) {
      ...
      const next = queue.then(async () => {
        ...
        if (!compiler || compiler.process.closed || compiler.identity !== identity) {
          compiler?.process.close();
          compiler = { process: new CompilerProcess(binary, root), identity };
          restarts++;
        }
        ...
        if (!catalog) {
          catalog = JSON.stringify(await readCatalog(root, request.catalogRevision));
          if (catalogs.size >= 4) catalogs.clear();
          catalogs.set(request.catalogRevision, catalog);
        }
        return compiler.process.request({
          source: request.source,
          catalog,
          ...(request.parts ? { parts: JSON.stringify(request.parts) } : {}),
        });
      });
      queue = next.catch(() => {});
      return next;
    ...
    async browser(inputs) {
      ...
      const chrome = await chromium.launch(CHROMIUM_LAUNCH()).catch(async (error) => {
        await vite.close();
        throw error;
      });
      closeBrowser = async () => {
        await chrome.close().catch(() => {});
        (vite.httpServer as Server | null)?.closeAllConnections();
        await vite.close();
      };
      ...
      try {
        ...
        await page.goto(serverUrl(vite) + '/tools/construction/review.html', { waitUntil: 'domcontentloaded', timeout: 120_000 });
        await page.waitForFunction(() => !!window.constructionReviewModule, undefined, { timeout: 120_000 });
      } finally {
        await warm.close();
      }
      return {
        async open(lease) {
          ...
          try {
            ...
            return {
              evaluate: (expression, isFunction, arg) =>
                page.evaluate(
                  ({ expression, isFunction, arg }) => {
                    const value = (0, eval)('(' + expression + ')');
                    return isFunction ? value(arg) : value;
                  },
                  { expression, isFunction, arg },
                ),
              close: () => context.close(),
...
export async function startSession(root: string, options: StartOptions = {}): Promise<{ started: boolean; record: SessionRecord }> {
  ...
  if (!Number.isFinite(idleMinutes) || idleMinutes <= 0 || idleMinutes > 24 * 60)
  ...
  try {
    const child = spawn(
      process.execPath,
      [
        join(import.meta.dir, 'session-daemon.ts'),
        root,
        '--idle-ms',
        String(Math.round(idleMinutes * 60_000)),
        ...(options.browser === false ? ['--no-browser'] : []),
      ],
      { cwd: root, detached: true, stdio: ['ignore', log, log], env: { ...process.env, CONSTRUCTION_SESSION: 'off' } },
    );
    child.on('exit', (code) => {
      exited = code;
    });
...
export async function runSession(root: string, options: { idleTimeoutMs?: number; browser?: boolean } = {}): Promise<string> {
  ...
  for (const signal of ['SIGINT', 'SIGTERM'] as const)
    process.on(signal, () => {
      void session.close('Received ' + signal + '.');
    });
