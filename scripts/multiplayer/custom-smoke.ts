...
const post = (path: string, cookie: string, body: unknown, method = 'POST', headers: Record<string, string> = {}) =>
  fetch(base + path, {
    method,
    headers: { cookie, origin: new URL(base).origin, 'Content-Type': 'application/json', ...headers },
...
const save = await post('/api/ships/' + source.id, a, input, 'PUT', { 'idempotency-key': crypto.randomUUID() });
...
const forged = { ...input, designId: 'forged-' + crypto.randomUUID() };
forged.source = { ...source, id: forged.designId, definition: { massKg: 1 }, catalog: { revision: catalog.revision } } as any;
const fake = await post('/api/ships/' + forged.designId, a, forged, 'PUT', { 'idempotency-key': crypto.randomUUID() });
assert.equal(fake.status, 200);
const fakeRevision = await fake.json();
const fakeJoin = await post('/api/join', a, {
  fleet: [{ kind: 'custom', designId: fakeRevision.designId, revisionId: fakeRevision.id }],
  version,
  mode: 'queue',
});
assert.equal(fakeJoin.ok, false);
assert.match((await fakeJoin.json()).error, /derived definitions, mass or catalogs/);
...
const fleet = [
  { kind: 'custom', designId: revision.designId, revisionId: revision.id },
  { kind: 'historical', presetId: 'fletcher' },
  { kind: 'custom', designId: revision.designId, revisionId: revision.id },
];
assert.equal((await post('/api/join', '', { fleet, version, mode: 'queue' })).status, 401);
const denied = await post('/api/join', b, { fleet, version, mode: 'queue' });
assert.equal(denied.ok, false);
const admitted = await post('/api/join', a, { fleet, version, mode: 'create-invite' });
assert.equal(admitted.status, 200, await admitted.clone().text());
const first = await admitted.json();
assert.equal(
  (
    await post('/api/join', a, {
      fleet: [{ kind: 'historical', presetId: 'fletcher' }],
      version,
      mode: 'join-invite',
...
const edit = await post(
  '/api/ships/' + source.id,
  a,
  { ...input, expectedRevisionId: revision.id, source: { ...source, revision: crypto.randomUUID() } },
  'PUT',
  { 'idempotency-key': crypto.randomUUID() },
);
assert.equal(edit.status, 200);
const newer = await edit.json();
const deleted = await fetch(base + '/api/ships/' + revision.designId, {
  method: 'DELETE',
  headers: { cookie: a, origin: new URL(base).origin, 'if-match': newer.id },
});
assert.equal(deleted.status, 204);
const joined = await post('/api/join', b, {
  fleet: [
    { kind: 'custom', designId: bFirst.designId, revisionId: bFirst.id },
    { kind: 'custom', designId: bNext.designId, revisionId: bNext.id },
    { kind: 'historical', presetId: 'fletcher' },
  ],
  version,
  mode: 'join-invite',
...
const second = await joined.json();
const clients: any[] = [];
async function connect(cookie: string, ticket: string) {
  ...
  const client = { socket, metadata: undefined as any, frames: [] as any[], errors: [] as any[], content: undefined as any };
  clients.push(client);
  ...
  socket.onmessage = async (e) => {
    ...
    const m = bytes ? JSON.parse(gunzipSync(bytes[0] === 0 ? bytes.subarray(1) : bytes).toString()) : JSON.parse(e.data);
    if (m.type === 'matched') {
      client.content = await verifyMatchContent(base, cookie, ticket, m.contentHash);
      client.metadata = m;
    } else if (m.type === 'error') client.errors.push(m);
    else client.frames.push(m);
  };
  return client;
}
...
try {
  const stolen = await connect(b, first.ticket);
  await until(() => stolen.errors.length > 0);
  stolen.socket.close();
  let ca = await connect(a, first.ticket);
  const cb = await connect(b, second.ticket);
  await until(() => !!ca.metadata && !!cb.metadata);
  const wrong = ca;
  wrong.socket.send(JSON.stringify({ type: 'ready', version, contentHash: '0'.repeat(64) }));
  await until(() => wrong.socket.readyState === WebSocket.CLOSED);
  ca = await connect(a, first.ticket);
  await until(() => !!ca.metadata);
  ...
  ca.socket.send(JSON.stringify({ type: 'surrender' }));
  ...
  console.log(
    JSON.stringify({
      ...
      checks: [
        'account requirement',
        'copied revision rejection',
        'self-match rejection',
        'copied ticket rejection',
        'custom command ship',
        'repeated custom ship',
        'mixed fleets',
        'edit/delete after admission',
        'different saved revisions',
        'forged stats/catalog rejection',
        'Ready hash mismatch and custom reconnect',
        'participant content hash',
        'ready and battle',
      ],
      matchId: ca.metadata.matchId,
    }),
  );
} finally {
  await post('/api/cancel', a, { ticket: first.ticket });
