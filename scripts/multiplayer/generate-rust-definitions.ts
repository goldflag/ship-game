...
function rustType(original: ts.Type, hint: string): string {
  ...
  const fields = checker.getPropertiesOfType(type).map(property => {
    ...
    const defaults = ((name === 'ShipDefinition' && field === 'maneuvering') || (name === 'ConstructionPrimitive' && field === 'tilt') || (name === 'ConstructionData' && ['finish', 'paint'].includes(field)) || (name === 'ConstructionEquipmentPart' && ['wall_sizing', 'rigging_surface'].includes(field))) ? ', default, skip_serializing_if = "Option::is_none"' : '';
