...
const cases=[];
for(const [id,enemy,duration,battery] of [['bismarck','baltimore',60,'main'],['fletcher','type-viic',60,'torpedo'],['enterprise-cv6','enterprise-cv6',180,'main']] as const){
 ...
 if(id==='enterprise-cv6')for(const s of sim.definition.airWing!.squadrons)sim.launchAircraft(s.id);
