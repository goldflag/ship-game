...
const historicalFleet = ['enterprise-cv6', 'enterprise-cv6', 'bismarck', 'baltimore', 'fletcher', 'fletcher', 'fletcher', 'fletcher'].map(presetId=>({kind:'historical',presetId}));
...
const post = async (path: string, body: object, cookie?:string) => fetch(`${base}/api/${path}`, { method: 'POST', headers: { 'content-type': 'application/json',cookie:cookie ?? cookies.get((body as any).ticket) ?? await testAccount(base),origin:new URL(base).origin }, body: JSON.stringify(body) });
async function join(mode: string, inviteCode?: string) {
  ...
  const result=await response.json();cookies.set(result.ticket,cookie);return result;
}
...
async function customFleet(cookie:string) {
  const selected:any[]=[];
  for(let i=0;i<8;i++) {
    const source=structuredClone(template);source.id='design-'+crypto.randomUUID();source.revision=crypto.randomUUID();
    const response=await fetch(base+'/api/ships/'+source.id,{method:'PUT',headers:{cookie,origin:new URL(base).origin,'Content-Type':'application/json','idempotency-key':crypto.randomUUID()},body:JSON.stringify({designId:source.id,name:source.name,source,schemaVersion:1,catalogRevision:source.construction.catalogRevision,expectedRevisionId:null})});
    assert.equal(response.status,200,await response.clone().text());const saved=await response.json();selected.push({kind:'custom',designId:saved.designId,revisionId:saved.id});
  }
  return selected;
}
...
function connect(ticket: string) {
  ...
  socket.onmessage = async event => {
    ...
    else { const message = JSON.parse(gunzipSync(new Uint8Array(event.data)[0] === 0 ? new Uint8Array(event.data).subarray(1) : new Uint8Array(event.data)).toString()); if (message.type === 'matched') { await verifyMatchContent(base,cookies.get(ticket)!,ticket,message.contentHash,false); client.messages.push(message); } else if(measuring) { client.frames++; client.bytes += event.data.byteLength; } }
  };
  return client;
}
try {
  for (let i = 0; i < 2; i++) {
    ...
    for(const client of clients.slice(i*2)) client.socket.send(JSON.stringify({type:'ready',version,contentHash:client.messages.find(m=>m.type==='matched').contentHash}));
  }
  ...
  let loading=true,compileJobs=0;
  ...
  const compilerLoad=template?(async()=>{while(loading){
    const source=structuredClone(template);source.id='load-'+crypto.randomUUID();source.revision=crypto.randomUUID();
    const response=await fetch(compilerURL+'/compile',{method:'POST',headers:{'Content-Type':'application/json','x-service-secret':process.env.SERVICE_SECRET!},body:JSON.stringify({accountId:'qualification-load',sourceJson:JSON.stringify(source)})});
    assert.equal(response.status,200,await response.clone().text());await response.arrayBuffer();compileJobs++;
  }})().catch(error=>{compileFailure=error;}):Promise.resolve();
  await sleep(3500);
  clients.forEach(c=>{c.frames=0;c.bytes=0;});measuring=true;
  const heartbeat = setInterval(() => clients.forEach(c => c.socket.send(JSON.stringify({ type: 'ping', nonce: Date.now() >>> 0 }))), 3000);
  try { await sleep(duration * 1000); } finally { measuring=false; loading=false; clearInterval(heartbeat); await compilerLoad; }
