...
def main():
    ...
    if args.aircraft=='all':
        summary=ROOT/'.build/reviews/aircraft/schematic-comparisons.json';summary.parent.mkdir(parents=True,exist_ok=True)
        summary.write_text(json.dumps({'schemaVersion':1,'contentHash':expected_hash,'comparatorSha256':sha(Path(__file__)),'aircraft':results},indent=2)+'\n')
