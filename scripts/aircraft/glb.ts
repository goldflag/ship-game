...
export const aircraftNodeIds = [
  ...
  'propeller.spin',
  'control.rudder',
  'control.elevator.port',
  'control.elevator.starboard',
  'control.aileron.port',
  'control.aileron.starboard',
  'gear.port',
  'gear.starboard',
  'gear.tail',
  'socket.payload',
...
export const aircraftFoldIds = (id: string): string[] =>
  ['f4f-4-wildcat', 'tbd-1-devastator'].includes(id) ? ['wing.fold.port', 'wing.fold.starboard'] : [];
...
export function inspectAircraftGlb(
  bytes: Buffer,
  aircraft: AircraftDimensions,
  contentHash: string,
  options: { requireTexturedSurface?: boolean } = {},
) {
  ...
  require(bytes.length >= 28 &&
    bytes.readUInt32LE(0) === 0x46546c67 &&
    bytes.readUInt32LE(4) === 2 &&
    bytes.readUInt32LE(8) === bytes.length, 'Invalid GLB header');
  require(bytes.length <= 5 * 1024 * 1024, 'GLB exceeds the 5 MiB asset budget');
  const jsonLength = bytes.readUInt32LE(12),
    binaryHeader = 20 + jsonLength;
  require(bytes.readUInt32LE(16) === 0x4e4f534a &&
    jsonLength % 4 === 0 &&
    binaryHeader + 8 <= bytes.length, 'Missing or malformed JSON chunk');
  ...
  require(Array.isArray(gltf.buffers) &&
    gltf.buffers.length === 1 &&
    !gltf.buffers[0].uri &&
    gltf.buffers[0].byteLength <= binary.length &&
    gltf.buffers[0].byteLength > binary.length - 4, 'GLB must use one self-contained binary buffer');
  require(!gltf.extensionsRequired?.length, 'Asset requires unsupported glTF extensions');
  for (const view of gltf.bufferViews ?? [])
    require(view.buffer === 0 &&
      (view.byteOffset ?? 0) >= 0 &&
      view.byteLength >= 0 &&
      (view.byteOffset ?? 0) + view.byteLength <= gltf.buffers[0].byteLength, 'Invalid binary buffer view');
  for (const img of gltf.images ?? [])
    require(!img.uri &&
      img.bufferView !== undefined &&
      Boolean(gltf.bufferViews[img.bufferView]) &&
      ['image/png', 'image/jpeg'].includes(img.mimeType ?? ''), 'Texture images must be embedded PNG/JPEG resources');
  ...
  gltf.nodes.forEach((node, index) => {
    ...
    const shape = (values: number[] | undefined, count: number) =>
      values === undefined || (values.length === count && values.every(Number.isFinite));
    require(shape(node.matrix, 16) &&
      shape(node.translation, 3) &&
      shape(node.rotation, 4) &&
      shape(node.scale, 3), `Invalid transform on node ${index}`);
  ...
  const descendsFrom = (index: number, ancestor: number) => {
    for (let current: number | undefined = index; current !== undefined; current = parents.get(current))
  ...
  for (const [index, node] of gltf.nodes.entries())
    if (node.mesh !== undefined) require(descendsFrom(index, root), 'Mesh outside aircraft.root');
  ...
  const readAccessor = (index: number, expectedType: string) => {
    ...
    require(accessor &&
      ...
      !accessor.sparse, `Unsupported ${expectedType} accessor ${index}`);
    ...
    const componentBytes = sizes[accessor.componentType],
      components = expectedType === 'VEC3' ? 3 : expectedType === 'VEC2' ? 2 : 1;
    require(componentBytes &&
      (expectedType === 'SCALAR' || accessor.componentType === 5126), `Unsupported accessor component type ${accessor.componentType}`);
    const stride = view.byteStride ?? componentBytes * components,
      offset = accessor.byteOffset ?? 0;
    require(stride >= componentBytes * components &&
      offset >= 0 &&
      offset + (accessor.count - 1) * stride + componentBytes * components <= view.byteLength, `Accessor ${index} exceeds its buffer view`);
      ...
      for (let component = 0; component < components; component++) {
        ...
        const value =
          accessor.componentType === 5126
            ? binary.readFloatLE(address)
            : componentBytes === 1
              ? binary.readUInt8(address)
              : componentBytes === 2
                ? binary.readUInt16LE(address)
                : binary.readUInt32LE(address);
  ...
  for (const [index, node] of gltf.nodes.entries()) {
    ...
    for (const primitive of mesh.primitives) {
      ...
      require(primitive.material !== undefined &&
        Boolean(gltf.materials?.[primitive.material]), 'Every primitive must have an embedded material');
      ...
      const material = gltf.materials![primitive.material] as {
      ...
      if (baseColor) {
        ...
        require(Number.isInteger(baseColor.index) &&
          imageIndex !== undefined &&
          Number.isInteger(imageIndex) &&
          Boolean(gltf.images?.[imageIndex]), 'Base-color texture references a missing embedded image');
        ...
        require(img.mimeType === 'image/png'
          ? imageBytes.length >= 24 && imageBytes.subarray(0, 8).toString('hex') === '89504e470d0a1a0a'
          : imageBytes.length >= 4 && imageBytes.readUInt16BE(0) === 0xffd8, 'Embedded base-color image has an invalid signature');
        texturedPrimitives++;
      }
      ...
      require(accessor.min?.length === 3 &&
        accessor.max?.length === 3 &&
        minimum.every((v, axis) => Math.abs(v - accessor.min![axis]) < 0.001) &&
        maximum.every((v, axis) => Math.abs(v - accessor.max![axis]) < 0.001), 'Position accessor bounds disagree with binary vertices');
      let triangleIndices = values.map((_, index) => index);
      if (primitive.indices !== undefined) {
        const indices = readAccessor(primitive.indices, 'SCALAR');
        require([5121, 5123, 5125].includes(indices.accessor.componentType) &&
          indices.values.every(([value]) => value < values.length), 'Triangle index outside its vertex accessor');
      ...
      for (let triangle = 0; triangle < vertexCount; triangle += 3) {
        const a = worldPositions[triangleIndices[triangle]],
          b = worldPositions[triangleIndices[triangle + 1]],
          c = worldPositions[triangleIndices[triangle + 2]];
        const area = new Vector3().subVectors(b, a).cross(new Vector3().subVectors(c, a)).length() / 2;
        require(Number.isFinite(area) &&
          area > 1e-14, `Degenerate triangle in node ${node.name ?? index}, triangle ${triangle / 3} (area ${area} m²)`);
  ...
  if (options.requireTexturedSurface)
    require(uvPrimitives > 0 &&
      texturedPrimitives > 0, 'Production aircraft needs UVs and an embedded base-color texture bound to geometry');
  if (options.requireTexturedSurface)
    require(normalPrimitives === primitives, 'Production aircraft needs finite unit normals on every primitive');
  ...
  const near = (actual: number, expected: number, label: string) =>
    require(Math.abs(actual - expected) <=
      expected * 0.03, `${label} ${actual.toFixed(3)} m differs from catalog ${expected} m by over 3%`);
  ...
  const foldRotation = (id: string, fraction: number) => {
    ...
    require(Array.isArray(axis) &&
      axis.length === 3 &&
      axis.every(Number.isFinite) &&
      Math.abs(Math.hypot(...axis) - 1) < 0.001, `${id} needs a unit runtime fold axis`);
    require(typeof data.foldAngleDegrees === 'number' &&
      data.foldAngleDegrees > 0 &&
      data.foldAngleDegrees <= 180, `${id} needs a valid fold angle`);
  ...
  const movingIds: string[] = [
    ...aircraftNodeIds.filter((id) => id === 'propeller.spin' || id === 'arrestor.hook' || id.startsWith('control.')),
    ...foldIds,
  ];
  const joints = movingIds.map((id) => {
    ...
    const rotation = id.startsWith('wing.fold.')
      ? foldRotation(id, 0.5)
      : id === 'propeller.spin'
        ? new Matrix4().makeRotationZ(0.35)
        : id === 'control.rudder'
          ? new Matrix4().makeRotationY(0.35)
          : new Matrix4().makeRotationX(0.35);
    const articulated = frames({ index, rotation });
    let maximumTravel = 0;
    for (const mesh of movingMeshes)
      for (const vertex of vertices.get(mesh)!)
        maximumTravel = Math.max(
          maximumTravel,
          vertex
            .clone()
            .applyMatrix4(world.get(mesh)!)
            .distanceTo(vertex.clone().applyMatrix4(articulated.get(mesh)!)),
        );
    require(maximumTravel > 0.005, `${id} articulation does not move its geometry`);
    for (const mesh of vertices.keys())
      if (!movingMeshes.includes(mesh)) require(world.get(mesh)!.equals(articulated.get(mesh)!), `${id} moves unrelated geometry`);
    return {
      id,
      pivot: new Vector3().setFromMatrixPosition(world.get(index)!).toArray(),
      movingMeshes: movingMeshes.length,
      testedRotationRadians: id.startsWith('wing.fold.') ? (Number(gltf.nodes[index].extras!.foldAngleDegrees) * Math.PI) / 360 : 0.35,
      maximumVertexTravel: maximumTravel,
    };
  });
  for (const id of ['gear.port', 'gear.starboard', 'gear.tail', 'socket.payload', 'socket.deck'])
    require(gltf.nodes[nodeIndex(id)].mesh === undefined, `${id} must retain an empty pivot/socket node`);
  ...
  if (foldIds.length) {
    ...
    require(foldedWingspan < aircraft.wingspan * 0.7, 'Folded wings must reduce the overall deck footprint');
    foldSweepMinimumY = Infinity;
    for (let sample = 0; sample <= 20; sample++) {
      const pose = frames(foldIds.map((id) => ({ index: nodeIndex(id), rotation: foldRotation(id, sample / 20) })));
      for (const [mesh, verts] of vertices)
        for (const vertex of verts) foldSweepMinimumY = Math.min(foldSweepMinimumY, vertex.clone().applyMatrix4(pose.get(mesh)!).y);
    }
    require(foldSweepMinimumY >= bounds[0].y - 0.02, 'Wing fold sweep extends below the tyres');
  }
  const centerZ = (bounds[0].z + bounds[1].z) / 2;
  require(new Vector3().setFromMatrixPosition(world.get(nodeIndex('propeller.spin'))!).z < centerZ &&
    new Vector3().setFromMatrixPosition(world.get(nodeIndex('control.rudder'))!).z > centerZ, 'Aircraft must face runtime -Z');
  for (const control of ['control.elevator', 'control.aileron']) {
    require(new Vector3().setFromMatrixPosition(world.get(nodeIndex(`${control}.port`))!).x < 0 &&
      new Vector3().setFromMatrixPosition(world.get(nodeIndex(`${control}.starboard`))!).x >
        0, 'Port/starboard controls violate runtime +X starboard convention');
  }
  return {
    schemaVersion: 1,
    ...
    bounds: bounds.map((v) => v.toArray()),
    measured: { wingspan: dimensions.x, height: dimensions.y, length: dimensions.z },
    nodeIds: [...aircraftNodeIds, ...foldIds],
    foldedWingspan,
    foldSweepMinimumY,
    joints,
    triangles,
...
export function inspectAircraftLods(
  models: [Buffer, Buffer, Buffer],
  aircraft: AircraftDimensions,
  contentHash: string,
  options: { requireTexturedSurface?: boolean } = {},
) {
  const reports = models.map((model, level) => {
    try {
      return inspectAircraftGlb(model, aircraft, contentHash, options);
    } catch (error) {
      throw new Error(`LOD${level}: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  if (first / base > 0.75 || second / base > 0.45)
    throw new Error(`${aircraft.id}: LOD reduction misses the approximate 0.45/0.20 triangle targets`);
  return reports;
}
