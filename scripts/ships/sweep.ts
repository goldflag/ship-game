...
for (const r of rows.sort((x, y) => Number(y.reachable > 0) - Number(x.reachable > 0) || x.mount.localeCompare(y.mount))) {
  ...
  const detail = r.reachable
    ? `${r.reachable}/${r.poses} contact poses reachable, e.g. ${r.kind === 'mount' ? `${r.mount} ${r.example!.slice(0, 3).join('/')} with ${r.against} ${r.example!.slice(3).join('/')}` : `train ${r.example![0]}°, elevation ${r.example![1]}°, recoil ${r.example![2]}`}`
