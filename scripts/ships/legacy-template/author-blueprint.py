"""__SHIP_NAME__ blueprint authoring: a Blender-recipe preset scaffolded by `ship:new --legacy`.
...
example):
  ...
  python3 assets/ships/__SHIP_ID__/author-blueprint.py [--loft loft.json]
...
(see the README), then `bun run ship:build __SHIP_ID__`.
"""
import argparse
import json
import math
...
b = dict(schemaVersion=1, id='__SHIP_ID__', name='__SHIP_NAME__',
         configuration='Scaffolded starter · replace with the approved vessel, year and fit',
         coordinates='meters-y-up-bow-negative-z', modelUrl='/models/__SHIP_ID__.glb', hull=hull,
...
for id, name, z, bearing in MAIN:
    b['mounts'].append(dict(id=id, name=name, partId='__MAIN_PART__', battery='main', position=[0, round(deck_y(z) + .6, 3), round(z, 3)],
...
print(f'Authored __SHIP_ID__: {len(sections)} sections, {round(volume * 1.025)} t at the design waterline, {len(b["mounts"])} mounts, '
