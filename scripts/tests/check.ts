 ...
 * plausibly affect, against the known-failures ledger. `bun run build` remains the gate before a PR. `--all` runs every
...
const projects: [label: string, config: string, applies: boolean][] = [
  ...
  ['Typecheck construction, browser and harness scripts', 'tsconfig.construction.json', changed.some(file =>
    /^(scripts\/(construction|browser|diagnostics)|tools\/construction)\//.test(file) || /^scripts\/ships\/(overlay|sweep|floating)\.ts$/.test(file))],
...
const tests = testFiles(cwd), { everything, reasons } = affectedTests(cwd, touched, tests);
...
if (touched.some(file => file.startsWith('crates/'))) console.log('Rust changed: run the affected crate tests too (see crates/README.md).');
process.exit(ok ? 0 : 1);
