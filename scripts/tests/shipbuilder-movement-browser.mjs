...
await page.evaluate(async()=>{
 ...
 await mountShipbuilderReview(s,c);
...
const drag=async(delta,finish='commit',axis='X')=>{
 ...
 else if(finish==='blocked')assert((await page.locator('[data-coords]').textContent()).includes('Stopped at another block'),'Collision feedback appears during a blocked drag');
 else if(finish==='return')await page.mouse.move(px,py);
 await page.mouse.up();await page.waitForTimeout(100);
};
await drag([10,0,0],'blocked');await waitPos(2);checks.push('Real pointer capture clamps a fast X drag flush to the neighbor');
await page.keyboard.press('Meta+z');await waitPos(0);checks.push('One undo restores the entire drag');
await page.keyboard.press('Meta+Shift+z');await waitPos(2);checks.push('Redo restores the stopped position');
...
await page.keyboard.press('ArrowLeft');await waitPos(1);checks.push('Focused-axis arrow moves exactly one metre');
await drag([0,3,0],'escape','Y');await waitPos(1);checks.push('Escape cancels a real Y drag without moving the block');
await drag([0,3,0],'return','Y');await waitPos(1);checks.push('Returning the pointer to its origin leaves no move');
...
await field.fill('15');await field.press('Enter');await waitPos(2);
assert(await field.inputValue()==='2','Position entry stops at contact and displays the accepted coordinate');
await field.fill('8');await field.press('Enter');await page.waitForTimeout(80);
assert(await field.inputValue()==='2','Rejected position entry restores the unchanged coordinate');
// Body keyboard shortcuts use the same guard.
await field.blur();await page.keyboard.press('ArrowRight');await waitPos(2);
checks.push('General keyboard nudges cannot overlap blocks');
await page.keyboard.press('PageUp');await waitPos(2,1);checks.push('A touching block can slide vertically');
await page.keyboard.press('Meta+z');await waitPos(2);
