...
try {
  ...
  // Railing options apply both to the pending route and the selected fitting.
  ...
  await page.getByLabel('Rail count',{exact:true}).selectOption('2');
  ...
  if(rail.path?.heightM!==1.5||rail.path?.railCount!==2)throw new Error('Railing options were not saved');
  await height.fill('1.8');await height.press('Tab');await ready();
  await page.getByLabel('Rail count',{exact:true}).selectOption('3');await ready();
  const changed=await page.evaluate(()=>window.constructionEditor!.source().construction.equipment.at(-1)!.path);
  if(changed?.heightM!==1.8||changed?.railCount!==3)throw new Error('Selected railing options were not applied');
  ...
  if(await page.evaluate(()=>window.constructionEditor!.source().construction.equipment.at(-1)!.path!.railCount)!==2)throw new Error('Rail count undo failed');
  await page.keyboard.press('Control+z');await ready();
  ...
  if(await page.evaluate(()=>window.constructionEditor!.source().construction.equipment.length)!==13)throw new Error('Cancelled ladder changed source');
  ...
  console.log('PASS: surface ladder clicks, mirror, undo/redo, three doors and two wall vents, adjustable two/three-rail railing, option undo and cancellation with real pointer input.');
