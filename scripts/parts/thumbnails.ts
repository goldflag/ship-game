...
const recipeFiles = [
  'src/ships/componentMaterials.ts',
  'assets/parts/materials.json',
  'assets/ships/appearance/finishes.json',
  'src/ui/shipbuilding/slotImages.ts',
  'src/game/loadShipModel.ts',
  'src/game/constructionTubeGeometry.ts',
  'src/game/constructionVentModel.ts',
  'assets/parts/construction/vent_geometry.ts',
  'src/game/constructionPathModel.ts',
  'src/ships/constructionPaths.ts',
  'assets/parts/construction/path_geometry.ts',
  'assets/parts/construction/ladder_geometry.ts',
  'src/ships/constructionLadders.ts',
  'assets/parts/construction/access_geometry.ts',
  'src/game/constructionAccessModel.ts',
];
const threeVersion = JSON.parse(await readFile(join(root, 'node_modules/three/package.json'), 'utf8')).version;
const recipeHash = hash(
  JSON.stringify([threeVersion, await Promise.all(recipeFiles.map(async (path) => [path, hash(await readFile(join(root, path)))]))]),
);
...
for (const revision of await readdir(catalogsRoot))
  catalogs.push(JSON.parse(await readFile(join(catalogsRoot, revision, 'catalog.json'), 'utf8')));
...
for (const { part, catalog } of parts.values()) {
  const entry = await expected(part),
    old = previous.images[part.modelUrl];
  const bytes = await readFile(join(root, 'public', entry.url)).catch(() => undefined);
  if (
    previous.recipeHash === recipeHash &&
    old?.url === entry.url &&
    old.modelHash === entry.modelHash &&
    bytes &&
    hash(bytes) === old.imageHash
  )
    images[part.modelUrl] = old;
  else missing.push({ part, catalog, ...entry });
}
if (check) {
  if (missing.length)
    throw new Error(`Missing/stale component thumbnails (${missing.map((item) => item.part.id).join(', ')}). Run bun run part:thumbnails.`);
  ...
  try {
    ...
    await page
      .evaluate(async () => {
        const paths = ['/src/ui/shipbuilding/slotImages.ts', '/src/ui/shipbuilding/builderLayers.ts'];
    ...
    for (const { part, catalog, url, modelHash } of missing) {
      const png = await page.evaluate(
        async ({ part, catalog }) => {
          const rendererPath = '/src/ui/shipbuilding/slotImages.ts',
            palettePath = '/src/ui/shipbuilding/builderLayers.ts';
