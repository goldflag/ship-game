...
/** Keep the vendored Sky Pro bundle intact, but let browsers fetch/cache its images
...
export function vendorTextures(): Plugin {
  return {
    ...
    enforce: 'pre',
    transform(code, id) {
      if (!/\/vendor\/threejs-sky-pro\/build\/index\.js$/.test(id)) return;
      const transformed = code.replace(/(["'])data:image\/(png|jpeg|webp);base64,([A-Za-z0-9+/=]+)\1/g,
        (_literal, _quote, format: string, encoded: string) => {
          const reference = this.emitFile({
            type: 'asset',
            name: `sky-texture.${format === 'jpeg' ? 'jpg' : format}`,
