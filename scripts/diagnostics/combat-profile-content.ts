...
const entry = { id: d.id, contentHash: d.contentHash, sha256: createHash('sha256').update(bytes).digest('hex'), encoding: 'nsd1-base64', json: Buffer.from(bytes).toString('base64') };
const manifest = { ...base, ships: base.ships.map((s: {id:string}) => s.id === d.id ? entry : s), hydrostatics: base.hydrostatics.filter((s: {id:string}) => s.id !== d.id) };
await Bun.write(`${output}.manifest.json`, JSON.stringify(manifest));
await Bun.write(`${output}.nsd`, bytes);
const report = { input, contentHash: d.contentHash, runtimeBytes: bytes.length, gzipBytes: gzipSync(bytes).length, runtimeJsonBytes: Buffer.byteLength(JSON.stringify(definition)), readMs, jsonParseMs: parseMs, nsdDecodeMs: decodeMs, processMemory: process.memoryUsage(), note: 'Bun process, cold object admission with OS page cache uncontrolled; heap includes full authoring input and codec verification. Native instance/RSS measurements use runtime_bench.' };
