...
import { isOceanMapId, type OceanMapId } from '../../src/maps/catalog';
...
if (query.has('mobile') && !query.has('frame')) {
  ...
  // ?map=<id> chooses the waters (default Iron Bottom Sound) and ?range=<m> the custom spawn distance.
  ...
  const mapId = (isOceanMapId(query.get('map')) ? query.get('map') : 'iron-bottom-sound') as OceanMapId;
  const pveRequest: PveRequest | undefined = pveCount ? { version: 1, seed: 7, mapId, weather: 'partly-cloudy', difficulty: 'normal',
  ...
  function Review() {
    const [setup, setSetup] = useState<BattleSetup>({ playerShipId: 'bismarck', friendlyBots: roster(1), enemies: bots ? roster(4) : ['bismarck'], spawnDistance: Number(query.get('range')) || 5000, mapId });
