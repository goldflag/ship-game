...
for (let window = 0; window < 3; window++) {
  ...
  for (let tick = 0; tick < 3600; tick += 1) {
    ...
    // The worker's actual path: Rust encodes the update against its last frame.
    t = performance.now(); runtime.snapshot_delta([]); ms.delta += performance.now() - t;
    batches++; shells = Math.max(shells, next.shells.length);
  }
