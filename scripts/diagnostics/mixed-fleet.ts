...
export function mixedSetup(perTeam = 10, ships: readonly string[] = MIXED_SHIPS) {
  if (!Number.isInteger(perTeam) || perTeam < 2 || perTeam > 30) throw new Error('Choose 2–30 ships per team.');
  const team = Array.from({ length: perTeam }, (_, i) => ships[i % ships.length]);
  return { playerShipId: team[0], friendlyBots: team.slice(1), enemies: [...team], spawnDistance: 5000 };
}
export function mixedSimulation(perTeam = 10, ships: readonly string[] = MIXED_SHIPS) {
  const setup = mixedSetup(perTeam, ships);
