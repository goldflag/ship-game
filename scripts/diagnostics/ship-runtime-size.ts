...
const out = process.argv[3] ?? '.build/runtime-size';
await mkdir(out, { recursive: true });
const start = performance.now(),
  source = await Bun.file(`public/models/${id}.json`).text(),
  readMs = performance.now() - start;
let t = performance.now();
const def = JSON.parse(source),
  parseMs = performance.now() - t;
const sections = Object.entries(def).map(([section, value]) => {
  const bytes = Buffer.from(JSON.stringify(value));
  return { section, bytes: bytes.length, gzipBytes: gzipSync(bytes).length, count: Array.isArray(value) ? value.length : undefined };
});
t = performance.now();
const encoded = encodeRuntimeDefinition(runtimeProjection(def)),
  encodeMs = performance.now() - t;
t = performance.now();
const decoded = decodeRuntimeDefinition(encoded),
...
const report = {
  ...
  runtimeSha256: sha(encoded),
  bytes: Buffer.byteLength(source),
  runtimeBytes: encoded.length,
  gzipBytes: gzipSync(source).length,
  runtimeGzipBytes: gzipSync(encoded).length,
  readMs,
  parseMs,
  encodeMs,
  decodeMs,
  exactRuntimeFields: true,
  omitted: [
    'construction.primitives',
    'construction.surfaces',
    'construction.boundaries',
    'construction.loads',
    'loading.contributions except equipment power ratings',
  ],
  sections,
};
await Bun.write(`${out}/${id}.json`, JSON.stringify(report, null, 2));
