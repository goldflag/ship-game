...
test('representative open-sea heights reach the CPU sea in metres', () => {
...
test('height grows continuously over the full slider range and calm water has no foam source', () => {
  for (const map of OCEAN_MAPS) {
    ...
    expect(calm.significantHeightM).toBe(0); expect(calm.crestFoam).toBe(0); expect(calm.windwardFoam).toBe(0);
  }
});
