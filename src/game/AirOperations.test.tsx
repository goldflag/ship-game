...
import { makeTestEnvironment, makeTestInput, makeTestRig } from './testing/fakes';
...
test('every carrier squadron card references a published PNG thumbnail', () => {
  for (const id of Object.keys(shipPresets)) {
    ...
    const html = renderToStaticMarkup(<AirOperations data={{ ship: sim.ship, order: 1, camera: 'Chase', trail: [], fps: 60, airOperationsOpen: true, combat: sim.telemetry('main', [0, 0, -5000]) }} desk={null} bindings={defaultKeybindings()}/>);
...
test('hiding carrier instruments leaves the map navigation surface interactive', () => {
  ...
  const data = { ship: simulation.ship, order: 1, camera: 'Chase' as const, trail: [], fps: 60, airOperationsOpen: true, combat: simulation.telemetry('main', [0, 0, -5000]) };
...
test('merged air groups retain selection through snapshots and deduplicate subsequent merges', () => {
  ...
  const html = renderToStaticMarkup(<FleetCommand data={{ ship: simulation.ship, order: 1, camera: 'Chase', trail: [], fps: 60, airOperationsOpen: true, combat: simulation.telemetry('main', [0, 0, -5000]) }} desk={fleetDesk(game)} bindings={defaultKeybindings()}/>);
...
test('selected route points toward a distant target through the zoom level where it passes behind the camera', () => {
  ...
  for (const radius of [8000, 4000, 2000, 1000, 600, 300]) {
    ...
    const html = renderToStaticMarkup(<AirOperations data={{ ship: simulation.ship, order: 1, camera: 'Chase', trail: [], fps: 60, airOperationsOpen: true, selectedFlightId: flight.id, combat }} desk={fleetDesk(game)} bindings={defaultKeybindings()}/>);
...
test('the operations roster exposes the full inventory and retains a flight target when the ship target changes', () => {
  ...
  const html = renderToStaticMarkup(<AirOperations data={{ ship: sim.ship, order: 1, camera: 'Chase', trail: [], fps: 60, airOperationsOpen: true, selectedFlightId: sim.player.airWing!.flights[0].id, combat: sim.telemetry('main', [0, 0, -5000]) }} desk={null} bindings={defaultKeybindings()}/>);
...
test('the map releases aim, blocks firing and restores aiming on close without losing flight selection', () => {
  ...
  const rig = makeTestRig({ setEnabled: mock(), capturePointer: mock(), setShellView: mock(), update: mock() });
  const game = Object.assign(Object.create(Game.prototype), { simulation, shellFollow: new ShellFollow(), input: makeTestInput({ clear: mock() }), rig, battlefieldCamera: new BattlefieldCamera(new PerspectiveCamera()), host: { clientWidth: 1280, clientHeight: 800 },
...
for (const [width, height] of [[2048, 1176], [390, 844]]) test(`carrier water covers maximum zoom and pan at ${width}×${height}`, () => {
  ...
  // The actual surface geometry and horizon growth, without a GPU.
  const surface = new OceanGeometry(16, camera.far);
  const ocean = { ensureHorizon: (far: number) => surface.ensureHorizon(far) };
  const simulation = new CombatSimulation(shipPreset('enterprise-cv6'));
  const battlefieldCamera = new BattlefieldCamera(camera);
  const rig = makeTestRig();
  const game = Object.assign(Object.create(Game.prototype), { camera, simulation, ocean, rig, battlefieldCamera, shellFollow: new ShellFollow(), input: makeTestInput(),
  ...
  try {
    ...
    const mesh = surface.mesh.geometry;
    for (const x of [-1, 1]) for (const y of [-1, 1]) {
      ...
      expect(point.clone().project(camera).z).toBeLessThan(1);
      expect(surface.covers(point.x, point.z)).toBe(true);
    }
    game.setAirOperationsOpen(false);
    battlefieldCamera.cancelTransition();
    game.setAirOperationsOpen(true);
    expect(surface.mesh.geometry).toBe(mesh); // Reopening reuses the expanded ring.
...
test('closing the carrier map restores the chosen weather visibility', () => {
  ...
  for (const weather of ['clear', 'storm-clouds', 'fog'] as const) {
    ...
    expect(authored).toMatchObject({ start: expected.fog.start, end: expected.fog.end, color: expected.fog.color.slice(1), skyBlendDistance: expected.fog.skyBlend });
    const game = Object.assign(Object.create(Game.prototype), { camera, simulation, ocean, rig, environment, shellFollow: new ShellFollow(), input: makeTestInput(),
    ...
    game.setAirOperationsOpen(true); expect(ocean.fog.end).toBe(900000); expect(ocean.fog.start).toBe(400000);
    game.setAirOperationsOpen(false);
    expect({ ...ocean.fog, color: ocean.fog.color.getHexString() }).toEqual(authored);
  }
});
...
test('multiple selected squadrons highlight cards and manifest rows and expose both roles', () => {
  ...
  const html = renderToStaticMarkup(<AirOperations data={{ ship: sim.ship, order: 1, camera: 'Chase', trail: [], fps: 60, airOperationsOpen: true, selectedFlightIds: [fighter.id, bomber.id], combat }} desk={null} bindings={defaultKeybindings()}/>);
