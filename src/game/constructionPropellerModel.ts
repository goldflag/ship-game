...
import type { ConstructionPropellerSupport } from '../ships/blueprint';
...
export function createConstructionPropellerSupports(supports: readonly ConstructionPropellerSupport[] = []): THREE.Group {
  ...
  for (const support of supports) {
    ...
    support.members.forEach((member, index) => {
      ...
      const material = new THREE.MeshStandardMaterial({ color: constructionPaintColor(member.kind === 'shaft' ? 'dark-gray' : 'red-oxide'), roughness: CONSTRUCTION_FINISH.steelRoughness, metalness: CONSTRUCTION_FINISH.metalness });
