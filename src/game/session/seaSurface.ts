/** The sea's long-wave surface as the Rust authority samples it: pure maths, no map data. Ship
 * geometry and machinery import this module, so map edits never touch ship model fingerprints. */
...
export function seaHeight(sea: SeaState, x: number, z: number, time: number): number {
  return seaHeightFelt(sea, x, z, time, 0);
}
/** The authority's roll sample: each component's height weighted by the pressure it still has `depth` below the
 * surface, e^(−k·depth), so shorter waves fade faster. At depth 0 this is `seaHeight`. */
export function seaHeightFelt(sea: SeaState, x: number, z: number, time: number, depth: number): number {
  if (sea.amplitudeM === 0) return 0;
  const sample = (direction: number, wavelength: number, phase: number) => {
    const k = 2 * Math.PI / wavelength;
    return Math.sin(k * (x * Math.cos(direction) + z * Math.sin(direction)) - Math.sqrt(9.81 * k) * time + phase) * Math.exp(-k * depth);
  };
...
/** The authority's `ROLL_PRESSURE_DEPTH`: roll feels the waves at half the draft (the Smith effect). */
export const ROLL_PRESSURE_DEPTH = .5;
 ...
 * for sample: mean height under the waterplane, the wave slope across 80% of the
 * length, and across 80% of the beam as felt at half the draft. The berth rides
 * this without a stepped session. */
export function seaResponse(sea: SeaState, hull: { length: number; beam: number; draft: number }, pose: { x: number; z: number; heading: number }, time: number): SeaResponse {
  ...
  const felt = (x: number) => seaHeightFelt(sea, pose.x + cos * x, pose.z + sin * x, time, hull.draft * ROLL_PRESSURE_DEPTH);
  let heave = 0;
  for (const z of [-.4, -.2, 0, .2, .4]) for (const x of [-.3, .3]) heave += at(x * hull.beam, z * hull.length) / 10;
  const clamp = (value: number, limit: number) => Math.max(-limit, Math.min(limit, value));
  return { heave,
    roll: clamp((felt(hull.beam * .4) - felt(-hull.beam * .4)) / (hull.beam * .8), .18),
