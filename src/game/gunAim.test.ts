...
test('loaded guns show their current direction before traversing and converge on the commanded aim', () => {
  ...
  expect(untrained.every(point => point.status === 'ready' && !point.aligned)).toBe(true);
  ...
  // The solver trains from barrel 0; the circle represents the twin's midpoint.
  expect(new Vector3(...trained.point).distanceTo(new Vector3(...aim))).toBeLessThan(mount.weapon.barrelSpacing / 2 + .1);
...
test('aim circles agree with actual short-shot splashes, including inherited ship velocity', () => {
  ...
  sim.step({ throttle: 1, rudder: 0 }, { aim, fire: true, battery: 'main' });
  ...
  expect(prediction.point[1]).toBe(0);
  expect(prediction.aligned).toBe(false);
