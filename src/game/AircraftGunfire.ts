...
export class AircraftGunfire {
  ...
  private write(mesh: THREE.InstancedMesh, index: number, opacity: number) {
    this.dummy.updateMatrix(); mesh.setMatrixAt(index, this.dummy.matrix);
    (mesh.geometry.getAttribute('tracerOpacity') as THREE.InstancedBufferAttribute).setX(index, opacity);
  }
  update(sim: CombatSimulation, camera: THREE.Camera) {
    ...
    for (const mesh of [this.ribbons, this.cores, this.tips, this.muzzles]) {
      ...
      mesh.instanceMatrix.array.fill(0, used * 16); mesh.instanceMatrix.needsUpdate = true;
      const opacity = mesh.geometry.getAttribute('tracerOpacity') as THREE.InstancedBufferAttribute;
      opacity.array.fill(0, used); opacity.needsUpdate = true;
    }
  }
