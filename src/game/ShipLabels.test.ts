...
test('overhead condition percentages, meters and loss spans use each ship maximum', () => {
  ...
  try {
    for (const id of ['yamato', 'baltimore']) {
      ...
      const fraction = Number(host.find('ship-label-fill')!.style.transform.slice('scaleX('.length, -1));
      expect(fraction).toBeCloseTo(.6, 4); // Displayed HP is rounded to whole points.
      expect(parseFloat(host.find('ship-label-loss')!.style.left)).toBeCloseTo(60, 8);
      expect(parseFloat(host.find('ship-label-loss')!.style.width)).toBeCloseTo(40, 8);
