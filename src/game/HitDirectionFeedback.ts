...
export class HitDirectionFeedback {
  ...
  update(simulation: CombatSimulation, viewBearing: number): HitDirectionCue[] {
    ...
    for (const event of simulation.events) {
      ...
      if (event.shipId !== simulation.player.motion.id || !['penetration', 'stopped', 'ricochet', 'module'].includes(event.kind)) continue;
      const shell = event.shell, hitTime = event.tick * FIXED_DT;
