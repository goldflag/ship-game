 ...
 * ones the transmission and shoreline terms read, so reflections add no texture units. */
...
/** The underwater view composed over the scene pass; the stub leaves the frame unchanged. */
export function underwaterPost(_scenePass: PassNode, color: Node<'vec4'>): Node<'vec4'> {
  return color;
}
