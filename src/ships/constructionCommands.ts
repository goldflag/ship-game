...
export type ConstructionCommand =
  ...
  | {
      op: 'surface-patch';
      targets: Pick<ConstructionSurfaceAssignment, 'primitiveId' | 'face' | 'panelId'>[];
...
export function applyConstructionBatch(
  ...
  if (batch.version !== 1 || !Array.isArray(batch.commands) || batch.commands.length > 10_000 || typeof batch.label !== 'string')
    throw new Error('Expected a version-1 command batch with at most 10,000 commands.');
  ...
  for (const command of batch.commands) {
    switch (command.op) {
      ...
      case 'primitive-patch': {
        ...
        upsert(data.primitives, patchPrimitive(part, command.changes));
        break;
      }
      case 'hull-sections': {
      ...
      case 'hull-station': {
        ...
        if (
          ...
          Object.keys(command.changes).some((key) => !['t', 'points'].includes(key))
        )
          throw new Error('Section changes accept t and points only.');
      ...
      case 'equipment-patch': {
        ...
        equipment(patchEquipment(part, command.changes));
        break;
      }
      case 'copy': {
        ...
        if (
          ...
          (!Array.isArray(command.offset) || command.offset.length !== 3 || !command.offset.every(Number.isFinite))
        )
          throw new Error('Copy offset must contain three finite coordinates.');
        const ids = requireIds(command.copies.map((p) => p.from));
        const destinations = new Map(command.copies.map((p) => [p.from, p.to]));
        const known = new Set([...data.primitives, ...data.equipment, ...data.boundaries, ...data.loads].map((p) => p.id));
        if (
          ids.size !== command.copies.length ||
          new Set(destinations.values()).size !== ids.size ||
          [...destinations.values()].some((id) => typeof id !== 'string' || !id.length || known.has(id))
        )
          throw new Error('Copy destination IDs must be new and unique; source IDs must not repeat.');
      ...
      case 'load':
        ...
        break;
      case 'remove':
        removeConstructionSelection(draft, requireIds(command.ids));
        break;
      case 'move': {
      ...
      case 'rotate': {
        const ids = requireIds(command.ids);
        if (data.boundaries.some((w) => ids.has(w.id)) || data.loads.some((l) => ids.has(l.id)))
          throw new Error('Rotate accepts hull pieces and equipment only.');
      ...
      case 'surface': {
        ...
        if (
          panelId !== undefined &&
          !customHullPanels(data.primitives.find((p) => p.id === primitiveId)!).some(
            (panel) => panel.face === face && panel.panelId === panelId,
          )
        )
          throw new Error('Unknown custom hull panel.');
      ...
      case 'surface-patch': {
        if (
          ...
          typeof command.changes !== 'object' ||
          Array.isArray(command.changes) ||
          Object.keys(command.changes).some((key) => !['thicknessMm', 'material', 'paint', 'open'].includes(key))
        )
          throw new Error('Expected surface targets and armor, material, paint or opening changes.');
        ...
        for (const target of command.targets) {
          ...
          if (target.panelId !== undefined && !customHullPanels(part).some((p) => p.face === target.face && p.panelId === target.panelId))
            throw new Error('Unknown custom hull panel.');
        ...
        assignConstructionSurfaces(draft, keys, command.changes);
        break;
      }
      case 'vertices': {
        ...
        if (
          !['vertex', 'edge', 'face', 'ring'].includes(command.selection.mode) ||
          !Number.isInteger(command.selection.index) ||
          !selectionCorners(command.selection, editableMesh(primitive)).length
        )
          throw new Error('Unknown vertex, edge, face or ring.');
        replaceVertexPrimitives(
          draft,
          freeformEdit(
            draft,
            ...
            command.selection,
            command.delta,
            command.mirror ?? [false, false, false],
            command.nearby ?? false,
          ),
        );
        break;
      }
      default:
        throw new Error('Unknown construction command.');
    }
  }
...
export function constructionDiffCommands(before: ConstructionSource, after: ConstructionSource): ConstructionCommand[] {
  ...
  for (const surface of b.surfaces)
    if (!kept.has(key(surface)) && a.primitives.some((part) => part.id === surface.primitiveId))
      throw new Error('A face assignment cannot be removed by command; assign its default values instead.');
