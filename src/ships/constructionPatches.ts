import type { ConstructionEquipment, ConstructionFittingDefinition, ConstructionPrimitive } from './blueprint';
import { EQUIPMENT_PATCH, FITTING_PATCH, PRIMITIVE_PATCH, type ObjectSpec } from './constructionCommandSchema';
...
export type FittingPatch = Patch<Omit<ConstructionFittingDefinition, 'id'>>;
...
export const FITTING_PATCH_FIELDS = patchFields(FITTING_PATCH);
...
export function patchFitting(def: ConstructionFittingDefinition, changes: FittingPatch): ConstructionFittingDefinition {
  const next = structuredClone(def); merge(next as unknown as Record<string, unknown>, changes, FITTING_PATCH_FIELDS); return next;
}
