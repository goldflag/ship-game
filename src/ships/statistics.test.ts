...
test('every preset prints a complete sheet whose figures come from the compiled definition', () => {
  for (const id of Object.keys(shipPresets)) {
    ...
    expect(sections.find((s) => s.id === 'model-basis')!.notes!.map((n) => n.text)).toEqual([
      ...
      def.accuracy.internals,
      def.accuracy.weapons,
    ]);
  }
});
...
test('category scores stay within 0-100 and separate the presets by their simulation data', () => {
  const scores = Object.fromEntries(
    Object.keys(shipPresets).map((id) => [id, Object.fromEntries(shipScores(shipPreset(id)).map((s) => [s.id, s.score]))]),
  );
  for (const ship of Object.values(scores))
    for (const value of Object.values(ship)) {
  ...
  expect(shipScores(shipPreset('bismarck')).map((s) => s.id)).toEqual([
    'survivability',
    'artillery',
    'airDefense',
    'maneuverability',
...
test('air defense excludes small guns that cannot elevate to engage aircraft', async () => {
  ...
  def.mounts.forEach((mount) => {
    mount.weapon.elevationMaxDeg = 45;
  });
...
test('maximum range follows the low-arc solver: elevation limited, then capped at 30 km', () => {
  ...
  expect(
    maximumRangeM({ ...bismarck, ballistics: { ...bismarck.ballistics, dragPerSecond: 0 }, muzzleSpeed: 400, elevationMaxDeg: 85 }),
  ).toBeCloseTo(400 ** 2 / 9.81, 3);
  expect(
    maximumRangeM({ ...bismarck, ballistics: { ...bismarck.ballistics, dragPerSecond: 0 }, muzzleSpeed: 400, elevationMaxDeg: 30 }),
  ).toBeCloseTo((400 ** 2 * Math.sin(Math.PI / 3)) / 9.81, 3);
