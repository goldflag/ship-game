...
export function paintedHullFace(surface: ConstructionSurface, primitive?: ConstructionPrimitive, normals = surface.vertices.map(() => surface.normal)): PaintedHullFace[] {
  ...
  const y = primitive!.position[1] + localY;
  if (original.every(v => v.point[1] >= y)) return [face(original, surface.paint)];
  if (original.every(v => v.point[1] <= y)) return [face(original, 'red-oxide')];
  const clip = (below: boolean): Vertex[] => {
    const out: Vertex[] = [];
    for (let i = 0; i < original.length; i++) {
      const a = original[i], b = original[(i + 1) % original.length];
      const inside = (v: Vertex) => below ? v.point[1] <= y : v.point[1] >= y;
      if (inside(a)) out.push(a);
      if (inside(a) !== inside(b)) {
        const t = (y - a.point[1]) / (b.point[1] - a.point[1]);
