...
export const CONSTRUCTION_CONVENTIONS = {
  ...
  primitive: 'size [x,y,z] is the envelope centred at position; custom-hull size is [beam, depth, length]; rotationDeg is yaw, optional tilt adds pitch/roll (YXZ)',
...
export const CONSTRUCTION_SOURCE_LIMITS = { ...CONSTRUCTION_LIMITS, loads: CONSTRUCTION_LIMITS.equipment, sourceBytes: 16_000_000, idCharacters: 64, pathPoints: 64 } as const;
...
export type QueryTable = 'primitives' | 'equipment' | 'boundaries' | 'loads';
...
export interface Selector { ids?: string[]; kind?: string; part?: string; prefix?: string }

const round = (n: number, digits = 3) => { const v = Number(n.toFixed(digits)); return v === 0 ? 0 : v; };
...
const corners = (center: Vec3, size: Vec3): Vec3[] =>
  [-1, 1].flatMap((x) => [-1, 1].flatMap((y) => [-1, 1].map((z) => [center[0] + (x * size[0]) / 2, center[1] + (y * size[1]) / 2, center[2] + (z * size[2]) / 2] as Vec3)));
const partOf = (catalog: ConstructionCatalog | undefined, item: ConstructionEquipment) => catalog?.equipment.find((p) => p.id === item.partId);
...
export function closestIds(id: string, known: readonly string[], count = 3): string[] {
  ...
  return known
    .map((candidate) => ({ candidate, score: candidate.toLowerCase().includes(lower) || lower.includes(candidate.toLowerCase()) ? 0 : distance(lower, candidate.toLowerCase()) }))
...
interface Entry { table: QueryTable; id: string; kind: string; part?: string; record: object }
function entries(source: ConstructionSource, catalog?: ConstructionCatalog): Entry[] {
  ...
  return [
    ...
    ...c.equipment.map((record): Entry => ({ table: 'equipment', id: record.id, kind: partOf(catalog, record)?.kind ?? 'unknown', part: record.partId, record })),
...
function select(source: ConstructionSource, catalog: ConstructionCatalog | undefined, selector: Selector): Entry[] {
  ...
  if (selector.ids) {
    ...
    if (unknown.length)
      throw new Error(
        'Unknown ID' + (unknown.length > 1 ? 's' : '') + ': ' +
          unknown.map((id) => { const near = closestIds(id, [...known]); return id + (near.length ? ' (closest: ' + near.join(', ') + ')' : ' (no similar ID)'); }).join('; ') +
          '. Run summary for every ID.',
      );
  }
  const ids = selector.ids && new Set(selector.ids);
  return all.filter((e) => (!ids || ids.has(e.id)) && (!selector.kind || e.kind === selector.kind) && (!selector.part || e.part === selector.part) && (!selector.prefix || e.id.startsWith(selector.prefix)));
}
...
export function primitiveBounds(p: ConstructionPrimitive): BoundsRow {
  ...
  return { id: p.id, table: 'primitives', kind: p.kind, ...box(envelopeVertices(p).map((v) => worldVertex(p, v))), approximate: why.length > 0, ...(why.length ? { why } : {}) };
}
export function equipmentBounds(item: ConstructionEquipment, part: ConstructionEquipmentPart | undefined): BoundsRow {
  if (!part)
    return { id: item.id, table: 'equipment', kind: 'unknown', part: item.partId, datum: vec(item.position), min: vec(item.position), max: vec(item.position), approximate: true, why: ['part absent from the catalog; datum only'] };
  ...
  const bounds = world(center, local.size), why: string[] = [];
  ...
  if (rise > 0) { bounds.min[1] = round(bounds.min[1] - rise); why.push('includes the turret rise support'); }
  ...
  return { id: item.id, table: 'equipment', kind: part.kind, part: part.id, datum: vec(item.position), ...bounds, approximate: why.length > 0, ...(why.length ? { why } : {}), ...(spaces ? { occupancy: spaces } : {}) };
}
function allBounds(source: ConstructionSource, catalog: ConstructionCatalog | undefined, chosen?: Entry[]): BoundsRow[] {
  return (chosen ?? entries(source, catalog)).flatMap((e): BoundsRow[] =>
    e.table === 'primitives' ? [primitiveBounds(e.record as ConstructionPrimitive)]
    : e.table === 'equipment' ? [equipmentBounds(e.record as ConstructionEquipment, partOf(catalog, e.record as ConstructionEquipment))]
    : e.table === 'loads' ? [{ id: e.id, table: 'loads', kind: 'load', ...box(corners((e.record as { center: Vec3 }).center, (e.record as { size: Vec3 }).size)), approximate: false }]
    : []);
}
...
export function constructionBounds(source: ConstructionSource, catalog: ConstructionCatalog | undefined, selector: Selector = {}, all = false) {
  const chosen = hasSelector(selector) ? select(source, catalog, selector) : undefined;
  const planes = (chosen ?? []).filter((e) => e.table === 'boundaries').map((e) => { const b = e.record as { axis: string; offset: number }; return { id: e.id, table: e.table, plane: { axis: b.axis, offset: b.offset } }; });
  ...
  return {
    ...
    ...(!chosen && !all && source.construction.equipment.length ? { omitted: source.construction.equipment.length + ' equipment rows; select with --ids, --kind, --part or --prefix, or pass --all' } : {}),
  };
}
...
export interface NearQuery { point?: Vec3; radius?: number; box?: Aabb; limit?: number; kind?: string }
export function constructionNear(source: ConstructionSource, catalog: ConstructionCatalog | undefined, query: NearQuery) {
  ...
  if (query.point && !(Number.isFinite(query.radius) && query.radius! >= 0)) throw new Error('A point query needs a radius of 0 m or more.');
  const region: Aabb = query.box ? box([query.box.min, query.box.max]) : { min: query.point!, max: query.point! };
  const reach = query.box ? 0 : query.radius!, middle = region.min.map((n, k) => (n + region.max[k]) / 2);
  const hits = allBounds(source, catalog)
    .filter((row) => !query.kind || row.kind === query.kind)
    .map((row) => ({ row, distance: length(gapTo(row, region)), centre: Math.hypot(...row.min.map((n, k) => (n + row.max[k]) / 2 - middle[k])) }))
  ...
  return {
    ...
    hits: hits.slice(0, limit).map(({ row, distance }) => ({ id: row.id, table: row.table, kind: row.kind, ...(row.part ? { part: row.part } : {}), distance, min: row.min, max: row.max, approximate: row.approximate })),
  };
}
export function constructionBetween(source: ConstructionSource, catalog: ConstructionCatalog | undefined, idA: string, idB: string) {
  ...
  const gap = gapTo(a, b);
  return {
    note: NOTE + ' gap is per axis [x,y,z] between the two boxes; negative is the overlap depth on that axis. The boxes intersect only when all three are negative.',
...
// ── Exact records ───────────────────────────────────────────────────────────
export function constructionGet(source: ConstructionSource, catalog: ConstructionCatalog | undefined, selector: Selector, options: { fields?: string[]; surfaces?: boolean } = {}) {
  if (!hasSelector(selector)) throw new Error('Select records with --ids, --kind, --part or --prefix.');
  const chosen = select(source, catalog, selector), fields = options.fields;
  if (fields) {
    const known = new Set(chosen.flatMap((e) => Object.keys(e.record)));
    const unknown = fields.filter((f) => !known.has(f));
    if (chosen.length && unknown.length) throw new Error('No selected record has field ' + unknown.join(', ') + '. Available: ' + [...known].sort().join(', ') + '.');
  }
  const project = (record: object) => (fields ? Object.fromEntries(Object.entries(record).filter(([key]) => key === 'id' || fields.includes(key))) : record);
...
// ── Summary ─────────────────────────────────────────────────────────────────
export function constructionSummary(source: ConstructionSource, catalog: ConstructionCatalog | undefined, options: { kind?: string; positions?: 'all' | 'none'; sourceBytes?: number } = {}) {
  const c = source.construction;
  ...
  return {
    ...
    limits: {
      ...
      equipment: used(c.equipment.length, CONSTRUCTION_SOURCE_LIMITS.equipment),
    ...
    primitives: c.primitives.filter((p) => wanted(p.kind)).map((p) => [p.id, p.kind, vec(p.position), vec(p.size), p.tilt ? vec([p.tilt.pitchDeg, p.rotationDeg, p.tilt.rollDeg]) : round(p.rotationDeg), assigned.get(p.id) ?? 0]),
    ...
    equipment: Object.fromEntries(Object.keys(kinds).filter(wanted).map((kind) => [kind, Object.fromEntries([...groups.values()].filter((g) => g.kind === kind).map((g) => [
      g.part,
      placed(kind) ? Object.fromEntries(g.items.map((item) => [item.id, [...vec(item.position), round(item.bearingDeg)]])) : g.items.map((item) => item.id),
    ]))])),
...
export function compactJson(value: unknown, width = 320, indent = ''): string {
  ...
  const parts = items
    ...
    : Object.entries(value).filter(([, item]) => item !== undefined).map(([key, item]) => JSON.stringify(key) + ':' + compactJson(item, width, inner));
  return (items ? '[' : '{') + '\n' + parts.map((part) => inner + part).join(',\n') + '\n' + indent + (items ? ']' : '}');
}

/** `ship:catalog` filters. Without options every part is returned unchanged, as before. */
export function catalogParts(catalog: ConstructionCatalog, options: { query?: string; kind?: string; ids?: string[]; brief?: boolean } = {}) {
  const known = catalog.equipment.map((part) => part.id), unknown = (options.ids ?? []).filter((id) => !known.includes(id));
  if (unknown.length) throw new Error('Unknown catalog part' + (unknown.length > 1 ? 's' : '') + ': ' + unknown.map((id) => { const near = closestIds(id, known); return id + (near.length ? ' (closest: ' + near.join(', ') + ')' : ''); }).join('; ') + '.');
  const kinds = [...new Set(catalog.equipment.map((part) => part.kind))];
  if (options.kind && !kinds.includes(options.kind as ConstructionEquipmentPart['kind'])) throw new Error('Unknown equipment kind ' + options.kind + '. Kinds: ' + kinds.join(', ') + '.');
  const query = (options.query ?? '').toLowerCase();
  const parts = catalog.equipment.filter((part) => (!options.ids || options.ids.includes(part.id)) && (!options.kind || part.kind === options.kind) && [part.id, part.name, part.kind].some((value) => value.toLowerCase().includes(query)));
  return options.brief
    ? parts.map(({ id, name, kind, placement, size, boundsCenter, wallMount, path }) => ({ id, name, kind, placement, size, boundsCenter, ...(wallMount ? { wallMount } : {}), ...(path ? { path: path.kind } : {}) }))
    : parts;
}
