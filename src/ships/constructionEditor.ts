...
import { constructionShipPaint, isConstructionSurfaceFinish, isConstructionWear } from './constructionPaints';
...
export function editableConstructionSurfaces(source: ConstructionSource, surfaces: readonly ConstructionSurface[]): ConstructionSurface[] {
  ...
  return surfaces.filter(
    (surface) => primitiveIds.has(surface.primitiveId) && CONSTRUCTION_FACES.includes(surface.face as ConstructionFace),
  );
}
...
export function projectConstructionSurfaces(source: ConstructionSource, surfaces: readonly ConstructionSurface[]): ConstructionSurface[] {
  ...
  return surfaces.map((surface) => {
    ...
    const assignment =
      assignments.get(surfaceSelectionKey(surface)) ??
      (surface.panelId !== undefined ? assignments.get(surfaceKey(surface.primitiveId, surface.face)) : undefined);
    ...
    const material = assignment?.material ?? 'steel',
      paint = assignment?.paint ?? constructionShipPaint(source),
      open = !!assignment?.open;
    return surface.thicknessMm === thicknessMm && surface.material === material && surface.paint === paint && surface.open === !!open
      ? surface
      : { ...surface, thicknessMm, material, paint, open: !!open };
  });
}
...
export function decodeConstructionSource(value: unknown): ConstructionSource {
  ...
  const vector = (value: unknown, path: string) => {
    if (!Array.isArray(value) || value.length !== 3) throw new Error(`${path} needs three coordinates`);
    value.forEach((v) => number(v, path));
  };
  const rows = (value: unknown, path: string) => {
    if (!Array.isArray(value)) throw new Error(`${path} must be an array`);
    return value.map((v) => object(v, path));
  };
  const source = object(value, 'Source');
  if (source.schemaVersion !== 1 || source.coordinates !== 'meters-y-up-bow-negative-z')
    throw new Error('Unsupported source version or coordinates');
  ...
  const piece = (p: Record<string, unknown>) => {
    ...
    number(p.rotationDeg, 'Primitive rotation');
    if (p.smoothGroup !== undefined) string(p.smoothGroup, 'Smooth group');
    if (
      typeof p.kind !== 'string' ||
      (p.kind !== 'vertex' && p.kind !== 'custom-hull' && p.kind !== 'balcony' && !Object.hasOwn(CONSTRUCTION_SHAPES, p.kind))
    )
    ...
    if (p.kind === 'balcony') {
      ...
      number(balcony.heightM, 'Edge height');
      number(balcony.wallThicknessM, 'Wall thickness');
      ...
      for (const point of points) {
        ...
        number(point.x, 'Point X');
        number(point.z, 'Point Z');
        if (ids.has(point.id as string) || !['open', 'wall', 'railing', 'triple-railing'].includes(String(point.edge)))
          throw new Error('Invalid balcony point identity or edge');
    ...
    if (p.tilt !== undefined) {
      ...
      for (const key of ['pitchDeg', 'rollDeg']) {
        number(tilt[key], key);
        if (Math.abs(tilt[key] as number) > 3600) throw new Error('Block tilt must be within ±3600°');
      }
    }
    if (p.kind === 'custom-hull') {
      ...
      if (hull.redPaintY !== undefined) {
        number(hull.redPaintY, 'Red paint Y');
        if (Math.abs(hull.redPaintY as number) > 500) throw new Error('Red paint Y must be between −500 and 500 m');
      }
      if (hull.paintBands !== undefined) {
        const error = hullPaintBandsError(hull.paintBands);
        if (error) throw new Error(error);
      }
      if (hull.bilgeKeels !== undefined) {
        const keel = object(hull.bilgeKeels, 'Bilge keels');
        const error = bilgeKeelError(keel as unknown as import('./blueprint').ConstructionBilgeKeels);
        if (error) throw new Error(error);
      }
      ...
      for (const station of stations) {
        ...
        for (const point of points) {
          number(point.x, 'Point X');
          number(point.y, 'Point Y');
          if (point.contour !== undefined) number(point.contour, 'Outline position');
        }
      }
    ...
    if (p.mesh !== undefined) {
      ...
      if (
        ...
        p.shaping !== undefined ||
        mesh.version !== 1 ||
        !['prism', 'rings', 'polyhedron'].includes(String(mesh.family))
      )
        throw new Error('Unsupported freeform topology');
      ...
      if (
        ...
        mesh.vertices.length > 256 ||
        !Array.isArray(mesh.reference) ||
        mesh.reference.length !== mesh.vertices.length
      )
        throw new Error('Freeform requires 4–256 vertices and matching mirror references');
      mesh.vertices.forEach((v) => vector(v, 'Freeform vertex'));
      mesh.reference.forEach((v) => vector(v, 'Mirror reference'));
      const index = (n: unknown) => typeof n === 'number' && Number.isInteger(n) && n >= 0 && n < (mesh.vertices as unknown[]).length;
      const faces = rows(mesh.faces, 'Freeform faces'),
        ids = new Set<string>();
      if (faces.length < 4 || faces.length > 256) throw new Error('Freeform requires 4–256 faces');
      for (const f of faces) {
        string(f.id, 'Face ID');
        if (
          (f.id as string).length > 80 ||
          ids.has(f.id as string) ||
          !CONSTRUCTION_FACES.includes(f.name as ConstructionFace) ||
          !Array.isArray(f.corners) ||
          f.corners.length < 3 ||
          f.corners.length > 64 ||
          !f.corners.every(index) ||
          new Set(f.corners).size !== f.corners.length
        )
          throw new Error('Invalid freeform face');
        ids.add(f.id as string);
      }
      if (
        !Array.isArray(mesh.rings) ||
        mesh.rings.length > 24 ||
    ...
    if (p.shaping !== undefined) {
      ...
      if (p.kind !== 'vertex' || shape.version !== 1 || !['round', 'chamfer'].includes(String(shape.style)))
        throw new Error('Unsupported freeform shaping');
      number(shape.radius, 'Radius');
      if (
        !Array.isArray(shape.edges) ||
        shape.edges.length > 12 ||
        shape.edges.some((e) => !Number.isInteger(e) || e < 0 || e > 11) ||
        new Set(shape.edges).size !== shape.edges.length
      )
        throw new Error('Invalid edge treatment selection');
    }
  ...
  for (const surface of rows(data.surfaces, 'Surfaces')) {
    ...
    string(surface.paint, 'Paint');
    number(surface.thicknessMm, 'Armor thickness');
    if (!CONSTRUCTION_FACES.includes(surface.face as ConstructionFace) || !['steel', 'armor-steel'].includes(surface.material as string))
      throw new Error('Unsupported surface face or material');
    if (
      surface.panelId !== undefined &&
  ...
  for (const part of rows(data.equipment, 'Equipment')) {
    string(part.id, 'Equipment ID');
    string(part.partId, 'Equipment variant');
    vector(part.position, 'Equipment position');
    number(part.bearingDeg, 'Equipment bearing');
    if (part.wall !== undefined) {
      ...
      for (const key of ['widthM', 'heightM']) {
        number(wall[key], key);
        if ((wall[key] as number) < 0.15 || (wall[key] as number) > 5) throw new Error('Wall fitting dimensions must be 0.15–5 m');
      }
      if (wall.mirrorId !== undefined && (typeof wall.mirrorId !== 'string' || wall.mirrorId === part.id))
        throw new Error('Invalid wall mirror partner');
      if (wall.turnDeg !== undefined && ![90, 180, 270].includes(wall.turnDeg as number))
        throw new Error('Wall fittings turn in quarter turns of 90, 180 or 270°');
    }
    if (part.gun !== undefined) {
      ...
      for (const key of ['barbetteHeightM', 'initialElevationDeg', 'traverseDeg', 'elevationMinDeg', 'elevationMaxDeg'])
        if (gun[key] !== undefined) number(gun[key], key);
      if (gun.traverseLimitsDeg !== undefined) {
        if (!Array.isArray(gun.traverseLimitsDeg) || gun.traverseLimitsDeg.length !== 2)
          throw new Error('Gun traverse limits require two angles');
    ...
    if (part.launcher !== undefined) {
      ...
      if (!Array.isArray(launcher.launchArcsDeg) || launcher.launchArcsDeg.length < 1 || launcher.launchArcsDeg.length > 8)
        throw new Error('Launcher requires 1–8 firing arcs');
      launcher.launchArcsDeg.forEach(angles);
    }
    if (part.path !== undefined) {
      const path = object(part.path, 'Equipment path');
      if (!Array.isArray(path.points) || path.points.length < 2 || path.points.length > 64)
        throw new Error('Equipment paths require 2–64 points');
      ...
      if (path.access !== undefined) {
        ...
        for (const [key, min, max] of [
          ...
          number(access[key], key);
          if ((access[key] as number) < min || (access[key] as number) > max) throw new Error(`Invalid ladder ${key}`);
        }
        if (!['both', 'left', 'right', 'none'].includes(access.handrails as string))
          throw new Error('Choose both, left, right or no handrails');
      }
      if (path.slackM !== undefined) number(path.slackM, 'Rope slack');
      if (path.heightM !== undefined) {
        number(path.heightM, 'Railing height');
        if ((path.heightM as number) < 0.3 || (path.heightM as number) > 3) throw new Error('Railing height must be between 0.3 and 3 m');
      }
  ...
  for (const wall of rows(data.boundaries, 'Boundaries')) {
    ...
    number(wall.offset, 'Boundary offset');
    number(wall.thicknessMm, 'Boundary thickness');
    if (!['x', 'y', 'z'].includes(wall.axis as string)) throw new Error('Unsupported boundary axis');
  }
  for (const load of rows(data.loads, 'Loads')) {
    string(load.id, 'Load ID');
    string(load.name, 'Load name');
    vector(load.center, 'Load center');
    vector(load.size, 'Load size');
    number(load.massKg, 'Load mass');
  }
...
export function decodeSavedConstruction(revision: ConstructionRevision, catalogRevision = revision.catalogRevision): ConstructionSource {
  ...
  if (source.id !== (revision.sourceId ?? revision.designId))
    throw new ConstructionStoreError(
      'corrupt',
      'The source and saved design identities disagree. Recover an earlier revision; the original is preserved.',
    );
  if (source.construction.catalogRevision !== revision.catalogRevision)
    throw new ConstructionStoreError(
      'catalog',
      'The source and saved catalog identities disagree. Recover an earlier revision; the original is preserved.',
    );
  return source;
}
...
export async function loadSavedConstructionWithCatalog(
  store: ConstructionStore,
  designId: string,
  resolveCatalog = loadConstructionCatalog,
) {
  const saved = await loadSavedConstruction(store, designId);
  let catalog;
  try {
    catalog = await resolveCatalog(saved.revision.catalogRevision);
  } catch (cause) {
    throw new ConstructionStoreError(
      'catalog',
      `The saved equipment revision could not be loaded. ${cause instanceof Error ? cause.message : String(cause)}. Download the original or recover a compatible revision; no equipment was substituted.`,
      { cause },
    );
  }
...
export function moveConstructionSelection(source: ConstructionSource, selected: ReadonlySet<string>, delta: Vec3): void {
  ...
  for (const item of source.construction.equipment)
    if (selected.has(item.id) && !moved.has(item.id)) {
      ...
      if (twin) {
        twin.position = [-item.position[0], item.position[1], item.position[2]];
        moved.add(twin.id);
      }
    }
  for (const load of source.construction.loads)
...
export function mirroredPrimitive(primitive: ConstructionPrimitive): ConstructionPrimitive {
  if (primitive.kind === 'balcony')
    return {
      ...structuredClone(primitive),
      position: [-primitive.position[0], primitive.position[1], primitive.position[2]],
      ...mirroredOrientation(primitive),
      balcony: mirroredBalcony(primitive),
    };
  if (primitive.mesh) {
    const out = structuredClone(primitive),
      m = out.mesh!;
    out.position[0] *= -1;
    Object.assign(out, mirroredOrientation(primitive));
    m.vertices.forEach((v) => (v[0] *= -1));
    m.reference.forEach((v) => (v[0] *= -1));
    m.faces.forEach((f) => {
      f.corners.reverse();
      if (f.name === 'port') f.name = 'starboard';
      else if (f.name === 'starboard') f.name = 'port';
    });
  ...
  if (primitive.kind === 'vertex') {
    const out: ConstructionPrimitive = {
      ...structuredClone(primitive),
      position: [-primitive.position[0], primitive.position[1], primitive.position[2]],
      ...mirroredOrientation(primitive),
      vertices: [1, 0, 3, 2, 5, 4, 7, 6].map((i) => {
        const v = cornerVertices(primitive)[i];
        return [-v[0], v[1], v[2]];
      }),
    };
    if (out.shaping) {
      const s = out.shaping;
      s.edges = s.edges.map((i) => mirroredIndices('edge', [i], [true, false, false]).find((j) => j !== i) ?? i);
    }
    return out;
  }
...
export function mirroredFace(face: ConstructionFace, kind: ConstructionPrimitive['kind']): ConstructionFace {
  ...
  if (yaw === -90)
    return (
      ({ port: 'bow', bow: 'port', starboard: 'stern', stern: 'starboard' } as Partial<Record<ConstructionFace, ConstructionFace>>)[face] ??
      face
    );
  if (yaw === 90)
    return (
      ({ port: 'stern', stern: 'port', starboard: 'bow', bow: 'starboard' } as Partial<Record<ConstructionFace, ConstructionFace>>)[face] ??
      face
    );
...
export function mirroredEquipment(part: ConstructionEquipment): ConstructionEquipment {
  return {
    ...
    bearingDeg: normalizedBearing(-part.bearingDeg),
            ...
            ...(part.path.access
              ? {
                  access: {
                    ...part.path.access,
                    handrails:
                      part.path.access.handrails === 'left'
                        ? 'right'
                        : part.path.access.handrails === 'right'
                          ? 'left'
                          : part.path.access.handrails,
                  },
                }
              : {}),
            points: part.path.points.map((point) => [-point[0], point[1], point[2]] as Vec3),
          },
        }
      : {}),
  };
...
export function copyConstructionSelection(
  ...
  data.primitives.push(
    ...originals.map((part) => ({
      ...(options.mirror ? mirroredPrimitive(part) : structuredClone(part)),
      id: ids.get(part.id)!,
  ...
  data.surfaces.push(
    ...data.surfaces
      .filter((surface) => kinds.has(surface.primitiveId))
      .map((surface) => ({
        ...surface,
        primitiveId: ids.get(surface.primitiveId)!,
        face: options.mirror ? mirroredFace(surface.face, kinds.get(surface.primitiveId)!) : surface.face,
        ...(surface.panelId ? { panelId: options.mirror ? mirroredPanelId(surface.panelId) : surface.panelId } : {}),
      })),
  );
  data.equipment.push(
    ...data.equipment
      ...
      .map((part) => ({
        ...(options.mirror ? mirroredEquipment(part) : structuredClone(part)),
        id: ids.get(part.id)!,
        position: position(part.position),
  ...
  data.loads.push(
    ...data.loads
      .filter((load) => selected.has(load.id))
      .map((load) => ({ ...structuredClone(load), id: ids.get(load.id)!, center: position(load.center) })),
  );
  return [...ids.values()];
}

export function assignConstructionSurfaces(
  source: ConstructionSource,
  ...
  for (const primitive of source.construction.primitives)
    for (const { face, panelId } of [
      ...
      if (!assignment) {
        const inherited = source.construction.surfaces.find(
          (surface) => surface.primitiveId === primitive.id && surface.face === face && surface.panelId === undefined,
        );
        assignment = {
          primitiveId: primitive.id,
          face,
          thicknessMm: source.construction.defaultThicknessMm,
          material: 'steel',
          paint: constructionShipPaint(source),
          ...inherited,
          ...(panelId ? { panelId } : {}),
        };
