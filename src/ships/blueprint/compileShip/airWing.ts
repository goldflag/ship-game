...
export function validateAirWing(b: Rec, h: Rec, modules: Rec[]): void {
  if (b.airWing !== undefined) {
    ...
    for (const key of ['launchPosition', 'recoveryPosition']) {
      ...
      if (Math.abs(p[0]) > (h.beam as number) / 2 || Math.abs(p[2]) > (h.length as number) / 2 || p[1] < 0 || p[1] > 40)
        fail(`airWing.${key}`, 'must be over the flight deck');
    }
    ...
    for (const squadron of squadrons) {
      id(squadron.id, 'squadron.id');
      text(squadron.name, 'squadron.name');
      if (!Object.hasOwn(GAMEPLAY_AIRCRAFT, String(squadron.modelId)) || GAMEPLAY_AIRCRAFT[String(squadron.modelId)] !== squadron.role)
        fail('squadron.modelId', 'unknown aircraft or incompatible role');
    ...
    if (wing.deckLayout !== undefined) {
      ...
      const point = (value: unknown, path: string) => {
        ...
        if (Math.abs(p[0]) > (h.beam as number) || Math.abs(p[2]) > (h.length as number) * 0.55 || p[1] < 0 || p[1] > 40)
          fail(path, 'invalid deck position');
        if (!onSurface(p[0], p[2]) || Math.abs(p[1] - Number(surface!.baseY) - Number(surface!.height)) > 0.5)
          fail(path, 'must rest on the authored flight deck');
        return p;
      };
      const start = point(layout.launchStart, 'deckLayout.launchStart'),
        end = point(layout.launchEnd, 'deckLayout.launchEnd');
      const touchdown = point(layout.recoveryTouchdown, 'deckLayout.recoveryTouchdown'),
        stop = point(layout.recoveryStop, 'deckLayout.recoveryStop');
      if (end[2] >= start[2] - 30 || stop[2] >= touchdown[2] - 10)
        fail('airWing.deckLayout', 'launch and recovery paths must run toward the bow');
      ...
      for (const spot of spots) {
        id(spot.id, 'deck spot.id');
        point(spot.position, 'deck spot.position');
        if (!['fighter', 'dive-bomber', 'torpedo-bomber'].includes(String(spot.preferredRole)))
          fail('deck spot.preferredRole', 'unknown aircraft role');
      }
      ...
      for (const elevator of elevators) {
        ...
        numeric(elevator.widthM, 'elevator.widthM', 3, 30);
        numeric(elevator.lengthM, 'elevator.lengthM', 3, 30);
        const shape = structure!.footprint as number[][];
        const xs = shape.map((p) => p[0]),
          zs = shape.map((p) => p[1]);
        if (
          Number(elevator.widthM) > Math.max(...xs) - Math.min(...xs) + 0.001 ||
          Number(elevator.lengthM) > Math.max(...zs) - Math.min(...zs) + 0.001
        )
          fail('elevator', 'operating dimensions exceed the fitted platform');
      }
    }
  }
}
