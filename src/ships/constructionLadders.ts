...
export function ladderWallProjection(part: ConstructionEquipmentPart, item: ConstructionEquipment, surfaces?: readonly ConstructionSurface[]) {
  const p=part.path;
  if(p?.kind!=='ladder')return;
  const r=item.bearingDeg*Math.PI/180, c=Math.cos(r),s=Math.sin(r),normal=wallNormal(item.bearingDeg);
  return surfaces ? (local:Vec3):Vec3|undefined=>{
...
export function fittedLadder(part: ConstructionEquipmentPart, item: ConstructionEquipment, surfaces?: readonly ConstructionSurface[]) {
  const p=part.path;
  if(p?.kind!=='ladder')return;
  return ladderRungs(pathOf(item).points,{widthM:p.widthM!,standOffM:p.standOffM!,postSpacingM:p.postSpacingM!},ladderWallProjection(part,item,surfaces));
}
