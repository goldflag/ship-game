...
self.onmessage = async (event: MessageEvent<{ type: 'compile'; id: number; source: ConstructionSource }>) => {
  const { id, source } = event.data;
  try {
    if (event.data.type !== 'compile') throw new Error('Unknown compiler request.');
