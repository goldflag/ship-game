...
export function App() {
  ...
  return <ShipContext value={selectedShip}><main className="game-shell" style={{ '--hud-scale': hudScale } as CSSProperties}
    ...
    {phase === 'garage' && ready && !error && <Garage key={selectedShip.id} switching={switching} switchError={switchError} onSelectShip={switchShip} game={game.current} ready={ready} fps={data.fps} onLaunch={openBattleSetup} onPve={() => { if (pveRequest) setPveRequest({ ...pveRequest, seed: crypto.getRandomValues(new Uint32Array(1))[0] }); setPveOpen(true); }} onMultiplayer={() => setMultiplayerOpen(true)} onSettings={() => game.current?.setPaused(true)}/>}
