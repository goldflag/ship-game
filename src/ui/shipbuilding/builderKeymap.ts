...
const onlyWallFittingsSelected = (tool: BuilderTool) =>
  tool.selectedEquipment.length > 0 &&
  tool.selectedEquipment.length === tool.state.selected.size &&
  tool.selectedEquipment.every((e) => e.wall);
...
function raiseOrLower(tool: BuilderTool, event: BuilderKey) {
  ...
  if (
    s.selected.size &&
    tool.selectedEquipment.length === s.selected.size &&
    tool.selectedEquipment.every((item) => tool.partOf(item)?.kind === 'gun')
  )
...
export const BUILDER_KEYMAP: readonly BuilderKeyBinding[] = [
  // ---- drawing a connected path: the route owns Escape, Enter and its own undo
  {
    id: 'path-cancel',
    keys: ['Esc'],
    does: 'Cancel the path being drawn',
    mods: 'any',
    match: ['Escape'],
    when: drawing,
    prevent: true,
    run: (tool) => tool.cancelPath(),
  },
  {
    id: 'path-finish',
    keys: ['Enter'],
    does: 'Finish the path being drawn',
    mods: 'any',
    match: ['Enter'],
    when: drawing,
    prevent: true,
    run: (tool) => tool.finishPath(),
  },
  {
    id: 'path-pop',
    keys: ['⌫'],
    does: 'Remove the last pending path point',
    mods: 'any',
    match: ['Backspace'],
    when: drawingPoints,
    prevent: true,
    run: (tool) => tool.popPathPoint(),
  },
  {
    id: 'path-pop-undo',
    keys: ['⌘Z'],
  ...
  },
  {
    id: 'path-hold',
    keys: ['R', 'Del'],
    does: 'Ignored while a path has pending points',
    mods: 'any',
    match: ['r', 'Delete'],
    when: drawingPoints,
  },
  ...
  // ---- history
  {
    ...
    match: ['z'],
    prevent: true,
    run: (tool, event) => (event.shiftKey ? tool.door.redo() : tool.door.undo()),
  },
  ...
  // ---- the Hull Rotate tool: the axis keys mean the same quarter turn in every hull tool; here they also aim the gizmo
  {
    id: 'rotate-tool-axis',
    keys: ['X', 'Y', 'Z'],
    does: 'Rotate tool: aim the gizmo and turn the block 90° (Shift reverses)',
    mods: 'plain',
    match: AXES,
    when: rotating,
    prevent: true,
  ...
  },
  {
    id: 'rotate-tool-turn',
    keys: ['R', '⇧R'],
    does: 'Rotate tool: turn the block 90° about the chosen axis',
    mods: 'plain',
    match: ['r'],
    when: rotating,
    prevent: true,
    once: true,
    run: (tool, event) => tool.rotateBlock(tool.state.rotationAxis, quarter(event)),
  },
  {
    id: 'rotate-tool-leave',
    keys: ['Esc'],
    does: 'Rotate tool: return to Select',
    mods: 'plain',
    match: ['Escape'],
    when: rotating,
    prevent: true,
    run: (tool) => tool.setTool('select'),
  },
  ...
  // ---- standing toggles, in every layer and tool
  {
    id: 'snap',
    keys: ['N'],
    does: 'Toggle snapping',
    mods: 'plain',
    match: ['n'],
    prevent: true,
    once: true,
    run: (tool) => tool.toggleSnapping(),
  },
  {
    id: 'grid',
    keys: ['S'],
    does: 'Cycle grid spacing (the move step in freeform)',
    mods: 'plain',
    match: ['s'],
    prevent: true,
    once: true,
    run: (tool) => tool.cycleGrid(),
  },
  {
    id: 'projection',
    keys: ['P'],
    does: 'Toggle perspective / orthographic camera',
    mods: 'plain',
    match: ['p'],
    prevent: true,
    once: true,
    run: (tool) => tool.toggleProjection(),
  },
  {
    id: 'freeform',
    keys: ['D'],
    does: 'Hull: enter or finish freeform editing for one selected shape',
    mods: 'plain',
    match: ['d'],
    when: (tool) => tool.state.layer === 'hull' && !tool.pathPart,
    prevent: true,
    once: true,
  ...
  // ---- freeform shaping: its own keys, then everything but the view strip and Mirror is held back
  {
    id: 'freeform-leave',
    keys: ['Esc'],
    does: 'Freeform: finish shaping',
    mods: 'no-command',
    match: ['Escape'],
    when: shaping,
    prevent: true,
    run: (tool) => tool.exitFreeform(),
  },
  {
    id: 'freeform-step',
    keys: ['G'],
    does: 'Freeform: cycle the move increment',
    mods: 'no-command',
    match: ['g'],
    when: shaping,
    prevent: true,
    run: (tool) => tool.cycleUnit(),
  },
  {
    id: 'freeform-projection',
    keys: ['O'],
    does: 'Freeform: toggle orthographic / perspective',
    mods: 'no-command',
    match: ['o'],
    when: shaping,
    prevent: true,
  ...
  },
  {
    id: 'freeform-mode',
    keys: ['1', '2', '3', '4'],
    does: 'Freeform: select vertices, edges, faces or rings',
    mods: 'plain',
    match: ['1', '2', '3', '4'],
    when: shaping,
    prevent: true,
    once: true,
    run: (tool, event) => tool.setFreeformMode(FREEFORM_MODES[Number(event.key) - 1]),
  },
  {
    id: 'freeform-hold',
    keys: [],
    does: 'Freeform: other keys wait until shaping ends',
    mods: 'no-command',
    match: 'any',
    when: (tool, event) => shaping(tool) && !FREEFORM_PASSES.includes(event.key.toLowerCase()) && event.key !== 'Home',
  },
  ...
  // ---- clipboard and select all. Without a selection, ⌘C and ⌘X stay with the browser so selected text still copies.
  {
    id: 'copy',
    keys: ['⌘C', '⇧⌘C'],
    does: 'Copy 1 m to starboard, mirror-copy across the centerline',
    mods: 'command',
    match: ['c'],
    when: hasSelection,
    prevent: true,
    run: (tool, event) => tool.copy(event.shiftKey),
  },
  {
    id: 'cut',
    keys: ['⌘X'],
    ...
    when: hasSelection,
    prevent: true,
    run: (tool) => tool.remove(),
  },
  {
    id: 'select-all',
    keys: ['⌘A'],
    does: 'Select every hull piece and fitting',
    mods: 'command',
    match: ['a'],
    prevent: true,
    run: (tool) => tool.selectAll(),
  },
  ...
  // ---- plain keys
  {
    id: 'escape',
    keys: ['Esc'],
    does: 'Close a panel, dismiss a proposal, end a measurement, then return to Select and clear',
    mods: 'no-command',
    match: ['Escape'],
    run: escape,
  },
  {
    id: 'remove',
    keys: ['Del', '⌫'],
    does: 'Remove the selection; a wall merges its rooms',
    mods: 'no-command',
  ...
  { id: 'fit', keys: ['Home'], does: 'Frame the ship', mods: 'no-command', match: ['Home'], prevent: true, run: (tool) => tool.fit() },
  {
    id: 'raise-lower',
    keys: ['PgUp', 'PgDn'],
    does: 'Raise or lower the selection; selected turrets rise on their barbettes',
    mods: 'no-command',
    match: ['PageUp', 'PageDown'],
    prevent: true,
    run: raiseOrLower,
  },
  {
    id: 'resize-wall-fitting',
    keys: ['←', '→', '↑', '↓'],
    does: 'Resize a door, window or porthole (Shift for fine steps)',
    mods: 'no-command',
    match: ARROWS,
    when: (tool) => !!wallPiece(tool) || onlyWallFittingsSelected(tool),
    prevent: true,
    run: resizeWallFittings,
  },
  {
    id: 'nudge',
    keys: ['←', '→', '↑', '↓'],
    does: 'Nudge the selection by the snap step',
    mods: 'no-command',
    match: ARROWS,
    when: hasSelection,
    prevent: true,
  ...
  },
  {
    id: 'drawer',
    keys: ['0'],
    does: 'Open or close every card of the layer',
    mods: 'no-command',
    match: ['0'],
    run: (tool, event, chrome) => {
  ...
  },
  {
    id: 'turn-block',
    keys: ['X', 'Y', 'Z'],
    does: 'Hull: turn the cursor block or selected blocks 90° in pitch, yaw or roll (Shift reverses)',
    mods: 'no-command',
    match: AXES,
    when: (tool) => tool.state.layer === 'hull',
    prevent: true,
    once: true,
    run: (tool, event) => tool.turn(AXES.indexOf(event.key.toLowerCase()), quarter(event)),
  },
  {
    id: 'view',
    keys: ['Q'],
    does: 'Cycle Orbit, Plan, Profile and Bow',
    mods: 'no-command',
    match: ['q'],
    run: (tool) => tool.cycleView(),
  },
  {
    id: 'warnings',
    keys: ['W'],
    does: 'Open or close the checks list',
    mods: 'no-command',
    match: ['w'],
    run: (_tool, _event, chrome) => chrome.toggleWarnings(),
  },
  {
    id: 'rotate',
    keys: ['R', '⇧R'],
    does: 'Rotate the cursor piece or selection: ±90° hull, 15° fittings (Shift-R 1°)',
    mods: 'no-command',
    match: ['r'],
    run: (tool, event) => tool.rotate(event.shiftKey),
  },
  { id: 'mirror', keys: ['M'], does: 'Toggle Mirror', mods: 'no-command', match: ['m'], run: (tool) => tool.toggleMirror() },
  {
    id: 'centers',
    keys: ['C'],
    does: 'Show or hide the centers of gravity and buoyancy',
    mods: 'no-command',
    match: ['c'],
    run: (tool) => tool.toggleCenters(),
  },
  {
    id: 'arcs',
    keys: ['A'],
    does: 'Machinery, Armament and Outfit: show or hide gun arcs',
    mods: 'no-command',
    match: ['a'],
    when: (tool) => tool.state.layer === 'fittings',
    run: (tool) => tool.toggleArcs(),
  },
  {
    id: 'rail',
    keys: [],
    does: "The layer's rail: each tool and action prints its own key (see BUILDER_RAIL)",
    mods: 'no-command',
    match: 'any',
    run: (tool, event) => {
...
function bindingMatches(binding: BuilderKeyBinding, tool: BuilderTool, event: BuilderKey): boolean {
  ...
  if (
    binding.mods === 'command'
      ? !command
      : binding.mods === 'plain'
        ? command || event.altKey
        : binding.mods === 'no-command'
          ? command
          : false
  )
    return false;
  if (binding.match !== 'any' && !binding.match.some((key) => (key.length === 1 ? key === event.key.toLowerCase() : key === event.key)))
    return false;
