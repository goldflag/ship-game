...
export function pathEquipment(id: string, partId: string, points: readonly Vec3[], slackM = 0): ConstructionEquipment {
  const position = [...points[0]] as Vec3;
  return { id, partId, position, bearingDeg: 0, path: { points: points.map(p => p.map((v, k) => v - position[k]) as Vec3), slackM } };
}
...
export function pathAnchor(part: ConstructionEquipmentPart, source: ConstructionSource, catalog: ConstructionCatalog, hit: { id?: string; point: Vec3; normal?: Vec3; axis: 0 | 1 | 2 }, step: number): Vec3 | undefined {
  const profile = part.path;
  if (!profile) return undefined;
  const fitting = source.construction.equipment.find(item => item.id === hit.id);
  if (fitting) {
    if (profile.kind === 'railing') return undefined;
  ...
  const normal = hit.normal ?? [0, 1, 0], point = hit.point.map((v, k) => k === hit.axis ? v : snapCoordinate(v, step)) as Vec3;
  const error = normal.reduce((sum, v, k) => sum + v * (point[k] - hit.point[k]), 0);
  point[hit.axis] -= error / (normal[hit.axis] || 1);
  const radius = profile.kind === 'railing' ? 0 : profile.diameterM * (profile.kind === 'chain' ? 2 : .5);
