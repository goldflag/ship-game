...
export default function CustomHullEditor({ integration }: { integration?: { hull: Hull; onApply(hull: Hull): void; onClose(): void } }) {
  ...
  const content = <main className={`hp-root hp-variant-${variant}${pending && error ? ' hp-invalid' : ''}${integration ? ' hp-integrated' : ''}`}>
    ...
    <div className="hp-hull-strip">{!integration && <><label>Editing<select aria-label="Active hull" value={h.id} onChange={e => changeHull(e.target.value)}>{displayed.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}</select></label><button disabled={hulls.length >= 3} onClick={() => { const copy = clone(h); copy.id = uid(); copy.name += ' copy'; copy.offset += h.beam * 1.5; copy.stations.forEach(s => { s.id = uid(); }); commit([...hulls, copy]); setActive(copy.id); setSelected([copy.stations[3].id]); setFit(v => v + 1); }}>Duplicate hull</button></>}<span className="hp-strip-note">Flat faces · symmetric · {hulls.length} hull block{hulls.length > 1 ? 's' : ''}</span></div>
