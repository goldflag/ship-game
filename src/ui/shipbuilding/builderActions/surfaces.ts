/** Armor, paint, openings and finishes on hull faces and fittings.
...
import {
  CONSTRUCTION_PAINTS,
  DEFAULT_CONSTRUCTION_WEAR,
  constructionShipPaint,
  isConstructionSurfaceFinish,
  isConstructionWear,
...
export function applyItem(
  ...
  thickness = this.state.customMm,
): ConstructionSubmission | undefined {
  ...
  switch (item.kind) {
    case 'thickness':
      thickness = item.mm; // falls through
    case 'armor':
      return assign(`Assign ${thickness} mm armor`, {
        thicknessMm: thickness,
        material: thickness > 0 ? 'armor-steel' : 'steel',
        open: false,
      });
    case 'opening':
      return assign('Open faces to sea', { open: true });
    case 'paint':
      return assign(`Paint ${item.name.toLowerCase()}`, { paint: item.id });
    default:
      return undefined;
  }
}
...
/** Weathering drawn in port and battle; choosing the default where the field is absent changes nothing. */
export function setWear(this: BuilderTool, wear?: ConstructionSource['construction']['wear']): ConstructionSubmission | undefined {
  if (wear !== undefined && !isConstructionWear(wear)) return undefined;
  if (wear === this.data.wear || (this.data.wear === undefined && wear === DEFAULT_CONSTRUCTION_WEAR)) return undefined;
...
export function setShipPaint(this: BuilderTool, paint?: string): ConstructionSubmission | undefined {
  ...
  if (paint)
    for (const item of this.data.equipment)
      if (this.partOf(item)?.path?.kind !== 'rope' && (item.paint === previous || item.paint === paint))
        commands.push({ op: 'equipment-patch', id: item.id, changes: { paint: null } });
  ...
  if (this.state.mirror)
    for (const item of fittings)
      if (targets.has(item.id)) {
        const twin = mirrorTwinEquipment(this.source, item);
        if (twin) targets.add(twin.id);
      }
  const commands = fittings
    .filter((item) => targets.has(item.id))
    .map((item): ConstructionCommand => {
      const value = { ...item };
      if (paint) value.paint = paint;
      else delete value.paint;
...
export function applyScheme(this: BuilderTool, id: 'two-tone' | 'disruptive'): ConstructionSubmission {
  ...
  const coated = new Set(
    this.data.primitives.filter((p) => p.kind === 'custom-hull' && hullPaintBands(p.customHull).length > 0).map((p) => p.id),
  );
  const redBottom = new Set(
    this.editableSurfaces.filter((s) => s.face === 'bottom' && !coated.has(s.primitiveId)).map(surfaceSelectionKey),
  );
  if (id === 'two-tone')
    return this.run('Apply two-tone paint', [
  ...
  return this.run(
    'Apply disruptive paint',
    this.data.primitives.flatMap((primitive) =>
      this.surfaceCommands(new Set(keys.filter((key) => key.startsWith(`${primitive.id}:`) && !key.endsWith(':bottom'))), {
        paint: Math.abs(Math.floor(primitive.position[2] / 12)) % 2 ? 'sea-blue' : 'light-gray',
      }),
    ),
  );
}
...
export function applyFaces(this: BuilderTool, keys: readonly string[]): ConstructionSubmission | undefined {
  ...
  if (!editable.length) {
    if (keys.length) this.update({ notice: 'This fixed equipment support follows its fitting. Choose a hull face to edit.' });
    return undefined;
  }
  const target = new Set(editable),
    tool = this.state.tool;
  if (tool === 'opening') {
    const first = this.editableSurfaces.find((entry) => surfaceSelectionKey(entry) === editable[0])!;
    return this.run(
      first.open ? 'Close skin' : 'Open faces to sea',
      this.surfaceCommands(this.withMirrorFaces(target), { open: !first.open }),
    );
  }
