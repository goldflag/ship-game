...
export function copy(this: BuilderTool, mirrorCopy = false): ConstructionSubmission | undefined {
  ...
  if (this.selectedEquipment.length && this.selectedEquipment.every((e) => e.wall) && !this.selectedPrimitives.length) {
    ...
    for (const original of this.selectedEquipment) {
      ...
      if (mirrorCopy && original.wall!.mirrorId) {
        this.notify('This fitting already has a linked mirror.');
        continue;
      }
      if (mirrorCopy && !offCenterline(original.position)) {
        this.notify('A centerline fitting cannot have a separate mirrored partner.');
        continue;
      }
      const a = structuredClone(original),
        b = mirroredEquipment(original);
      b.id = this.newId('equipment');
      if (mirrorCopy) {
        copied.push(b.id);
        a.wall!.mirrorId = b.id;
        b.wall!.mirrorId = a.id;
        commands.push({ op: 'equipment', value: a }, { op: 'equipment', value: b });
      } else {
        a.id = this.newId('equipment');
        copied.push(a.id);
        const r = (a.bearingDeg * Math.PI) / 180,
          spacing = this.partOf(a)!.size[0] + 0.25;
        a.position[0] += Math.cos(r) * spacing;
        a.position[2] += Math.sin(r) * spacing;
        if (original.wall!.mirrorId) {
          ...
          delete a.wall!.mirrorId;
          commands.push({ op: 'equipment', value: a });
        }
      }
    }
  ...
  if (
    !blockPlacementAllowed(
      this.source,
      next.construction.primitives.filter((p) => copied.includes(p.id)),
    )
  ) {
  ...
  this.update({
    ...
    surfaces: new Set(),
    notice: mirrorCopy ? 'Mirrored a copy across the centerline.' : 'Copied the selection 1 m to starboard.',
  });
  return outcome;
}

export function centerSelection(this: BuilderTool): ConstructionSubmission | undefined {
  const centers = constructionSnapFeatures(this.source, this.catalog).filter(
    (f) => this.state.selected.has(f.owner) && f.kind === 'center',
  );
...
export function movePieces(this: BuilderTool, ids: string[], requested: Vec3, mirror = true): ConstructionSubmission | undefined {
  ...
  if (wall) {
    const normal = wallNormal(wall.bearingDeg),
      d = delta.reduce((sum, v, k) => sum + v * normal[k], 0);
    delta = delta.map((v, k) => v - d * normal[k]) as Vec3;
  }
  ...
  for (const wall of this.data.boundaries)
    if (moving.has(wall.id)) commands.push({ op: 'boundary', value: { ...wall, offset: wall.offset + delta[AXIS[wall.axis]] } });
  if (this.data.equipment.some((e) => moving.has(e.id) && e.wall) && this.compiled) {
    const next = applyConstructionBatch(this.source, {
      version: 1,
    ...
    for (const item of next.construction.equipment) {
      if (!item.wall || JSON.stringify(item.position) === JSON.stringify(this.data.equipment.find((e) => e.id === item.id)?.position))
        continue;
      const part = this.partOf(item);
      if (part) commands.push({ op: 'equipment', value: seatWallFitting(item, part, this.compiled.surfaces) });
    }
  }
