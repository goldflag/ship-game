/** Native layout suggestions: request and apply.
...
export async function suggest(this: BuilderTool, selectedPart = false) {
  if (this.pathPart) {
    this.update({ notice: 'Click connected points on the ship, then press Enter to finish this path.' });
    return;
  }
  ...
  const ids = selectedPart
    ...
    : this.catalog.equipment
        .filter((part) => {
          ...
          fittedKinds.add(part.kind);
          return true;
        })
        .map((part) => part.id)
        .slice(0, 16);
  if (!ids.length) {
    this.update({ notice: selectedPart ? 'Choose a fitting slot to place by suggestion.' : 'Every internal family is already fitted.' });
    return;
  }
  this.door.setBusy('Finding a layout');
  ...
  try {
    ...
    if (proposed.source.id !== source.id || proposed.source.construction.catalogRevision !== data.catalogRevision)
      throw new Error('The layout belongs to a different design or equipment catalog. Request a new suggestion.');
    this.update({ suggestion: { proposal: proposed, revision } });
  } catch (cause) {
    if (!abort.signal.aborted) this.fail(cause);
  } finally {
    if (this.suggestionRequest === abort) this.suggestionRequest = undefined;
    this.door.setBusy('');
  }
}
...
export function applySuggestion(this: BuilderTool): ConstructionSubmission | undefined {
  ...
  const next = structuredClone(this.source),
    proposal = suggestion.proposal.source.construction;
  next.construction.equipment = structuredClone(proposal.equipment);
  next.construction.boundaries = structuredClone(proposal.boundaries);
  next.construction.loads = structuredClone(proposal.loads);
  for (const item of next.construction.equipment)
    if (
      this.state.fittingPaint &&
      !this.data.equipment.some((existing) => existing.id === item.id) &&
      this.partOf(item)?.placement !== 'internal'
    )
      item.paint = this.state.fittingPaint;
