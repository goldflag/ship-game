...
export function placeAt(
  ...
  const piece = hullPlacement ?? this.piece,
    active = this.active,
    { mirror } = this.state;
  ...
  if (piece.kind === 'hull' && active?.kind === 'shape') {
    ...
    if (!blockPlacementAllowed(this.source, pieces)) {
      this.update({ notice: OVERLAP_NOTICE });
      return undefined;
    }
    if (this.data.primitives.length + pieces.length > LIMITS.primitives) {
      this.door.setError(
        `A design supports up to ${LIMITS.primitives} hull pieces. Use larger pieces or remove a section before adding more.`,
      );
      return undefined;
    }
    return this.run(
      pieces.length > 1 ? 'Lay hull pieces' : 'Place hull piece',
      pieces.map((value) => ({ op: 'primitive', value })),
    );
  }
  if (piece.kind === 'equipment' && active?.kind === 'part') {
    ...
    for (const point of points) {
      ...
      parts.push({
        id: this.newId('equipment'),
        partId: active.id,
        position: [...point],
        bearingDeg: bearingDeg ?? piece.bearingDeg,
        ...(piece.wall ? { wall: { ...piece.wall } } : {}),
        ...(this.state.layer === 'fittings' && this.state.fittingPaint ? { paint: this.state.fittingPaint } : {}),
      });
      if (piece.wall && points.length > 1 && this.compiled)
        parts[parts.length - 1] = seatWallFitting(parts.at(-1)!, active.part, this.compiled.surfaces);
      if (mirror && offCenterline(point)) {
        const original = parts.at(-1)!,
          twin = { ...mirroredEquipment(original), id: this.newId('equipment') };
        if (original.wall) {
          original.wall.mirrorId = twin.id;
          twin.wall = { ...twin.wall!, mirrorId: original.id };
        }
        parts.push(twin);
      }
    }
    ...
    return this.run(
      parts.length > 1 ? 'Place fittings' : `Place ${active.name}`,
...
export function addBoundary(this: BuilderTool, axis: ConstructionBoundary['axis'], offset: number): ConstructionSubmission | undefined {
  if (this.data.boundaries.length >= LIMITS.boundaries) {
    this.door.setError(`A design supports up to ${LIMITS.boundaries} decks and bulkheads. Merge rooms before adding more.`);
    return undefined;
  }
  if (this.data.boundaries.some((wall) => wall.axis === axis && Math.abs(wall.offset - offset) < 1e-6)) {
    this.update({ notice: `A ${BOUNDARY_NAMES[axis].toLowerCase()} already sits here.` });
    return undefined;
  }
  return this.run(`Add ${BOUNDARY_NAMES[axis].toLowerCase()}`, [
    { op: 'boundary', value: { id: this.newId('boundary'), axis, offset, thicknessMm: 10 } },
  ]);
}
