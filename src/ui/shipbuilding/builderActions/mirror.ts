/** Mirror editing helpers: the twins an edit reaches and the faces across the centerline.
...
export function mirrored(this: BuilderTool, commands: ConstructionCommand[]) {
  return this.state.mirror ? withMirroredEdits(this.source, commands) : commands;
}
...
export function withMirrorFaces(this: BuilderTool, keys: ReadonlySet<string>): Set<string> {
  ...
  for (const key of keys) {
    ...
    const primitive = this.data.primitives.find((part) => part.id === primitiveId),
      twin = primitive && mirrorTwin(this.source, primitive);
    const reflected =
      primitive && twin ? surfaceKey(twin.id, mirroredFace(face as ConstructionFace, primitive.kind), mirroredPanelId(panelId)) : undefined;
