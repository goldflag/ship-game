/** Freeform hull sessions: enter, selection mode, commit and split.
...
export function enterFreeform(this: BuilderTool): boolean {
  ...
  this.update({
    ...
    surfaces: new Set(),
    freeformSettings: {
      ...this.state.freeformSettings,
      selection: freeformSelection(converted, this.state.freeformSettings.selection.mode),
    },
    freeform: { designId: this.source.id, baseline: structuredClone(part) },
  });
  return true;
}
...
export function setFreeformMode(this: BuilderTool, mode: HullSelectionMode) {
  const primitive = this.freeformPrimitive,
    selection = freeformSelection(primitive, mode);
  if (primitive && selection.mode === mode && mode !== this.state.freeformSettings.selection.mode)
    this.changeFreeformSettings({ selection });
}
...
export function commitFreeform(this: BuilderTool, replacements: ConstructionPrimitive[]): ConstructionSubmission | undefined {
  ...
  return shaped.length
    ? this.run(
        'Shape freeform hull',
        shaped.map((value) => ({ op: 'primitive', value })),
      )
    : undefined;
}
...
export function splitFreeform(this: BuilderTool) {
  const primitive = this.freeformPrimitive,
    settings = this.state.freeformSettings;
  if (!primitive) return;
  try {
    const twin = this.state.mirror ? this.freeformTwin : undefined;
    const next = structuredClone(this.source),
      ids = splitVertexPrimitive(next, primitive.id, settings.splitAxis, settings.count);
    if (twin) {
      // The twin splits into the reflected pieces, which run the other way along its mirrored local X.
      const twins = splitVertexPrimitive(next, twin.id, settings.splitAxis, settings.count),
        pieces = next.construction.primitives;
      ids.forEach((id, i) => {
        const at = pieces.findIndex((p) => p.id === twins[settings.splitAxis === 0 ? ids.length - 1 - i : i]);
        pieces[at] = twinPrimitive(
