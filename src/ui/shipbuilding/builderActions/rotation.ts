/** Turning: the rotate tool, quarter turns of blocks, wall fittings and fitted parts.
...
export function rotate(this: BuilderTool, fine = false): ConstructionSubmission | undefined {
  ...
  if (piece?.kind === 'equipment' && piece.wall) {
    this.update({ wallTurn: normalizedBearing(this.state.wallTurn + (fine ? -90 : 90)) });
    return undefined;
  }
  ...
  if (piece && piece.kind !== 'boundary') {
    this.update({ bearing: normalizedBearing(this.state.bearing + (fine ? 1 : 15)) });
    return undefined;
  }
  if (this.state.selected.size)
    return this.command(
      'Rotate selection',
      this.mirrored([
        { op: 'rotate', ids: [...this.state.selected], degrees: this.selectedPrimitives.length ? (fine ? -90 : 90) : fine ? 1 : 15 },
      ]),
    );
  return undefined;
}
...
export function turnWallFittings(
  this: BuilderTool,
  items: readonly ConstructionEquipment[],
  degrees: number,
): ConstructionSubmission | undefined {
  if (this.locked) return undefined;
  if (!this.compiled) {
    this.notify('Wait for the hull to finish compiling before turning wall fittings.');
    return undefined;
  }
...
export function turn(this: BuilderTool, axis: number, degrees: number): ConstructionSubmission | undefined {
  ...
  const piece = this.piece,
    { bearing, tilt } = this.state;
  if (piece) {
    if (piece.kind !== 'hull') return axis === 1 ? this.rotate(degrees < 0) : undefined;
    if (axis !== 1 && piece.shape === 'balcony') {
      this.notify('Balconies turn about the vertical axis only.');
      return undefined;
    }
    const turned = rotateBlock(
      {
        id: '',
        kind: piece.shape,
        size: piece.size,
