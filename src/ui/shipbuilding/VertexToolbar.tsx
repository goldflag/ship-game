...
export function VertexToolbar({primitive,settings:s,onChange,cycleUnit,onCoordinate,onView,perspective,onProjection,onReset,onSplit,onExit}: {
  ...
  return <section className="sb-vertex-tools" aria-label="Vertex hull editor">
    ...
    <div className="sb-vertex-controls">
      ...
      <div className="sb-vertex-group"><div className="sb-vertex-views">{(['side','top','bow'] as const).map(v=><button key={v} onClick={()=>onView(v)}>{v==='side'?'Side':v==='top'?'Top':'Bow'}</button>)}</div><button onClick={onProjection} title="Toggle orthographic and perspective cameras (P)">{perspective?'Perspective':'Orthographic'} <kbd>P</kbd></button></div>
