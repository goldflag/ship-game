...
beforeEach(() => {
  Object.defineProperty(globalThis, 'document', { configurable: true, value: { createElement: () => new HandleElement() } });
...
for (const projection of ['orthographic', 'perspective'] as const) {
  for (const motion of ['orbit', 'pan'] as const) {
    test(`${projection} corner handles follow ${motion} and damping in the frame being rendered`, () => {
      ...
      const handles = new VertexHandles(host as unknown as HTMLElement, () => camera, () => {});
      ...
      handles.update(source, { id: 'cube', corner: 0, symmetry: false, axes: [false, false, false],
        unit: .2, axis: true, snap: false, onSelect() {}, onCommit() {} });
      try {
        for (let frame = 0; frame < 60; frame++) {
          ...
          const handle = host.children[0].children[0];
          ...
          camera.updateMatrixWorld();
          const corner = new THREE.Vector3(-2, -2, -2).project(camera);
