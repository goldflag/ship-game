...
export class BuilderTool extends BuilderSelectors {
  ...
  constructor(door: BuilderRevisionDoor, context: BuilderToolContext, initial: Partial<BuilderToolState> = {}) {
    ...
    this.lastSourceId = door.source.id;
    this.lastRevision = door.source.revision;
    this.unsubscribe = door.subscribe(this.observe);
  }
  dispose() {
    this.unsubscribe();
    this.suggestionRequest?.abort();
    this.suggestionRequest = undefined;
  }
  ...
  newId(prefix: string) {
    return this.context.newId ? this.context.newId(prefix) : newConstructionId(prefix);
  }
  ...
  update(patch: Partial<BuilderToolState>) {
    ...
    if (next.groupShelves[fittingGroup(category)] !== category)
      next.groupShelves = { ...next.groupShelves, [fittingGroup(category)]: category };
    this.state = next;
    // A pending route belongs to one part on one layer and tool of one design.
    if (
      next.pathPoints.length &&
      (next.layer !== previous.layer || next.tool !== previous.tool || this.pathPartOf(next)?.id !== this.pathPartOf(previous)?.id)
    )
      this.state = { ...next, pathPoints: [] };
    this.reconcile();
  ...
  private reconcile() {
    ...
    if (p && !selectionCorners(this.state.freeformSettings.selection, p).length)
      this.state = { ...this.state, freeformSettings: { ...this.state.freeformSettings, selection: freeformSelection(p) } };
  }
  ...
  private observe = () => {
    ...
    if (source.revision !== this.lastRevision) {
      this.suggestionRequest?.abort();
      this.suggestionRequest = undefined;
    }
    this.lastSourceId = source.id;
    this.lastRevision = source.revision;
    if (patch) this.update(patch);
    else {
      const before = this.state;
      this.reconcile();
      if (this.state !== before) for (const listener of this.listeners) listener();
    }
  };

  /** Open a shelf or narrow it; the bar's first card is taken up when the held one is no longer on it. */
  setFittingFilter = (patch: Partial<FittingFilter>) => {
    this.update({ fittingFilter: { ...this.state.fittingFilter, ...patch }, pathPoints: [], sizeOverride: undefined });
  };
  setHullCategory = (hullCategory: HullCategory) => {
    if (hullCategory !== this.state.hullCategory) this.update({ hullCategory, sizeOverride: undefined });
  };
  setRotationAxis = (rotationAxis: number) => {
    if ([0, 1, 2].includes(rotationAxis)) this.update({ rotationAxis });
  };
  ...
  setSnapOverride = (held: boolean) => {
    if (held !== this.state.snapOverride) this.update({ snapOverride: held });
  };
  setSnapStep = (step: number) => {
    if (SNAP_STEPS.includes(step)) this.update({ snapSteps: { ...this.state.snapSteps, [this.snapKind]: step } });
  };
  /** The view bar's Snap control: 0.25 → 0.5 → 1 → 2 → 5 m for the current kind. */
  cycleSnap = () => {
    const kind = this.snapKind,
      steps = this.state.snapSteps;
    this.update({ snapSteps: { ...steps, [kind]: SNAP_STEPS[(SNAP_STEPS.indexOf(steps[kind]) + 1) % SNAP_STEPS.length] } });
  };
  ...
  scene(retained: ConstructionResult | undefined): BuilderScene {
    const s = this.state,
      locked = this.locked,
      freeformMode = this.freeformMode,
      pathPart = this.pathPart,
      freeformPrimitive = this.freeformPrimitive;
    return {
      source: this.source,
      result: retained,
      current: this.compiled,
      catalog: this.catalog,
      selected: s.selected,
      selectedSurfaces: s.surfaces,
      twins: locked ? NO_TWINS : this.selectionTwins,
      mirrorEdits: s.mirror && !locked,
      view: s.view,
      perspective: s.perspective,
      display: this.display,
      fitRequest: s.fitRequest,
      armorScale: this.armorScale,
      snapping: this.effectiveSnapping,
      gridStep: this.gridStep,
      gesture: locked ? 'none' : this.gesture,
      pickTargets: this.pickTargets,
      moveTargets: locked || freeformMode ? 'none' : this.moveTargets,
      placementPiece: locked || freeformMode ? undefined : this.piece,
      placementMirror: this.mirrorPiece,
      highlightFaces: this.faceLayer,
      rooms: s.layer === 'internals',
      showCenters: s.showCenters,
      arcs: this.arcs,
      proposed: this.proposed,
      measure: s.measure,
      measuring: s.tool === 'measure',
      pathDraft:
        !locked && pathPart
          ? {
              part: pathPart,
              points: s.pathPoints,
              bearingDeg: s.pathBearing,
              heightM: s.railingHeight,
              railCount: pathPart.path?.railCount ?? 3,
              slackM: pathPart.path?.kind === 'rope' ? Math.min(s.ropeSlack, pathSlackLimit(s.pathPoints)) : 0,
              mirror: s.mirror,
            }
          : undefined,
      rotation:
        this.rotationPrimitive && !locked
          ? {
              id: this.rotationPrimitive.id,
              axis: s.rotationAxis,
              snap: s.rotationSnap,
              onAxis: this.setRotationAxis,
              onCommit: this.rotateBlock,
            }
          : undefined,
      freeform:
        freeformPrimitive && !locked
          ? {
              ...s.freeformSettings,
              id: freeformPrimitive.id,
              onSelect: (selection) => this.changeFreeformSettings({ selection }),
              onCommit: this.commitFreeform,
            }
          : undefined,
    };
  }
  ...
  run = (label: string, commands: ConstructionCommand[]): ConstructionSubmission => {
    ...
    if (
      commands.some((c) => c.op === 'equipment' && c.value.wall) ||
      (this.data.equipment.some((e) => e.wall) && commands.some((c) => c.op === 'equipment' || c.op === 'move'))
    ) {
      try {
        ...
        for (const item of next.construction.equipment) {
          ...
          if (!this.compiled || !part || !wallFittingSupported(item, part, this.compiled.surfaces)) {
            this.door.setError(
              this.compiled
                ? 'The full fitting needs a closed hull side or wall on each fitted side. Move away from edges, or turn Mirror off for a single fitting.'
                : 'Wait for the hull to finish compiling before placing wall fittings.',
            );
        ...
        this.fail(error);
        return { accepted: false, reason: 'invalid', message: String(error) };
      }
    }
  ...
  command(label: string, commands: ConstructionCommand[]): ConstructionSubmission {
    return this.run(label, commands);
  }
  refused(): ConstructionSubmission | undefined {
    const refusal = this.door.refusal;
    return refusal && { accepted: false, ...refusal };
  }
  ...
  undo = () => {
    if (!this.locked) this.door.undo();
  };
  redo = () => {
    if (!this.locked) this.door.redo();
  };
  ...
  switchLayer = (next: BuilderLayer) => {
    this.update({
      layer: next,
      tool: DEFAULT_TOOL[next],
      surfaces: new Set(),
      measure: undefined,
      bearing: 0,
      tilt: undefined,
      ...(next === 'armor' || next === 'paint'
        ? { selected: new Set<string>() }
        : next === 'internals'
          ? { selected: new Set([...this.state.selected].filter((id) => this.internalIds.has(id))) }
          : {}),
    });
  };
  ...
  openTab = (tab: BuilderTab) => {
    ...
    if (!group) {
      this.switchLayer(tab as Exclude<BuilderTab, FittingGroup>);
      return;
    }
  ...
  setWallSize = (axis: 0 | 1, value: number) => {
    ...
    size[0] = dimensions.widthM;
    size[1] = dimensions.heightM;
  ...
  setThickness = (value: number) =>
    this.update({
      ...
      ...(this.state.layer === 'armor' && this.state.tool !== 'area' ? { tool: 'apply' as const } : {}),
    });
  ...
  selectAll = () =>
    this.update({
      selected: new Set(
        this.state.layer === 'internals' ? this.internalIds : [...this.data.primitives, ...this.data.equipment].map((part) => part.id),
      ),
    });
  ...
  nudge = (delta: Vec3): ConstructionSubmission | undefined =>
    this.state.selected.size ? this.movePieces([...this.state.selected], delta) : undefined;
  ...
  pointer = (event: BuilderPointerEvent): ConstructionSubmission | undefined => {
    switch (event.kind) {
      case 'pick':
        return this.pick(event.hit);
      case 'lay':
        return this.placeAt(event.points, event.bearingDeg, event.hullPlacement);
      case 'faces':
        return this.applyFaces(event.surfaces);
      case 'box':
        this.boxSelect(event.ids, event.additive);
        return undefined;
      case 'erase':
        return this.erase(event.id);
      case 'move':
        return this.movePieces(event.ids, event.delta);
      case 'rotate':
        return this.rotateFittings(event.ids, event.degrees);
      case 'ladder-draw':
        return this.drawLadder(event.points, event.bearingDeg);
      case 'path-point':
        this.addPathPoint(event.point, event.bearingDeg);
        return undefined;
      case 'path-finish':
  ...
  // ---- freeform hulls
  changeFreeformSettings = (patch: Partial<FreeformSettings>) =>
    this.update({ freeformSettings: { ...this.state.freeformSettings, ...patch } });
  cycleUnit = () =>
    this.changeFreeformSettings({
      unit: VERTEX_UNITS[(VERTEX_UNITS.findIndex((unit) => unit === this.state.freeformSettings.unit) + 1) % VERTEX_UNITS.length],
    });
  ...
  resetFreeform = () => {
    ...
    return freeform ? this.edit('Reset hull edit', [{ op: 'primitive', value: editableMesh(freeform.baseline) }]) : undefined;
  };
  ...
  key(event: BuilderKey, chrome: BuilderChrome) {
    dispatchBuilderKey(this, event, chrome);
  }
}
