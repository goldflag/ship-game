...
The custom hull section editor (`CustomHullEditor.tsx`) is a full-screen viewport of its own: `CustomHullViewport.tsx` draws the hull, rings, gizmo and guides in Orbit, Section, Plan and Profile views, `HullSectionRuler.tsx` is the bow-to-stern station strip, `customHullEditing.ts` holds the pure edit operations and `customHullSnap.ts` the snap targets. It also offers **Red lower hull**: a coating boundary set in meters above the hull's base (saved in meters from the hull center) or painted to the natively measured waterline. See [custom hull sections](../../../docs/shipbuilding.md#custom-hull-sections) and [red paint height](../../../docs/shipbuilding.md#custom-hull-red-paint-height) for controls, defaults and behavior.
