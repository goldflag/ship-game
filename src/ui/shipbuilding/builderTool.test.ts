...
test('wear changes one revision each; the implicit default wear is no edit', async () => {
  const { tool, data, labels } = await setup();
  const original = structuredClone(data());
  ...
  expect(labels()).toEqual(['Set ship wear', 'Set ship wear']);
  tool.undo(); expect(data().wear).toBe('battle-worn');
  tool.setWear(); expect(data()).toEqual(original);
});
