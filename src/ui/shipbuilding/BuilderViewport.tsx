...
class Viewport {
  ...
  private ortho = new THREE.OrthographicCamera(-50, 50, 35, -35, 0.1, 6000);
  private perspectiveCamera = new THREE.PerspectiveCamera(40, 1, 0.1, 5000);
    ...
    this.freeformHandles = new FreeformHandles(
      ...
      (replacements) => this.previewVertices(replacements),
      {
        ...
        resolve: (raw, free, primitive, options) => {
          ...
          const delta = this.resolveSnapping(
  ...
  fit() {
    ...
    const view = this.props.scene.view,
  ...
  update(props: ViewportProps) {
    ...
    if (snapFeaturesKey !== this.snapFeaturesKey) {
      ...
      this.snapFeatures = constructionSnapFeatures(props.scene.source, props.scene.catalog);
      this.clearSnap();
    }
    ...
    this.controls.mouseButtons = {
      LEFT: props.scene.perspective || props.scene.view === 'orbit' ? THREE.MOUSE.ROTATE : THREE.MOUSE.PAN,
    ...
    if (
      props.scene.view !== this.currentView ||
      ...
      for (const surface of nativeSurfaces ?? []) {
        ...
        const color = new THREE.Color(
          invalid.has(surface.primitiveId)
            ? SALMON
            : props.scene.display === 'armor'
              ? armorThicknessColor(surface.thicknessMm, props.scene.armorScale)
        ...
        for (const face of painted) {
          fan(
            ...
            vertices,
            colors,
            props.scene.display === 'armor' || invalid.has(surface.primitiveId)
              ? color
        ...
        const selectionKey = surfaceSelectionKey(surface),
          group = this.surfacesByKey.get(selectionKey);
        if (group) group.push(surface);
        else this.surfacesByKey.set(selectionKey, [surface]);
      }
      if (nativeSurfaces) {
        ...
        const mesh = new THREE.Mesh(
          ...
          props.scene.display === 'armor'
            ...
            : new THREE.MeshStandardMaterial({
                ...
                metalness: 0,
                side: THREE.DoubleSide,
                transparent: props.scene.display === 'internals',
                opacity: props.scene.display === 'internals' ? 0.15 : 1,
                depthWrite: props.scene.display !== 'internals',
              }),
        );
            ...
            this.hull.add(
              new THREE.LineSegments(
                ...
                new THREE.LineBasicMaterial({
                  color: '#142a31',
                  transparent: true,
                  opacity: props.scene.display === 'internals' ? 0.35 : 0.9,
                  depthWrite: false,
                }),
              ),
            );
        ...
        for (const primitive of props.scene.source.construction.primitives) {
          const mesh = new THREE.Mesh(
            ...
            new THREE.MeshStandardMaterial({
              color: invalid.has(primitive.id) ? SALMON : constructionPaintColor(constructionShipPaint(props.scene.source)),
              roughness: 0.78,
              side: THREE.DoubleSide,
              transparent: props.scene.display !== 'paint',
              opacity: props.scene.display !== 'paint' ? 0.12 : 1,
              depthWrite: props.scene.display === 'paint',
            }),
          );
    ...
    if (!nativeSurfaces)
      for (const object of this.hull.children) {
    ...
    if (props.scene.display === 'internals' && props.scene.source.construction.version === 2) {
      for (const module of props.scene.result?.definition?.modules ?? []) {
        ...
        const box = new THREE.Mesh(
          ...
          new THREE.MeshBasicMaterial({
            color: props.scene.selected.has(owner.id) ? BRASS : SALMON,
            transparent: true,
            opacity: 0.55,
            depthWrite: false,
          }),
    ...
    for (const wall of props.scene.source.construction.boundaries) {
      ...
      const plane = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({
          ...
          transparent: true,
          opacity: invalid.has(wall.id) ? 0.5 : props.scene.selected.has(wall.id) ? 0.3 : 0.09,
          depthWrite: false,
          side: THREE.DoubleSide,
        }),
      );
      plane.userData.sourceId = wall.id;
      plane.visible = invalid.has(wall.id) || props.scene.display === 'internals' || props.scene.selected.has(wall.id);
      this.details.add(plane);
      if (plane.visible) this.pickMeshes.push(plane);
      if (wall.axis === 'y') {
        plane.userData.deckTopY = wall.offset + wall.thicknessMm / 2000;
        this.deckMeshes.push(plane);
      }
    }
    ...
    for (const group of this.surfacesByKey.values()) {
      const surface = group[0],
        chosen = props.scene.selectedSurfaces.has(surfaceSelectionKey(surface));
    ...
    for (const id of props.scene.twins.values()) {
      ...
      const edges = new THREE.LineSegments(
        primitive ? primitiveOutlineGeometry(primitive) : itemOutlineGeometry(this.pickMeshes, id),
        new THREE.LineBasicMaterial({ color: MINT, depthTest: false }),
      );
      if (primitive) {
        edges.position.set(...primitive.position);
  ...
  private previewVertices(replacements?: ConstructionPrimitive[]) {
    ...
    for (const original of this.props.scene.source.construction.primitives) {
      ...
      const mesh = new THREE.Mesh(
        geometry,
        new THREE.MeshStandardMaterial({
          color: constructionPaintColor(constructionShipPaint(this.props.scene.source)),
          roughness: 0.78,
          side: THREE.DoubleSide,
        }),
      );
  ...
  private updateGhost() {
    ...
    if (key !== this.ghostKey) {
      ...
      if (piece) {
        const build = (item: BuilderPlacement, group: THREE.Group, opacity: number, mirrored = false) => {
          let equipment = item.kind === 'equipment' && item.partId ? this.equipment.clone(item.partId, true) : undefined;
          if (equipment) {
            ...
            if (part && item.kind === 'equipment') {
              if (item.wall && pick) {
                ...
                equipment = createConstructionWallModel(
                  ...
                  this.props.scene.current?.surfaces ?? [],
                  this.props.scene.source.construction.primitives,
                );
        ...
        build(piece, this.ghost, piece.kind === 'boundary' ? 0.12 : 0.25);
        if (mirror) build(mirror, this.ghostMirror, 0.12, true);
    ...
    if (!visible) {
      ...
      if (piece && !this.pointerStart?.move && !this.moveHandles.dragging) this.clearSnap();
      return;
    }
    const position = [...pick!.placement] as Vec3;
  ...
  private tintPlacement(blocked: boolean) {
    ...
    for (const group of [this.ghost, this.ghostMirror])
  ...
  private resolveSnapping(context: string, raw: Vec3, grid: Vec3, directions: Vec3[], moving: SnapFeature[], excluded: Set<string>): Vec3 {
    ...
    const result = resolveSnap({
      ...
      directions,
      moving,
      targets: this.snapFeatures.filter((f) => !excluded.has(f.owner)),
      settings: this.props.scene.snapping ?? DEFAULT_SNAPPING,
      project,
  ...
  private snapMove(ids: string[], raw: Vec3, free: boolean[]): Vec3 {
    const selected = new Set(ids),
      all = this.snapFeatures.filter((f) => selected.has(f.owner));
    ...
    return this.resolveSnapping(
      `move:${ids.join(',')}`,
      raw,
      raw.map((v) => snapCoordinate(v, this.props.scene.gridStep)) as Vec3,
  ...
  private drawSnapGuides() {
    ...
    if (centered) {
      ...
      centerline =
        this.workingAxis === 2 || this.props.scene.view === 'bow'
          ? [
              [0, this.snapBounds.min.y - padding, point[2]],
              [0, this.snapBounds.max.y + padding, point[2]],
            ]
          : [
              [0, point[1], this.snapBounds.min.z - padding],
              [0, point[1], this.snapBounds.max.z + padding],
            ];
    }
  ...
  private pick(
    event: { clientX: number; clientY: number; shiftKey?: boolean; ctrlKey?: boolean; metaKey?: boolean },
    targets: BuilderScene['pickTargets'],
  ): BuilderPick | undefined {
    ...
    const allowed =
      targets !== 'hull' && this.props.scene.pickTargets === 'internals'
        ? internalSelectionIds(this.props.scene.source, this.props.scene.catalog)
        : undefined;
    // Internal packages only fit inside the hull, so their ray passes the skin to the floor within.
    const cursor = this.props.scene.placementPiece,
      internal = targets === 'hull' && cursor?.kind === 'equipment' && cursor.inset !== undefined && !cursor.wall;
    const floor = internal ? this.interiorHit(ray) : undefined,
      direction = floor?.direction ?? ray.ray.direction;
    const hit = internal
      ? floor?.hit
      : ray
          .intersectObjects(targets === 'hull' ? this.hullMeshes : this.pickMeshes, false)
    ...
    const id =
      supportId && this.props.scene.source.construction.equipment.some((item) => item.id === supportId)
        ? supportId
    ...
    if (piece?.kind === 'hull' && piece.shape === 'balcony')
      piece = this.pointerStart?.start?.hullPlacement ?? balconyPlacement(piece, normal);
    ...
    if (piece?.kind === 'hull' && piece.shape === 'balcony' && surface && Math.abs(normal[1]) < 0.9)
      placement = seatBalconyOnHull(piece, placement, surface, this.props.scene.result?.surfaces ?? []);
    ...
    return ray.intersectObjects([...this.hullMeshes, ...this.deckMeshes], false).map((hit) => {
      ...
      const normal = deck
        ...
        : hit.object.userData.hull
          ? this.surfaceTriangles[hit.faceIndex ?? -1]?.normal
  ...
  private clearFloor(piece: Extract<BuilderPlacement, { kind: 'equipment' }>, placement: Vec3): Vec3 {
    ...
    for (const corner of baseFootprint(piece, placement)) {
      ...
      const beneath = interiorFloor(
        this.crossings(drop).filter((crossing) => !crossing.deck || crossing.point[1] <= corner[1] + 1e-3),
        [0, -1, 0],
        true,
      ).floor;
  ...
  private armorReading(event: { clientX: number; clientY: number }, pick?: BuilderPick): { title: string; detail: string } | undefined {
    ...
    const turret =
      this.navigating() || this.props.scene.measuring
        ? undefined
        : this.equipment.armorHit(ray, this.props.scene.source, this.props.scene.catalog);
  ...
  private highlight(event: { clientX: number; clientY: number }) {
    ...
    const groups = this.props.scene.highlightFaces
      ? [this.surfacesByKey.get(key) ?? []]
      : [...this.surfacesByKey.values()].filter((group) => group[0].primitiveId === key);
  ...
  private updateFillPreview() {
    ...
    const end = hit.placement,
      extents = pieceExtents(piece);
    const low = start.placement.map((value, index) => Math.min(value, end[index]) - extents[index] / 2),
      high = start.placement.map((value, index) => Math.max(value, end[index]) + extents[index] / 2);
    this.fillPreview.visible = true;
    this.fillPreview.position.set((low[0] + high[0]) / 2, (low[1] + high[1]) / 2, (low[2] + high[2]) / 2);
    this.fillPreview.scale.set(Math.max(0.01, high[0] - low[0]), Math.max(0.01, high[1] - low[1]), Math.max(0.01, high[2] - low[2]));
  }
  ...
  private updateStrokePreview() {
    ...
    const points =
      row && drag?.start === row && piece?.kind === 'equipment' && piece.rowSpacing
        ? this.rowPoints(row, hit, piece.rowSpacing)
    ...
    for (const point of points) {
      if (piece?.kind === 'equipment' && piece.wall) {
        const part = this.props.scene.catalog.equipment.find((p) => p.id === piece.partId),
          surfaces = this.props.scene.current?.surfaces ?? [];
        if (!part) continue;
        const item = seatWallFitting(
          {
            ...
            partId: part.id,
            position: point,
            bearingDeg: drag?.start?.bearingDeg ?? piece.bearingDeg,
            wall: piece.wall,
          },
          part,
          surfaces,
        );
        for (const fitting of this.props.scene.placementMirror
          ? [item, { ...item, position: [-item.position[0], item.position[1], item.position[2]] as Vec3, bearingDeg: -item.bearingDeg }]
          : [item]) {
          const template = this.equipment.clone(part.id, true);
          if (!template) continue;
          const model = createConstructionWallModel(template, part, fitting, surfaces, this.props.scene.source.construction.primitives);
          model.position.fromArray(fitting.position);
          model.rotation.y = (-fitting.bearingDeg * Math.PI) / 180;
          this.strokePreview.add(model);
        }
        continue;
      }
      const instance = this.ghost.clone(true);
      instance.traverse((n) => {
      ...
      if (this.ghostMirror.visible && this.props.scene.placementMirror && Math.abs(point[0]) > 1e-6) {
        const twin = this.ghostMirror.clone(true);
        twin.traverse((n) => {
          n.userData.sharedPreviewResources = true;
        });
  ...
  private updateFacesPreview() {
    ...
    mesh.renderOrder = 20;
    edges.renderOrder = 21;
    this.facesPreview.add(mesh, edges);
  }
  ...
  private placeTags() {
    ...
    const narrow = width <= 1100,
      builder = this.props.tags.length ? this.host.closest('.shipbuilder') : null;
    ...
    const ledger = builder?.querySelector<HTMLElement>('.sb-ledger'),
      hostRect = builder ? this.host.getBoundingClientRect() : null;
    ...
    const feedback = this.overlay.querySelector<HTMLElement>('[data-gesture-feedback]');
    if (feedback) {
    ...
    if (measure) {
      const from = this.props.scene.measure?.from,
        to = this.props.scene.measure?.to ?? (from && this.hover ? this.pick(this.hover, 'hull')?.point : undefined);
      if (from && to) {
        const a = new THREE.Vector3(...from),
          b = new THREE.Vector3(...to);
        this.measureLine.geometry.setFromPoints([a, b]);
        const middle = this.project(a.clone().add(b).multiplyScalar(0.5));
        if (middle) {
          measure.innerHTML = `<b>${a.distanceTo(b).toFixed(2)} m</b>${this.props.scene.measure?.to ? '' : ' · click to end'}`;
  ...
  private movePick(event: PointerEvent): MoveDrag | undefined {
    ...
    const allowed =
      this.props.scene.pickTargets === 'internals' ? internalSelectionIds(this.props.scene.source, this.props.scene.catalog) : undefined;
    const ids = [...this.props.scene.selected].filter(
    ...
    return {
      ...
      delta: [0, 0, 0],
      built: false,
      constrain: mirroredMoveConstraint(this.props.scene.source, new Set(ids), new Set(twins)),
    };
  }
  ...
  private updateMoveHandles() {
    ...
    const ids = [...props.scene.selected].filter(
      (id) => (!allowed || allowed.has(id)) && (props.scene.moveTargets !== 'equipment' || equipment.some((item) => item.id === id)),
    );
    const selected = new Set(ids),
      blocks = primitives.filter((p) => selected.has(p.id));
    if (props.scene.freeform || props.scene.moveTargets === 'none' || !ids.length) {
    ...
    this.moveHandles.update({
      ...
      snap: (raw, free) => this.snapMove(ids, raw, free),
      ...
      preview: (delta, blocked) => {
        ...
        this.positionMovePreview(delta);
  ...
  private buildMovePreview(ids: string[], twins: string[] = []) {
    ...
    for (const id of [...ids, ...partners]) {
      ...
      if (item) {
        ...
        datum.position.set(...item.position);
        datum.rotation.y = (-item.bearingDeg * Math.PI) / 180;
        const ghost = this.equipment.clone(item.partId, true, item.path),
          part = this.props.scene.catalog.equipment.find((entry) => entry.id === item.partId);
  ...
  private positionMovePreview(delta: Vec3) {
    ...
    for (const child of this.movePreview.children) {
      ...
      if (wallItem) {
        const part = this.props.scene.catalog.equipment.find((p) => p.id === wallItem.partId),
          surfaces = this.props.scene.current?.surfaces ?? [];
        if (part) {
          ...
          if (child.userData.projectedKey !== key) {
            ...
            const template = this.equipment.clone(part.id, true);
            if (template)
              child.add(createConstructionWallModel(template, part, item, surfaces, this.props.scene.source.construction.primitives));
  ...
  private updateMove(event: { clientX: number; clientY: number }) {
    ...
    if (!move) return;
    if (!move.built) {
      this.buildMovePreview(move.ids, move.twins);
      move.built = true;
    ...
    this.positionMovePreview(move.delta);
  ...
  private rotationPick(event: PointerEvent): RotationDrag | undefined {
    ...
    if (scene.placementPiece?.kind === 'equipment' && this.ghost.visible && this.ghostPosition)
      return { ids: [], degrees: 0, travelDegrees: 0, lastX: event.clientX, position: [...this.ghostPosition] };
    return undefined;
  }
  ...
  private previewRotation(rotation: RotationDrag, degrees: number) {
    for (const item of this.props.scene.source.construction.equipment) {
      ...
      for (const datum of this.details.children)
  ...
  private pathPick(event: { clientX: number; clientY: number }): Vec3 | undefined {
    ...
    if (
      ...
      (!hit.surface || !this.props.scene.result?.surfaces.some((s) => surfaceSelectionKey(s) === hit.surface && !s.open))
    )
      return;
    const settings = this.props.scene.snapping ?? DEFAULT_SNAPPING;
    ...
    const grid = pathAnchor(
      ...
      this.props.scene.catalog,
      hit,
      settings.enabled && settings.grid ? this.props.scene.gridStep : null,
    );
    if (!raw || !grid || this.props.scene.source.construction.equipment.some((p) => p.id === hit.id)) return grid;
    const point = this.resolveSnapping(
      'path',
      raw,
      grid,
  ...
  private updatePathPreview() {
    ...
    if (points.length > 1 && (draft.part.path?.kind === 'ladder' || isAccessKind(draft.part.path?.kind))) {
      ...
      const add = (item: ConstructionEquipment) => {
        const model = createConstructionPathModel(draft.part, item.path, true, { item, surfaces: this.props.scene.result?.surfaces ?? [] });
        model.position.fromArray(item.position);
        model.rotation.y = (-item.bearingDeg * Math.PI) / 180;
        this.pathPreview.add(model);
      };
      add(item);
      if (draft.mirror)
        add({
