...
class Viewport {
  ...
  update(props: ViewportProps) {
    ...
    if (props.display === 'internals' && props.source.construction.version === 2) {
      for (const module of props.result?.definition?.modules ?? []) {
        if (module.kind !== 'magazine') continue;
        const owner = props.source.construction.equipment.find(e => module.id === `${e.id}-magazine`);
        if (!owner) continue;
        const box = new THREE.Mesh(new THREE.BoxGeometry(...module.size), new THREE.MeshBasicMaterial({ color: props.selected.has(owner.id) ? BRASS : SALMON, transparent: true, opacity: .55, depthWrite: false }));
