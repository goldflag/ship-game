    ...
    {combat.playerSunk && <div className="fleet-spectator" aria-label="Teammate spectating">
      {teammates.length ? <>
        ...
        <div className="fleet-spectator-controls">
          <Button variant="icon" disabled={teammates.length < 2} onClick={event => { desk?.issue({ kind: 'cycle-spectator', direction: -1 }); event.currentTarget.blur(); }} aria-label="Spectate previous teammate" title="Previous teammate · Left arrow" aria-keyshortcuts="ArrowLeft"><Icon name="chevron" size={16} style={{ transform: 'rotate(90deg)' }}/></Button>
          <Select id="spectated-ship" value={spectatedShipId ?? ''} onValueChange={value => desk?.issue({ kind: 'spectate', id: value })}>
          ...
          <Button variant="icon" disabled={teammates.length < 2} onClick={event => { desk?.issue({ kind: 'cycle-spectator', direction: 1 }); event.currentTarget.blur(); }} aria-label="Spectate next teammate" title="Next teammate · Right arrow" aria-keyshortcuts="ArrowRight"><Icon name="chevron" size={16} style={{ transform: 'rotate(-90deg)' }}/></Button>
        </div>
        {desk?.can.transferHelm && <Button onClick={event => { desk.issue({ kind: 'helm-wheel', reason: 'sunk' }); event.currentTarget.blur(); }} aria-keyshortcuts={bindings?.helmWheel[0] ?? undefined}>Take a helm{bindings && <kbd>{bindingLabel(bindings, 'helmWheel')}</kbd>}</Button>}
