...
test('a typed word and number pick settings and set their values', () => {
  ...
  expect(matchCommands('').length).toBe(20);
});
...
test('online battles offer only the visual diagnostics', () => {
  expect(labels('', true)).toEqual([['Copy scene diagnostics', undefined], ['Toggle bow waves', undefined], ['Toggle realistic sea state', undefined],
    ['Toggle physical reflections', undefined], ['Toggle physical water colour', undefined], ['Toggle realistic wakes', undefined], ['Switch ocean renderer', undefined]]);
