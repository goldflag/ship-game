...
test('reported aircraft flying together become one typed group and keep their markers', () => {
  const tracks = [
    ...
    track({
      ...
      estimatedPosition: [300, 800, 200],
      velocity: [0, 0, -100],
      visibleCondition: { observedTick: 90, fire: false, heavySmoke: true, listing: false, sinking: false },
    }),
  ...
  expect(clusters.map((c) => [c.label, c.trackIds.sort(), c.smoking, c.stale])).toEqual([
    ['2 fighters', ['a1', 'a2'], 1, false],
    ['1 torpedo bomber', ['a3'], 0, false],
    ['1 fighter', ['a4'], 0, true],
  ]);
...
test('the battle comparison keeps our column exact and builds theirs from reports only', () => {
  const comparison = battleComparison({
    own: [
      { id: 'bb', massKg: 43_978_000, integrity: 49_228, maxIntegrity: 50_750, lost: false },
      { id: 'dd', massKg: 2_924_000, integrity: 0, maxIntegrity: 2_400, lost: true },
    ],
    ...
    tracks: [
      ...
      track({
        id: 'e3',
        identifiedPresetId: 'fletcher',
        visibleCondition: { observedTick: 50, fire: true, heavySmoke: true, listing: true, sinking: true },
      }),
...
test('ship markers fade out as the hull grows on screen', () => {
  ...
  expect(markerOpacity(36)).toBeCloseTo(0.5);
...
test('an unclassified aircraft report takes its type and model from a recognised exterior', async () => {
  ...
  expect(
    reportedAircraftType(track({ id: 'a9', kind: 'aircraft', classification: 'Fighter' }), [{ id: 'a9', modelId: 'b5n2-kate' }]),
  ).toEqual({ type: 'fighter', model: 'B5N2 Kate' });
