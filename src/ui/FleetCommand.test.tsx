...
function fixture() {
  const definition = shipPreset('bismarck'),
    simulation = new CombatSimulation(definition);
  ...
  Object.assign(simulation, {
    ...
    observationTracks: [
      {
        id: 'hidden-air-report',
        kind: 'aircraft',
        status: 'stale',
        affiliation: 'hostile',
        classification: 'Torpedo bomber',
        estimatedPosition: [200, 400, 900],
        measuredPosition: [200, 400, 900],
        velocity: [0, 0, 0],
        uncertaintyM: 5000,
        lastObservedTick: 0,
        sources: [],
      },
      {
        ...
        estimatedPosition: [4000, 0, -9000],
        measuredPosition: [4000, 0, -9000],
        velocity: [0, 0, 0],
        uncertaintyM: 40,
        lastObservedTick: 0,
        sources: [{ observerId: id, kind: 'surface', tick: 0, strength: 1 }],
      },
    ],
    shipScores: { [id]: { damageDealt: 12400, frags: 1 } },
  });
  const desk = fleetDesk({
    simulation,
    selectedShipIds: [],
    selectedFlightIds: [],
    controlGroups: new Map([[1, { name: 'Group 1', shipIds: [id] }]]),
  ...
  const data: Telemetry = {
    ...
    fps: 60,
    trail: [],
    combat: simulation.telemetry('main', [0, 0, -5000]),
    fleetCommandMode: true,
    airOperationsOpen: true,
    selectedShipIds: [],
  };
...
test('the chart shows every formation, the enemy fleet and an order wheel with hotkeys for the selected ship', () => {
  ...
  for (const item of ['Move · G', 'Hold · H', 'Escort · E', 'Focus fire · F', '20 kn · + / −'])
...
test('standing-order report distinguishes temporary evasion from its retained route', async () => {
  ...
  const order = {
    movement: { type: 'route', waypoints: [[0, 1000]], speedMps: 10, looped: false },
    navigation: { status: 'evading-torpedo', waypoint: 0 },
  } as unknown as import('../multiplayer/generated/FleetOrderState').FleetOrderState;
...
test('enemy HP appears only for current sightings with sampled health', () => {
  ...
  Object.assign(simulation, {
    observedShips: [{ id: 'contact-a-1', health: 0.42, observedTick: 0 }],
    observedAircraft: [{ id: 'hidden-air-report', health: 0.73, observedTick: 0 }],
  });
...
const fleetFixture = (
  ...
  notices: { tick: number; shipId: string; kind: 'guide-assumed'; text: string }[] = [],
) => {
  const definition = shipPreset('bismarck');
  const simulation = new CombatSimulation(
    definition,
    resolveBattleFleet(
      { playerShipId: 'bismarck', friendlyBots: ['fletcher', 'baltimore'], enemies: ['yamato'], spawnDistance: 7500 },
      shipPreset,
    ),
  );
  const escorts: { id: string; leaderId: string; offset: [number, number]; radiusM: number; formation?: Formation; slot?: number }[] = [];
  const standing = (movement: FleetOrderState['movement']): FleetOrderState => ({
    movement,
    weapons: { guns: true, aa: true, torpedoes: false },
  ...
  Object.assign(simulation, {
    phase: 'running',
    fleetNotices: notices,
    fleetOrders: {
      player: standing({ type: 'hold' }),
    ...
    escortShip: (id: string, leaderId: string, offset: [number, number], radiusM: number, kind?: Formation, slot?: number) => {
      escorts.push({ id, leaderId, offset, radiusM, formation: kind, slot });
    },
  });
  ...
  const data: Telemetry = {
    ...
    fps: 60,
    trail: [],
    combat: simulation.telemetry('main', [0, 0, -7500]),
    fleetCommandMode: true,
    airOperationsOpen: true,
    selectedShipIds: [],
  };
  const render = (selectedShipIds: string[]) =>
    renderToStaticMarkup(<FleetCommand data={{ ...data, selectedShipIds }} desk={desk} bindings={defaultKeybindings()} />);
  return {
    render,
    desk,
    controlGroups,
    escorts,
    owned,
    classes: owned.map((c) => shipClassOf(simulation.actors.find((a) => a.motion.id === c.id)!.definition)),
  };
};
...
test('choosing a formation records it on the control group and re-stations every escort by role', () => {
  ...
  const group = {
    index: 1,
    name: 'Bismarck formation',
    leaderId: owned[0].id,
    shipIds: owned.map((c) => c.id),
    formation: 'column' as Formation,
  };
  ...
  expect(escorts.map((e) => [e.id, e.offset, e.slot])).toEqual([
    [owned[2].id, [0, 450], 0],
    [owned[1].id, [0, 900], 1],
  ]);
  expect(controlGroups.get(1)!.formation).toBe('column');
  ...
  escorts.length = 0;
  expect(applyGroupFormation(desk, { ...group, formation: 'column' }, 'double-column', members)).toBe(
    '2 escort orders queued · Double column',
  );
  ...
  expect(applyGroupFormation(desk, { ...group, shipIds: [owned[0].id] }, 'line-abreast', [members[0]])).toBe(
    'Bismarck formation · Line abreast · no escorts to station',
  );
});

test('a guide-assumed notice reaches the feedback line so the fleet knows who has the group', () => {
  const { render, owned } = fleetFixture('column', [
    { tick: 120, shipId: 'friendly-1', kind: 'guide-assumed', text: 'Fletcher 2 has the guide' },
  ]);
...
test('a reported strike shows its slung weapons, the ship on its course and the time to release', () => {
  ...
  const carrying = {
    ...
    affiliation: 'hostile',
    classification: 'Torpedo bomber',
    identifiedPresetId: null,
    identificationConfidence: 1,
    estimatedPosition: [ship.x + 4000, 30, ship.z],
    measuredPosition: [ship.x + 4000, 30, ship.z],
    velocity: [-60, 0, 0],
    uncertaintyM: 40,
    firstObservedTick: 0,
    lastObservedTick: simulation.tick,
    sources: [],
  };
  const released = {
    ...carrying,
    id: 'tb-2',
    estimatedPosition: [ship.x + 4100, 30, ship.z + 200],
    measuredPosition: [ship.x + 4100, 30, ship.z + 200],
  };
  const exterior = (track: typeof carrying, payload: boolean) => ({
    id: track.id,
    modelId: 'tbd-1-devastator',
    position: track.estimatedPosition,
    heading: -Math.PI / 2,
    velocity: track.velocity,
    observedTick: simulation.tick,
    observers: [id],
    controls: {},
    wingFold: 0,
    payload,
  ...
  Object.assign(simulation, {
    observationTracks: [carrying, released],
    observedAircraft: [exterior(carrying, true), exterior(released, false)],
  });
  const html = render();
...
test('a custom battle reads the same chart from its helm: a way back, no captain to follow, and the whole enemy fleet', () => {
  const definition = shipPreset('bismarck'),
    simulation = new CombatSimulation(definition, { friendlyBots: [shipPreset('fletcher')], enemies: [shipPreset('yamato')] });
  const id = simulation.ship.id;
  const standing = (movement: FleetOrderState['movement']): FleetOrderState => ({
    movement,
    weapons: { guns: true, aa: true, torpedoes: false },
