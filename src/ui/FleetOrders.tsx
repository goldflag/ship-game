...
export function FleetOrders({ desk, combat }: { desk: FleetDesk | null; combat: CombatTelemetry }) {
  ...
  if (!desk?.can.transferHelm) return null;
  const session = desk.frame;
  ...
  return <details className="fleet-orders"><summary>Fleet orders</summary>
    ...
    <div className="fleet-orders-controls">
      <button disabled={!enabled} onClick={() => send(() => desk.issue({ kind: 'hold', shipId: id }), 'Hold position ordered')}>Hold</button>
      <button disabled={!enabled} onClick={() => send(() => desk.issue({ kind: 'autonomous', shipId: id }), 'Autonomous movement ordered')}>Autonomous</button>
      <button disabled={!enabled || !combat.targetId} onClick={() => { if (combat.targetId) send(() => desk.issue({ kind: 'focus', shipId: id, targetId: combat.targetId! }), 'Focus target ordered'); }}>Attack selected enemy</button>
    </div>
    <div className="fleet-orders-controls">
      <button disabled={!enabled} onClick={() => { desk.issue({ kind: 'chart-waypoint', shipId: id }); setNotice('Click the navigation chart to set a waypoint'); }}>Chart waypoint</button>
      ...
      <button disabled={!enabled || !x || !z || ![Number(x), Number(z)].every(n => Number.isFinite(n) && Math.abs(n) <= 40000)} onClick={() => send(() => desk.issue({ kind: 'move', shipId: id, point: [Number(x), 0, Number(z)] }), 'Waypoint ordered')}>Move</button>
    </div>
