...
import { dragFormation, formationCenter, rotateFormation, zoomDeployment } from './deploymentGestures';
...
test('wheel zoom holds the cursor anchor fixed, including its scale limits', () => {
  ...
  expect(zoomDeployment({ ...view, zoom: 8 }, anchor, -240)).toEqual({ ...view, zoom: 8 });
  expect(zoomDeployment({ ...view, zoom: 1 }, anchor, 240)).toEqual({ ...view, zoom: 1 });
});
