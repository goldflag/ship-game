...
export interface FleetCourse {
  ...
  waypoints: Vec3[];
  blocked: boolean;
  movement?: FleetOrderState['movement'];
...
export interface FleetMarker {
  ...
  glyph: { hull: string; mark: string };
  /** A column guide's label sits to port of its escorts astern; other guides ride above the marker. */
  side: 'port' | 'starboard' | 'above';
  label: { x: number; anchor: 'end' | 'middle' | 'start'; y: number; barX: number; barY: number; orderY: number };
}
export interface FleetBracket {
  members: Contact[];
  points: Vec3[];
  all: boolean;
}
export interface StragglerTag {
  leader: Contact;
  straggler: { shipId: string; availableSpeedMps: number; gapM: number };
  ship?: Contact;
}
...
export function standingOrder(order?: FleetOrderState, nameFor: (id: string) => string = (id) => id): string {
  ...
  const maneuver =
    status === 'evading-aircraft'
      ? 'Avoiding observed aircraft'
      : status === 'evading-torpedo'
        ? 'Avoiding spotted torpedoes'
        : status === 'straggling'
          ? 'Damaged stragglers · Decision needed'
          : status === 'slowing-for-stragglers'
            ? 'Slowing for stragglers'
            : undefined;
  ...
  const label =
    task.type === 'route'
      ? `${task.looped ? 'Patrol' : 'Route'} · ${(task.speedMps * KNOTS_PER_MPS).toFixed(0)} kn · Waypoint ${(order.navigation?.waypoint ?? 0) + 1}/${task.waypoints.length}`
      : task.type === 'escort'
        ? `Escort ${nameFor(task.leaderId)} · ${status?.replaceAll('-', ' ') ?? 'Assigned'}`
        : task.type === 'hold-area'
          ? `Hold area · ${(task.radiusM / 1000).toFixed(1)} km`
          : task.type === 'move'
            ? 'Move to waypoint'
            : task.type === 'hold'
              ? 'Stop'
...
export const circlePoints = (x: number, z: number, radius: number): Vec3[] =>
  Array.from({ length: 65 }, (_, i) => [x + Math.sin((i / 64) * Math.PI * 2) * radius, 0, z + Math.cos((i / 64) * Math.PI * 2) * radius]);
  ...
  if (task?.type === 'escort') {
    ...
    if (leader)
      return [
        origin,
        [
          leader.x + Math.cos(leader.heading) * task.offset[0] - Math.sin(leader.heading) * task.offset[1],
          0,
          leader.z + Math.sin(leader.heading) * task.offset[0] + Math.cos(leader.heading) * task.offset[1],
        ],
      ];
  }
  return [];
...
function bracketPoints(members: { x: number; z: number }[], padM = 420): Vec3[] {
  const xs = members.map((m) => m.x),
    zs = members.map((m) => m.z);
  const minX = Math.min(...xs) - padM,
    maxX = Math.max(...xs) + padM,
    minZ = Math.min(...zs) - padM,
    maxZ = Math.max(...zs) + padM;
  return [
    [minX, 0, minZ],
    [maxX, 0, minZ],
    [maxX, 0, maxZ],
...
export function fleetView(frame: FleetFrame, inputs: FleetViewInputs): FleetView {
  ...
  const enemies: FleetEnemy[] = [
    ...combat.contacts.filter((c) => c.team === 'enemy' && !c.physicalLost),
    ...observations.map((c) => ({ id: c.id, name: reportName(c), x: reportPosition(c, tick)[0], z: reportPosition(c, tick)[2] })),
  ];
  ...
  const wings: FleetWing[] = actors
    .filter((a) => a.team === 'friendly' && a.airWing)
    .map((a) => ({ owner: a, wing: airWingTelemetry(a, actors as FleetActor[])! }));
  const flights: FleetFlight[] = wings.flatMap(({ owner, wing }) =>
    wing.groups.map((f) => ({ ...f, ownerId: owner.motion.id, carrierName: nameFor(owner.motion.id) })),
  );
  ...
  const enemyHealth = new Map(
    [...(frame.observedShips ?? []), ...observedModels]
  ...
  const payloadOf = (c: ContactTrack) =>
    reportState(c, tick) === 'current' ? observedModels.find((o) => o.id === c.id)?.payload : undefined;
  ...
  const strikes = airStrikes(
    clusters,
    observations,
    observedModels,
    tick,
    afloat.map((s) => ({ id: s.id, name: nameFor(s.id), x: s.x, z: s.z })),
  );
  ...
  const interceptorFor = (position: Vec3) =>
    flights
      .filter((f) => f.role === 'fighter' && f.armed > 0 && (f.active || f.deck?.canLaunch))
      .sort(
        (a, b) =>
          Number(b.active) - Number(a.active) ||
          Math.hypot(a.position[0] - position[0], a.position[2] - position[2]) -
            Math.hypot(b.position[0] - position[0], b.position[2] - position[2]),
      )[0];
  const coverage = frame.reconCoverage;
  const coverageTickAt = (point?: [number, number]) =>
    point && coverage
      ? coverage.cells.find((c) => Math.abs(c.x - point[0]) <= coverage.cellSizeM / 2 && Math.abs(c.z - point[1]) <= coverage.cellSizeM / 2)
          ?.lastObservedTick
      : undefined;
  const subject = !mapOpen
    ? ships.find((s) => s.id === inputs.spectatedShipId || s.id === inputs.controlledShipId)
    : selected.length === 1
      ? selected[0]
      : undefined;
  ...
  const selectedGroup = formations.find(
    (f) => f.shipIds.length === recipients.length && f.shipIds.every((id) => recipients.some((s) => s.id === id)),
  );
  const origin = subject ?? lead ?? { x: frame.ship.x, z: frame.ship.z };
  const stragglerTags: StragglerTag[] = recipients.flatMap((s) =>
    (orders[s.id]?.navigation?.formation?.stragglers ?? []).map((straggler) => ({
      leader: s,
      straggler,
      ship: ships.find((c) => c.id === straggler.shipId),
    })),
  );
  const rosterShips: Omit<OwnFleetShip, 'shipClass' | 'order' | 'massKg'>[] = ships.map((s) => {
    const wing = wings.find((w) => w.owner.motion.id === s.id)?.wing,
      status = orders[s.id]?.navigation?.status;
    return {
      id: s.id,
      name: s.name,
      hull: s.integrity,
      kn: speedKnOf(s.id),
      damageDealt: scores[s.id]?.damageDealt ?? 0,
      frags: scores[s.id]?.frags ?? 0,
      lost: s.physicalLost,
      routeBlocked: !s.physicalLost && status === 'blocked',
      warn:
        !s.physicalLost &&
        (s.integrity < 0.5 || status === 'straggling' || status === 'blocked' || status === 'immobile' || status === 'leader-lost'),
      aircraft: wing
        ? {
            remaining: Object.values(wing.counts).reduce((n, v) => n + v, 0) - wing.counts.lost - wing.counts.withdrawn,
            total: Object.values(wing.counts).reduce((n, v) => n + v, 0),
          }
        : undefined,
    };
  });
  const ownAircraft = rosterShips.reduce(
    (sum, s) => ({ remaining: sum.remaining + (s.aircraft?.remaining ?? 0), total: sum.total + (s.aircraft?.total ?? 0) }),
    { remaining: 0, total: 0 },
  );
  ...
  const ownShips: OwnFleetShip[] = rosterShips.map((s) => ({
    ...s,
    warn: !!s.warn,
    shipClass: shipClassOf(definitionOf(ships.find((c) => c.id === s.id)!)),
    order: standingOrderOf(s.id),
    massKg: actorOf(s.id)?.definition.hull.massKg ?? 0,
  }));
  const ownWing: OwnFleetAircraft = {
    remaining: ownAircraft.remaining,
    total: ownAircraft.total,
    airborne: wings.reduce((n, { wing }) => n + wing.groups.reduce((m, f) => m + f.airborne, 0), 0),
    onDeck: wings.reduce((n, { wing }) => n + wing.onDeck, 0),
    inHangar: wings.reduce((n, { wing }) => n + wing.inHangar, 0),
  };
  const comparison = battleComparison({
    own: ships.map((s) => {
      ...
      return {
        id: s.id,
        massKg: a?.definition.hull.massKg ?? 0,
        integrity: a?.damage.integrity ?? 0,
        maxIntegrity: a?.damage.maxIntegrity ?? 0,
        lost: s.physicalLost,
      };
    }),
    scores,
    tracks: observations,
    massOf: (id) => resolveShip(id).hull.massKg,
    enemyLostShips: combat.contacts.filter((c) => c.team === 'enemy' && c.physicalLost).length,
    ownAircraft,
    enemyAircraftLost: frame.aircraftLosses?.enemy,
  });
  const planesOf = (flight: { aircraftIds: string[] }) =>
    wings.flatMap(({ owner }) => owner.airWing!.planes.filter((p) => flight.aircraftIds.includes(p.id)));
  const flightCentroid = selectedFlights.length
    ? selectedFlights.reduce<Vec3>(
        (sum, f) => [
          sum[0] + f.position[0] / selectedFlights.length,
          sum[1] + f.position[1] / selectedFlights.length,
          sum[2] + f.position[2] / selectedFlights.length,
        ],
        [0, 0, 0],
      )
    : undefined;
  const weaponsFree = (kind: keyof WeaponsPolicy) => recipients.every((s) => orders[s.id]?.weapons[kind]);
  const leadRoute = (pending: Readonly<Record<string, PendingRoute>>): Vec3[] =>
    lead ? (pending[lead.id]?.points ?? (orders[lead.id]?.movement.type === 'route' ? routePoints(lead, orders[lead.id], ships) : [])) : [];
  const moveOrigin = (shiftHeld: boolean, route: Vec3[]): Vec3 | undefined =>
    shiftHeld && route.length > 1 ? route.at(-1)! : recipients.length > 1 ? centroid(recipients) : lead ? [lead.x, 0, lead.z] : undefined;
  const courseOf = (s: Contact, pending?: PendingRoute): FleetCourse => {
    ...
    if (!blocked && body.length > 1 && (pending || movement?.type === 'route') && points.length > 1)
      points = [centroid(body), ...points.slice(1)];
  ...
  const markerOf = (s: Contact): FleetMarker => {
    const own = s.team === 'friendly',
      status = orders[s.id]?.navigation?.status;
    const warn = own && (s.integrity < 0.5 || status === 'straggling' || status === 'blocked' || status === 'immobile');
    ...
    const label =
      side === 'port'
        ? { x: -14, anchor: 'end' as const, y: -3, barX: -54, barY: 2, orderY: 17 }
        : side === 'above'
          ? { x: 0, anchor: 'middle' as const, y: -32, barX: -20, barY: -27, orderY: -16 }
    ...
    return {
      ...
      status,
      warn,
      kn: own ? speedKnOf(s.id) : 0,
      hp: Math.max(0, Math.min(1, s.integrity)),
      definition,
      glyph: SHIP_GLYPHS[shipClassOf(definition) as ShipClass],
      side,
      label,
    };
  };
...
export function stationEscorts(issue: IssueOrder, leaderId: string, formation: Formation, members: readonly StationShip[]): number {
  const guide = members.find((m) => m.id === leaderId),
    followers = members.filter((m) => m.id !== leaderId);
  if (!guide || !followers.length) return 0;
  const stations = formationStations(formation, guide, followers);
  stations.forEach((station) =>
    issue({ kind: 'escort', shipId: station.id, leaderId, offset: station.offset, radiusM: escortRadius, formation, slot: station.slot }),
  );
...
export function applyGroupFormation(
  desk: Pick<FleetDesk, 'controlGroups' | 'issue'>,
  group: FleetFormation,
  formation: Formation,
  members: readonly StationShip[],
): string {
  const held = desk.controlGroups.get(group.index);
  desk.issue({
    kind: 'control-group',
    index: group.index,
    group: { name: held?.name ?? `Group ${group.index}`, shipIds: held?.shipIds ?? [...group.shipIds], formation },
  });
  const queued = stationEscorts(desk.issue, group.leaderId, formation, members);
  return queued
    ? `${queued} escort orders queued · ${formationLabel(formation)}`
    : `${group.name} · ${formationLabel(formation)} · no escorts to station`;
}
...
export function sendEscort(view: FleetView, issue: IssueOrder, leaderId: string): string {
  ...
  const queued = guide
    ? stationEscorts(issue, leaderId, formation, [guide, ...view.recipients.filter((s) => s.id !== leaderId)].map(view.stationShip))
    : 0;
...
export function sendRoute(
  ...
  previous: Vec3[],
): { note: string; pending?: PendingRoute } {
  ...
  const points: Vec3[] = append
    ? [...previous, [point[0], 0, point[1]]]
    : [
        [lead.x, 0, lead.z],
        [point[0], 0, point[1]],
      ];
  ...
  const pending =
    receipt && receipt !== before && receipt.shipId === lead.id && receipt.command === 'route' ? { points, receipt } : undefined;
  if (recipients.length > 1) sendEscort(view, issue, lead.id);
  return {
    note: `${append ? 'Waypoint appended' : 'Route queued'} · ${speedKn} kn${recipients.length > 1 ? ` · ${formationLabel(view.formationOf(lead.id))}` : ''}`,
    pending,
  };
}
...
export function setGroupFormation(view: FleetView, desk: Pick<FleetDesk, 'controlGroups' | 'issue'>, formation: Formation): string {
  ...
  return applyGroupFormation(
    desk,
    group,
    formation,
    view.ships.filter((s) => group.shipIds.includes(s.id) && !s.physicalLost).map(view.stationShip),
  );
}
export function toggleWeapons(view: FleetView, issue: IssueOrder, kind: keyof WeaponsPolicy): void {
  ...
  view.recipients.forEach((s) =>
    issue({
      kind: 'weapons',
      shipId: s.id,
      policy: { ...(view.orders[s.id]?.weapons ?? { guns: true, aa: true, torpedoes: false }), [kind]: !free },
    }),
  );
}
...
export function sendAirOrder(
  view: FleetView,
  issue: IssueOrder,
  armed: SquadronAction | undefined,
  target: SquadronTarget,
  ...
  return {
    sent: true,
    note: [
      `${queued.length} air group orders queued`,
...
export function sendSearch(
  ...
  if (view.boundary && Math.hypot(...center) + search.radiusM + SEARCH_TURNING_ROOM_M > view.boundary.radiusM)
    return { note: 'The search area needs 1.5 km of turning room inside the battle boundary.', sent: false };
  const compatible = view.selectedFlights.filter((f) => search.policy !== 'strike' || f.role !== 'fighter');
  const ready = compatible.filter(launchable);
  const queued = ready.filter((f) =>
    issue({
      kind: 'squadron',
      flightId: f.id,
      order: { kind: 'search-area', center, radiusM: search.radiusM, altitude: search.altitude, policy: search.policy },
    }),
  );
  return {
    sent: true,
    note: [
      `${queued.length} search orders queued`,
      compatible.length < view.selectedFlights.length && `${view.selectedFlights.length - compatible.length} fighter groups skipped`,
      ready.length < compatible.length && `${compatible.length - ready.length} groups not ready`,
      queued.length < ready.length && `${ready.length - queued.length} groups unavailable`,
    ]
      .filter(Boolean)
      .join(' · '),
...
export type ShipGesture =
  | { act: 'feedback'; text: string }
  | { act: 'air'; target: SquadronTarget }
  | { act: 'escort'; leaderId: string }
  | { act: 'select-ship'; id: string; additive: boolean }
  | { act: 'focus'; targetId: string }
  | { act: 'inspect'; id: string; target: boolean };
export function shipGesture(
  view: FleetView,
  ship: Pick<Contact, 'id' | 'team'>,
  o: { right: boolean; additive: boolean; armed?: ArmedOrder },
): ShipGesture {
  const { armed, right } = o;
  ...
  if (!view.selectedFlights.length && report?.status === 'stale' && (right || armed))
    return { act: 'feedback', text: 'Report is stale. Search its last reported area before attacking.' };
  if (view.selectedFlights.length && (right || typeof armed === 'object'))
    return { act: 'air', target: { kind: 'ship', id: ship.id, team: ship.team } };
  if (ship.team === 'friendly')
    return right || armed === 'escort' ? { act: 'escort', leaderId: ship.id } : { act: 'select-ship', id: ship.id, additive: o.additive };
...
export function reportGesture(view: FleetView, report: ContactTrack, o: { right: boolean; armed?: ArmedOrder }): ShipGesture {
  ...
  if (view.selectedFlights.length && (right || typeof armed === 'object'))
    return { act: 'air', target: { kind: 'squadron', id: report.id, team: 'enemy' } };
...
export function fleetWaterAction(
  armed: 'move' | 'search' | 'squadron' | 'other' | undefined,
  o: { right: boolean; shift: boolean; flights: boolean; lead: boolean },
): FleetWaterAction | undefined {
...
export function fleetBoxSelection(
  ships: readonly Candidate[],
  planes: readonly (Candidate & { flightId: string })[],
  preferred: 'ships' | 'aircraft',
  contains: (position: Vec3) => boolean,
): { kind: 'ships' | 'aircraft'; ids: string[] } {
...
export function boxSelect(
  ...
  preferred: 'ships' | 'aircraft',
  contains: (position: Vec3) => boolean,
  held?: { shipIds: readonly string[]; flightIds: readonly string[] },
): { kind: 'ships' | 'aircraft'; ids: string[] } {
...
export function advancePendingRoutes(
  routes: Record<string, PendingRoute>,
  view: Pick<FleetView, 'afloat' | 'tick'>,
...
export function nextNotice(view: Pick<FleetView, 'notices' | 'ships'>, seen: number): { seen: number; text?: string } {
  ...
  if (count > seen) {
    const last = notices[count - 1],
      ship = view.ships.find((s) => s.id === last.shipId);
...
export function projectAirMarker(
  chart: Pick<FleetChart, 'projectAircraft' | 'projectContact' | 'projectContactGroup' | 'projectFleetShip' | 'projectAirMap'>,
  marker: DOMStringMap,
  [x, y, z]: Vec3,
): [number, number] | null {
