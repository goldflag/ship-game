...
import { advancePendingRoute, advancePendingRoutes, applyGroupFormation, boxSelect, fleetBoxSelection, fleetDragMode, fleetView, fleetWaterAction, nextNotice, projectAirMarker, projectMapHeading, reportGesture, sendAirOrder, sendHold, sendRoute, sendSearch, shipGesture, type Contact, type FleetViewInputs, type PendingRoute } from './fleetView';
...
const hull = (id: string, presetId: string, x: number, z: number, speedMps = 8): FleetActor => ({
  motion: { id, x, z, speed: speedMps, heading: 0 }, team: 'friendly', definition: shipPreset(presetId), damage: { integrity: 9000, maxIntegrity: 10000, sunk: false },
...
function fixture() {
  const frame: FleetFrame & { orderReceipts: OrderReceipt[] } = {
    tick: 600, phase: 'running', ship: { id: 'unit-1', x: 0, z: 0 },
    actors: [hull('unit-1', 'bismarck', 0, 0, 10), hull('unit-2', 'fletcher', 0, 450, 10), hull('unit-3', 'fletcher', 0, 900, 10)],
    observationTracks: [report], observedShips: [{ id: 'contact-b-1', health: .42, observedTick: 600 } as never],
    missionRules: { area: { radiusM: 20000, warningMarginM: 1500 } } as never,
    fleetOrders: { 'unit-1': standing({ type: 'hold' }), 'unit-2': standing({ type: 'escort', leaderId: 'unit-1', offset: [0, 450], radiusM: 160, formation: 'column', slot: 0 }), 'unit-3': standing({ type: 'escort', leaderId: 'unit-1', offset: [0, 900], radiusM: 160, formation: 'column', slot: 1 }) },
    fleetNotices: [], orderReceipts: [], shipScores: { 'unit-1': { damageDealt: 12400, frags: 1 } },
  };
  const contacts = [contact('unit-1', 'Bismarck', 'bismarck', 0, 0), contact('unit-2', 'Fletcher', 'fletcher', 0, 450), contact('unit-3', 'Fletcher', 'fletcher', 0, 900), contact('enemy-1', 'Yamato', 'yamato', 9000, -9000, 'enemy')];
  ...
  const issue = (order: FleetOrder) => {
    orders.push(order);
    if (order.kind === 'route') { queue.enqueue(order.shipId, { type: 'route', waypoints: [order.point], speedMps: order.speedMps, looped: false, append: order.append }); frame.orderReceipts.push(queue.receipts.at(-1)!); }
    return true;
  };
  const controlGroups = new Map<number, ControlGroup>([[1, { name: 'Group 1', shipIds: ['unit-1', 'unit-2', 'unit-3'], formation: 'column' }]]);
  const inputs = (selectedShipIds: string[] = [], rest: Partial<FleetViewInputs> = {}): FleetViewInputs => ({ combat: { contacts, result: 'active' }, selectedShipIds, selectedFlightIds: [], controlGroups, mapOpen: true, ...rest });
...
test('rows and marks come from the frame: numbered names, the column under its guide, standing orders and the guide label side', () => {
  ...
  expect(v.formations.map(f => [f.index, f.name, f.leaderId, f.shipIds, f.formation])).toEqual([[1, 'Bismarck formation', 'unit-1', ['unit-1', 'unit-2', 'unit-3'], 'column']]);
  ...
  expect(v.courseOf(v.ships[1]).points).toEqual([[0, 0, 450], [0, 0, 450]]);
  expect(v.bracketOf(v.formations[0])!.points).toEqual([[-420, 0, -420], [420, 0, -420], [420, 0, 1320], [-420, 0, 1320]]);
  ...
  expect(v.healthOf(report)).toBe(.42);
  expect(v.actionable).toBe(true);
  expect(fleetView({ ...view().frame, phase: 'loading' }, { combat: { contacts: [], result: 'active' }, selectedShipIds: [], selectedFlightIds: [], controlGroups: new Map(), mapOpen: true }).actionable).toBe(false);
});
...
test('box-select then move: the box picks the hulls, the door gets one route for the lead and a station for every other ship', () => {
  ...
  expect(pending).toEqual({ points: [[0, 0, 0], [3000, 0, 4000]], receipt: frame.orderReceipts[0] });
  ...
  expect(extended.pending?.points).toEqual([[0, 0, 0], [3000, 0, 4000], [5000, 0, 6000]]);
  ...
  expect(orders.slice(-2)).toEqual([{ kind: 'hold-area', shipId: 'unit-1', position: [0, 0], radiusM: 500 }, { kind: 'hold-area', shipId: 'unit-2', position: [0, 450], radiusM: 500 }]);
});
...
test('a control-group assignment records the formation on the group and re-stations its escorts by role through the same door', () => {
  ...
  expect(orders).toEqual([
    ...
    { kind: 'escort', shipId: 'unit-2', leaderId: 'unit-1', offset: [0, -SCREEN_OUTER_RADIUS_M], radiusM: STATION_RADIUS_M, formation: 'screen', slot: 0 },
    { kind: 'escort', shipId: 'unit-3', leaderId: 'unit-1', offset: [0, SCREEN_OUTER_RADIUS_M], radiusM: STATION_RADIUS_M, formation: 'screen', slot: 1 },
  ]);
  ...
  const authority = { simulation: { escortShip: (...args: unknown[]) => { escorts.push(args); } }, controlGroups: new Map(controlGroups) } as unknown as FleetAuthority;
  const desk = fleetDesk(authority);
  expect(applyGroupFormation(desk, v.selectedGroup!, 'line-abreast', v.afloat.map(v.stationShip))).toBe('2 escort orders queued · Line abreast');
  expect(desk.controlGroups.get(1)!.formation).toBe('line-abreast');
  expect(escorts.map(e => [e[0], e[1], e[4], e[5]])).toEqual([['unit-2', 'unit-1', 'line-abreast', 0], ['unit-3', 'unit-1', 'line-abreast', 1]]);
  // A session without escort orders refuses them at the door.
  expect(fleetDesk({ simulation: {}, controlGroups: new Map() } as unknown as FleetAuthority).issue({ kind: 'escort', shipId: 'a', leaderId: 'b', offset: [0, 0], radiusM: 1, formation: 'column', slot: 0 })).toBe(false);
});

test('clicks on hulls and reports resolve to one gesture each, with the armed order and the selection deciding', () => {
  const { view } = fixture();
  const v = view(['unit-1']);
  expect(shipGesture(v, { id: 'unit-2', team: 'friendly' }, { right: false, additive: true })).toEqual({ act: 'select-ship', id: 'unit-2', additive: true });
  expect(shipGesture(v, { id: 'unit-2', team: 'friendly' }, { right: true, additive: false })).toEqual({ act: 'escort', leaderId: 'unit-2' });
  expect(shipGesture(v, { id: 'unit-2', team: 'friendly' }, { right: false, additive: false, armed: 'escort' })).toEqual({ act: 'escort', leaderId: 'unit-2' });
  expect(shipGesture(v, { id: 'enemy-1', team: 'enemy' }, { right: false, additive: false })).toEqual({ act: 'inspect', id: 'enemy-1', target: true });
  expect(shipGesture(v, { id: 'enemy-1', team: 'enemy' }, { right: true, additive: false })).toEqual({ act: 'focus', targetId: 'enemy-1' });
  expect(shipGesture(v, { id: 'enemy-1', team: 'enemy' }, { right: false, additive: false, armed: 'search' })).toEqual({ act: 'feedback', text: 'Choose water for the center of the search area.' });
  ...
  const stale = fleetView({ ...v.frame, observationTracks: [{ ...report, status: 'stale' }] }, { combat: { contacts: [], result: 'active' }, selectedShipIds: [], selectedFlightIds: [], controlGroups: new Map(), mapOpen: true });
  expect(reportGesture(stale, report, { right: true })).toEqual({ act: 'feedback', text: 'Report is stale. Search its last reported area before attacking.' });
  const sinking = fleetView({ ...v.frame, observationTracks: [{ ...report, visibleCondition: { sinking: true } as never }] }, { combat: { contacts: [], result: 'active' }, selectedShipIds: [], selectedFlightIds: [], controlGroups: new Map(), mapOpen: true });
  ...
  expect(reportGesture(v, air, { right: true })).toEqual({ act: 'feedback', text: 'Select a fighter group and Intercept, then choose an aircraft contact.' });
});
...
test('the air wing summary and orders come from the carriers in the frame', () => {
  ...
  const inputs = (selectedFlightIds: string[]): FleetViewInputs => ({ combat, selectedShipIds: [], selectedFlightIds, controlGroups: new Map(), mapOpen: true });
  ...
  const fighter = v.flights.find(f => f.role === 'fighter')!, bomber = v.flights.find(f => f.role !== 'fighter')!;
  ...
  const orders: FleetOrder[] = [];
  const issue = (order: FleetOrder) => { orders.push(order); return true; };
  ...
  expect(sendAirOrder(chosen, issue, defend, { kind: 'ship', id: simulation.ship.id, team: 'friendly' })).toEqual({ sent: true, note: '1 air group orders queued · 1 incompatible groups skipped' });
  ...
  expect(sendAirOrder(chosen, issue, patrol, { kind: 'ship', id: simulation.ship.id, team: 'friendly' })).toEqual({ sent: false, note: `Choose ${patrol.target}.` });
  expect(orders).toHaveLength(1);
  // A search needs turning room inside the boundary before any group is asked.
  expect(sendSearch(chosen, issue, [19000, 0], { radiusM: 4000, altitude: 'medium', policy: 'report' })).toEqual({ sent: false, note: 'The search area needs 1.5 km of turning room inside the battle boundary.' });
  expect(sendSearch(chosen, issue, [1000, 0], { radiusM: 4000, altitude: 'medium', policy: 'strike' })).toEqual({ sent: true, note: '1 search orders queued · 1 fighter groups skipped' });
  expect(orders.at(-1)).toEqual({ kind: 'squadron', flightId: bomber.id, order: { kind: 'search-area', center: [1000, 0], radiusM: 4000, altitude: 'medium', policy: 'strike' } });
});
...
test('mixed boxes use the active tab, deduplicate flights and still select ships from the Aircraft tab', () => {
  ...
  expect(fleetBoxSelection(ships, [...planes, planes[0]], 'aircraft', containsBox)).toEqual({ kind: 'aircraft', ids: ['carrier-a/flight', 'carrier-b/flight'] });
...
test('Shift-left drag selects, Ctrl/Cmd adds to the box selection, and unshifted modifiers orbit', () => {
  ...
  const { view } = fixture();
  expect(boxSelect(view(), 'ships', () => true, { shipIds: ['unit-3'], flightIds: [] })).toEqual({ kind: 'ships', ids: ['unit-3', 'unit-1', 'unit-2'] });
});
...
test('plane markers and group labels follow frame poses instead of cached telemetry coordinates', () => {
  let x = 100;
  const chart = { projectAirMap: () => [10, 20], projectAircraft: () => [x, 30], projectContact: () => [x, 40], projectContactGroup: () => [x, 50], projectFleetShip: () => null } as never;
