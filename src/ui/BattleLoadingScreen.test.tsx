...
test('PvE loading uses only its friendly briefing and never displays the previous Custom opponent', () => {
  const briefing = {
    ...
    groups: [],
    assignments: [],
    totals: { displacementKg: 2924000, ships: 1, aircraft: 0 },
  ...
  const html = renderToStaticMarkup(
    <BattleLoadingScreen
      briefing={briefing}
      setup={{ playerShipId: 'enterprise-cv6', friendlyBots: [], enemies: ['yamato'], mapId: 'north-atlantic', spawnDistance: 5000 }}
      state={{ label: 'Preparing ship recognition models', progress: 0.5, leaving: false }}
      onLeft={() => {}}
    />,
  );
  expect(html).toContain('PvE fleet command');
  expect(html).toContain('Unknown');
  expect(html).toContain('Fletcher');
  expect(html).not.toContain('Yamato');
