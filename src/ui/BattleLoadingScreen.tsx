...
export function BattleLoadingScreen({ setup, state, onLeft, multiplayer, briefing }: Props) {
  ...
  const friendly = briefing
    ? briefing.setup.ships.map((s) => s.presetId)
    : [setup.playerShipId, ...setup.friendlyBots.map((entry) => botSelection(entry).shipId)];
  const percent = Math.round(Math.min(state.progress, 1) * 100);
  const roster = (ids: string[], team: 'Friendly' | 'Enemy') => (
    <ol className="battle-loading-roster" aria-label={`${team} fleet`}>
      {ids.map((id, index) => (
        <li key={`${team}-${index}`} className={!briefing && team === 'Friendly' && index === 0 ? 'battle-loading-player' : undefined}>
  ...
  return (
    <section
      className={`loading-screen battle-loading ${state.leaving ? 'battle-loading-leaving' : ''}`}
      aria-live="polite"
      ...
      <div className="battle-loading-content">
        <p className="battle-loading-kicker">
          {briefing ? (
            ...
            <>
              {multiplayer ? '1v1 battle' : 'Custom battle'} ·{' '}
              {setup.timeHours !== undefined ? formatBattleTime(setup.timeHours) : time && time.id !== 'map' ? time.name : 'Map daylight'} ·{' '}
              {setup.cloudCover !== undefined
                ? `${setup.cloudCover}% clouds`
                : weather && weather.id !== 'map'
                  ? weather.name
                  : 'Map weather'}
              {setup.windSpeed !== undefined && ` · ${setup.windSpeed} m/s wind`} · {setup.spawnDistance / 1000} km
            </>
          )}
        </p>
        ...
        <div className="battle-loading-fleets">
          <div>
            <h2>
              Friendly fleet <span>{friendly.length}</span>
            </h2>
            {roster(friendly, 'Friendly')}
          </div>
          <div className="battle-loading-versus" aria-hidden="true">
            VS
          </div>
          <div>
            <h2>Enemy fleet {briefing ? <span>Unknown</span> : <span>{setup.enemies.length}</span>}</h2>
            {briefing ? (
              <p className="loading-subtitle">Enemy strength scales to your fleet. Establish contact to locate its ships.</p>
            ) : (
              roster(
                setup.enemies.map((entry) => botSelection(entry).shipId),
                'Enemy',
              )
            )}
          </div>
        ...
        <div
          ...
          role="progressbar"
          aria-label="Preparing the battle"
