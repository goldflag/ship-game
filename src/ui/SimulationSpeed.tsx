...
export function SimulationSpeed({ desk, data, bindings }: { desk: FleetDesk; data: Telemetry; bindings?: Keybindings }) {
  if (desk.frame.networked || !desk.frame.missionRules || !desk.can.setSpeed) return null;
  ...
  return <div className="fleet-simulation-speed" role="group" aria-label="Simulation speed">
    {([1, 2, 4] as const).map(speed => <button key={speed} aria-pressed={selected === speed}
      ...
      onClick={() => desk.issue({ kind: 'simulation-speed', speed })}>{speed}×</button>)}
