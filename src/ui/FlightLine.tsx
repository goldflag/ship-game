...
export interface FlightLineCarrier {
  id: string;
  name: string;
  hull: number;
  kn: number;
...
export interface FlightLineProps {
  ...
  search: {
    ...
    setAltitude(v: SearchAltitude): void;
    setPolicy(v: SearchPolicy): void;
  };
  ...
  onFollowLead(flight: FlightLineFlight): void;
  canFollow(flight: FlightLineFlight): boolean;
  onCentre(flight: FlightLineFlight): void;
  onService(flights: FlightSummary[], action: DeckServiceAction): void;
  onDeckPolicy(carrierId: string, policy: DeckPolicy): void;
  onPrioritize(carrierId: string, requestId: number): void;
  onCancelTask(carrierId: string, requestId: number): void;
  onToggleDeck(carrierId: string | undefined): void;
  onClose(): void;
}

const VERBS: Record<AirVerb, { label: string; key: string }> = {
  patrol: { label: 'Loiter', key: 'L' },
  attack: { label: 'Strike', key: 'A' },
  defend: { label: 'Defend', key: 'D' },
  intercept: { label: 'Intercept', key: 'I' },
  escort: { label: 'Escort', key: 'E' },
  return: { label: 'Return', key: 'R' },
  search: { label: 'Search', key: 'S' },
};
...
const ORDER_LABELS: Record<string, string> = {
  ...
  defend: 'Defend',
  intercept: 'Intercept',
  'intercept-contact': 'Intercept',
  escort: 'Escort',
  'search-area': 'Search',
  return: 'Return',
};
const POLICY_LABELS: Record<DeckPolicy, string> = {
  balanced: 'Balanced',
  'launch-first': 'Launch first',
  'recover-first': 'Recover first',
};
...
const armament = (p: Aircraft) =>
  lost(p) ? 0 : p.role === 'fighter' ? Math.max(0, Math.min(1, p.ammo / FIGHTER_AMMO_BURSTS)) : p.payload ? 1 : 0;
const condition = (p: Aircraft) => (lost(p) ? 'lost' : p.hp < 25 ? 'critical' : p.hp < 50 ? 'damaged' : undefined);
const planeTitle = (p: Aircraft) =>
  `${p.id.split('/').at(-1)} · ${Math.ceil(Math.max(0, p.hp))}% · ${p.lossReason ?? p.phase}${loaded(p) ? ' · armed' : p.role === 'fighter' ? ' · no ammunition' : ' · no payload'}`;
...
const whereabouts = (f: FlightSummary) =>
  f.airborne > 0
    ? f.enduranceSeconds !== null
      ? duration(f.enduranceSeconds)
      : 'airborne'
    : (f.deck?.onDeck ?? 0) > 0
      ? 'on deck'
      : 'in hangar';
const orderLine = (f: FlightSummary) =>
  sortie(f) ? (
    <>
      <b>{ORDER_LABELS[f.order.kind] ?? f.activity}</b> · {mission(f)}
    </>
  ) : (
    <b>{f.activity}</b>
  );
...
export function FlightLine({
  ...
  const verbs: AirVerb[] = selected.length
    ? [
        'patrol',
        ...(selected.some((f) => f.role === 'fighter') ? ['defend' as const] : []),
        ...(selected.some((f) => f.role !== 'fighter') ? ['attack' as const] : []),
        'intercept',
        'escort',
        'return',
        ...(hasBoundary ? ['search' as const] : []),
      ]
    : [];
  ...
  const hint =
    selected.length === 0 ? (
      <>
        Select a group · <kbd>1</kbd>–<kbd>9</kbd> · or click its planes on the chart
      </>
    ) : selected.length === 1 ? (
      <>
        <b>{selected[0].name}</b> · {selected[0].active ? mission(selected[0]) : selected[0].activity} ·{' '}
        {groupArmament(planesOf(selected[0])).label}
        {selected[0].notice ? ` · ${selected[0].notice}` : ''}
      </>
    ) : (
      <>
  ...
  const bar = (
    <div className="flight-line-bar" role="group" aria-label="Air group orders">
      {selected.length > 0 && (
        <>
          {verbs.map((verb) => (
            <button key={verb} disabled={verbDisabled(verb)} aria-pressed={armed === verb} onClick={() => onVerb(verb)}>
              {VERBS[verb].label}
              <kbd>{VERBS[verb].key}</kbd>
            </button>
          ))}
          <span className="flight-line-sep" />
          <button disabled={!canFollow(selected[0])} onClick={() => onFollowLead(selected[0])}>
            <Icon name="camera" size={13} />
            Follow lead
          </button>
          <button onClick={() => onCentre(selected[0])}>
          ...
          {decked.length > 0 && (
            <>
              <span className="flight-line-sep" />
              <AirGroupService flights={decked} enabled={actionable} command={onService} compact />
            </>
          )}
        </>
      )}
  ...
  const searchRow = armed === 'search' && hasBoundary && (
    <div className="flight-line-search" role="group" aria-label="Search settings">
      <label>
        Radius
        <Select value={search.radius} disabled={!actionable} onValueChange={(v) => search.setRadius(Number(v))}>
      ...
      <label>
        Altitude
        <Select value={search.altitude} disabled={!actionable} onValueChange={(v) => search.setAltitude(v as SearchAltitude)}>
      ...
      <label>
        On contact
        <Select value={search.policy} disabled={!actionable} onValueChange={(v) => search.setPolicy(v as SearchPolicy)}>
  ...
  const chip = (carrier: FlightLineCarrier) => {
    ...
    const waiting = deck?.queue.length
      ? deck.queue.every((r) => r.action === 'raise') && deck.occupied >= deck.capacity
        ? 'Waiting for deck space'
        : 'Handling tasks queued'
      : 'Deck crew ready';
    ...
    const task =
      ...
      (deck
        ? deck.suspended
          ? 'Deck operations suspended'
          : `${deck.task ?? waiting}${current ? ` · ${current.name}` : ''}`
        : carrier.order);
    const open = deckCarrier?.id === carrier.id;
    return (
      <div
        key={carrier.id}
        className="flight-line-carrier"
        data-carrier-id={carrier.id}
        role="group"
        aria-label={`${carrier.name} air wing`}
      >
        <div className="flight-line-carrier-name">
          <b>{carrier.name}</b>
          <span className={carrier.hull < 0.5 ? 'warn' : undefined}>
            {Math.round(carrier.hull * 100)}% · {carrier.kn} kn
          </span>
        </div>
        ...
        <p className="flight-line-task" data-suspended={!!closed || deck?.suspended || undefined} title={task}>
          {task}
        </p>
        {deck && (
          <button className="flight-line-deck-toggle" aria-expanded={open} onClick={() => onToggleDeck(open ? undefined : carrier.id)}>
            Deck · <b>{POLICY_LABELS[deck.policy]}</b>
            {deck.queue.length ? ` · queue ${deck.queue.length}` : ''}
            <Icon name="chevron" size={11} style={{ transform: open ? 'rotate(180deg)' : undefined }} />
          </button>
        )}
      </div>
    );
  ...
  const box = (flight: FlightLineFlight) => {
    ...
    return (
      <button
        key={flight.id}
        className={['flight-line-box', onDeck && 'decked', hoverId === flight.id && 'hovered'].filter(Boolean).join(' ')}
        ...
        onFocus={() => onHover(flight.id)}
        onBlur={() => onHover(undefined)}
        onClick={(event) => onSelect(flight.id, event.shiftKey || event.ctrlKey || event.metaKey)}
      >
        <span className="flight-line-box-head">
          <strong>{flight.name}</strong>
          {index < 9 && <kbd>{index + 1}</kbd>}
        </span>
        {model ? (
          <img
            className="flight-line-aircraft"
            src={assetUrl(`models/aircraft/${model}-thumbnail.png`)}
            alt=""
            width="76"
            height="34"
            draggable={false}
          ...
          <span className="flight-line-aircraft" />
        )}
        <span className="flight-line-count">
          {flight.surviving}
          <small>/{flight.total}</small>
        </span>
        <span className={`flight-line-status ${flight.notice ? 'notice' : onDeck ? 'decked' : ''}`}>
          <svg viewBox="-12 -12 24 24" aria-hidden="true">
            <path d={PLANE_GLYPHS[flight.role]} transform="scale(1.1)" />
          </svg>
          <span>{flight.notice ?? statusLabel(flight)}</span>
        </span>
        <span className="flight-line-ticks" aria-hidden="true">
          {planes.map((p) => (
            <i key={p.id} className={lost(p) ? 'lost' : p.hp < 50 ? 'hurt' : loaded(p) ? 'armed' : 'empty'} title={planeTitle(p)} />
          ))}
        </span>
        <span className="flight-line-foot">
          <b className={armament.empty ? 'dim' : armament.low ? 'low' : undefined}>{armament.label}</b>
          <span>{whereabouts(flight).replace('in hangar', 'hangar').replace('on deck', 'deck')}</span>
        </span>
      </button>
    );
  };
  ...
  const card =
    hovered &&
    (() => {
      const planes = planesOf(hovered),
        alive = planes.filter((p) => !lost(p));
      return (
        <div className="flight-line-card" role="tooltip" style={{ '--left': `${cardLeft}px` } as CSSProperties}>
          <div className="flight-line-card-head">
            <b>{hovered.name}</b>
            <span>
              {hovered.surviving}/{hovered.total}
              <small>
                {groupArmament(planes).label} · {whereabouts(hovered)}
                {hovered.airborne > 0 && hovered.enduranceSeconds !== null ? ' endurance' : ''}
              </small>
            </span>
          </div>
          <p className="flight-line-order">{orderLine(hovered)}</p>
          <div className="flight-line-planes">
            <span className="flight-line-cells">
              {planes.map((p) => (
                <i
                  key={p.id}
                  className="air-manifest-cell"
                  data-status={airStatus(p)}
                  data-condition={condition(p)}
                  title={planeTitle(p)}
                  style={{ '--condition': lost(p) ? 0 : Math.max(0, Math.min(1, p.hp / 100)), '--armament': armament(p) } as CSSProperties}
                />
              ))}
            </span>
            {alive.length > 0 && (
  ...
  const deck = deckCarrier && (
    <div className="flight-line-deck" style={{ '--left': `${deckLeft}px` } as CSSProperties}>
      <div className="flight-line-deck-head">
        <b>{deckCarrier.name} · flight deck</b>
        <button onClick={() => onToggleDeck(undefined)}>
          Close<kbd>Esc</kbd>
        </button>
      </div>
      <CarrierDeck
        name={deckCarrier.name}
        wing={deckCarrier.wing}
        enabled={actionable}
        setPolicy={(policy) => onDeckPolicy(deckCarrier.id, policy)}
        prioritize={(id) => onPrioritize(deckCarrier.id, id)}
        cancel={(id) => onCancelTask(deckCarrier.id, id)}
      />
    </div>
  );

  return (
    <section ref={line} className="flight-line" aria-label="Air groups">
      {bar}
      {searchRow}
      <div className="flight-line-row">
        {carriers.map((carrier) => (
          <div key={carrier.id} className="flight-line-wing" style={{ display: 'contents' }}>
            {chip(carrier)}
            {flights.filter((f) => f.ownerId === carrier.id).map(box)}
          </div>
        ))}
        <button className="flight-line-hide" onClick={onClose}>
          Hide<kbd>Esc</kbd>
        </button>
      </div>
      {card}
