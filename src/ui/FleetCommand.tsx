...
export function FleetCommand({ data, desk, bindings, instrumentsVisible = true }: { data: Telemetry; desk: FleetDesk; bindings: Keybindings; instrumentsVisible?: boolean }) {
  ...
  const [search, setSearch] = useState<{ radiusM: number; altitude: SearchAltitude; policy: SearchPolicy }>({ radiusM: 4000, altitude: 'medium', policy: 'report' });
  ...
  useEffect(() => { if (!airOpen) setDeckOpen(undefined); }, [airOpen]);
  ...
  const reveal = (x: number, z: number) => { if (map.current && !chart.projectAirMap(x, z)) chart.centerAirMapOn(x, z); };
  const selectShips = (shipIds: string[]) => { setFilter('ships'); issue({ kind: 'select-ships', ids: shipIds }); issue({ kind: 'select-flights', ids: [] }); setContactId(undefined); setArmed(undefined); };
  ...
  const selectAir = (id: string, additive = false) => { setFilter('aircraft'); issue({ kind: 'select-flight', id, additive }); issue({ kind: 'select-ships', ids: [] }); setContactId(undefined); setArmed(undefined); const flight = flights.find(f => f.id === id); if (flight && !additive) reveal(flight.position[0], flight.position[2]); };
  const issueAir = (target: SquadronTarget) => {
    const held = current.current;
    if (held.armed === 'search') { setFeedback('Choose water for the center of the search area.'); return; }
  ...
  const escort = (leaderId: string) => { setFeedback(sendEscort(view, issue, leaderId)); setArmed(undefined); };
  ...
  const move = (point: [number, number], appendRequested: boolean) => {
    ...
    if (!lead) { setFeedback(note); return; }
  ...
  const apply = (gesture: ShipGesture) => {
    ...
    else if (gesture.act === 'focus') { setFeedback(sendFocus(view, issue, gesture.targetId)); setArmed(undefined); }
    else { selectShips([]); setContactId(gesture.id); if (gesture.target) issue({ kind: 'select-target', id: gesture.id }); }
  };
  const targetShip = (ship: Pick<Contact, 'id' | 'team'>, right: boolean, additive: boolean) => apply(shipGesture(current.current.view, ship, { right, additive, armed }));
  ...
  const water = (event: { clientX: number; clientY: number; shiftKey: boolean }, right: boolean) => {
    ...
    else if (action === 'clear') { selectShips([]); issue({ kind: 'select-flights', ids: [] }); }
  };
  const arm = (order: ArmedOrder) => {
    if (!mapOpen) issue({ kind: 'fleet-command' }); setArmed(order); setFeedback('');
  ...
  const cycleFormation = () => chooseFormation(FORMATIONS[(FORMATIONS.findIndex(f => f.id === (selectedGroup?.formation ?? 'column')) + 1) % FORMATIONS.length].id);
  const airAction = (key: string) => {
    const held = current.current;
    if (!held.view.selectedFlights.length || !held.actionable) return;
    if (key === 'R') { setFeedback(sendReturn(held.view, issue)); return; }
  ...
  const escape = () => {
    ...
    else if (contactId) { setContactId(undefined); issue({ kind: 'select-target', id: '' }); }
  ...
  const selectFlightAt = (index: number) => { const flight = flights[index]; if (flight) selectAir(flight.id); };
  ...
  useEffect(() => {
    const keydown = (event: KeyboardEvent) => {
      ...
      if (toggle) { if (mapActive) issue({ kind: 'follow-ship', id: desk.selectedShipIds[0] ?? desk.frame.ship.id }); else issue({ kind: 'fleet-command' }); }
      else if (event.code === 'Space') issue({ kind: 'tactical-pause' });
      else if (showing && !mapActive && letter === 'T') { if (held.data.spectatedShipId && held.actionable) issue({ kind: 'take-fleet-helm', id: held.data.spectatedShipId }); }
      ...
      else if (event.code === 'Escape') { if (showing) handlers.current.escape(); else issue({ kind: 'menu' }); }
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      const rect = map.current?.getBoundingClientRect();
      if (rect && !drag.current && !document.querySelector('dialog[open]')) { const [dx, dy] = nav.current.step(dt, rect.width, rect.height); if (dx || dy) chart.panAirMap(dx, dy); }
  ...
  const endDrag = (event: ReactPointerEvent<SVGSVGElement>) => {
    ...
    const selection = boxSelect(view, filter, position => { const p = chart.projectAirMap(position[0], position[2], position[1]); return !!p && inside(p[0] * scaleX, p[1] * scaleY); },
    ...
    if (selection.kind === 'ships') selectShips(selection.ids);
      ...
      issue({ kind: 'select-ships', ids: [] }); setFilter('aircraft'); setContactId(undefined); setArmed(undefined);
    }
  };
  useEffect(() => { setPendingRoutes(routes => advancePendingRoutes(routes, view)); }, [desk, data]);
  useEffect(() => {
  ...
  const weaponsRow = recipients.length > 0 && <div className="fleet-command-weapons" role="group" aria-label="Weapons policy">{WEAPONS.map(kind => { const free = view.weaponsFree(kind); return <button key={kind} disabled={!actionable} aria-pressed={free} onClick={() => toggleWeapons(view, issue, kind)}><i/>{weaponLabel[kind]} {free ? 'free' : 'held'}</button>; })}</div>;
    ...
    return <div className={`fleet-command ${helm ? 'fleet-command-helm-mode' : 'fleet-command-follow'}`}>
      <section className="fleet-command-panel" aria-label={helm ? 'Fleet helm controls' : 'Following ship'}>
        ...
        <div className="fleet-command-panel-actions">
          ...
          {helm ? <button onClick={() => issue({ kind: 'follow-ship', id: data.controlledShipId! })}>Give back helm</button> : <button disabled={!actionable || !subject || subject.physicalLost} onClick={() => subject && issue({ kind: 'take-fleet-helm', id: subject.id })}>Take helm <kbd>T</kbd></button>}
          <button onClick={() => issue({ kind: 'fleet-command' })}><Icon name="compass" size={14}/>Fleet command <kbd>M</kbd></button>
        </div>
        {!helm && weaponsRow}
  ...
  const formationPicker = <div className="fleet-command-formations" role="group" aria-label="Formation">
    ...
    <div>{FORMATIONS.map(entry => <button key={entry.id} title={selectedGroup ? entry.hint : FORMATION_HINT} disabled={!actionable || !selectedGroup}
      aria-pressed={!!selectedGroup && selectedGroup.formation === entry.id} onClick={() => chooseFormation(entry.id)}>{entry.label}</button>)}</div>
  ...
  const airItems: WheelItem[] = [
    ...SQUADRON_ACTIONS.filter(a => selectedFlights.some(f => actionAvailable(a, f.role))).map(a => ({ kind: a.kind, label: a.label, sub: typeof armed === 'object' && armed.kind === a.kind ? `${a.key} · armed` : a.key, armed: typeof armed === 'object' && armed.kind === a.kind, disabled: !actionable || !view.airReady(a) })),
    { kind: 'return', label: 'Return', sub: 'R', disabled: !actionable || !selectedFlights.some(f => f.active) },
    ...(boundary ? [{ kind: 'search', label: 'Search', sub: armed === 'search' ? 'S · armed' : 'S', armed: armed === 'search', disabled: !actionable || !view.searchReady(search.policy) }] : []),
  ];
  ...
  const service = (groups: readonly { id: string }[], action: Parameters<typeof sendDeckService>[2]) => { const note = sendDeckService(issue, groups, action); setArmed(undefined); setFeedback(note); };
  ...
  const chartInteraction: SVGProps<SVGSVGElement> = {
    ...
    onWheel: e => { const r = e.currentTarget.getBoundingClientRect(); chart.zoomAirMap(e.deltaY, e.clientX - r.left, e.clientY - r.top); },
  };
  ...
  return <div onClickCapture={e => {
    ...
    <svg ref={map} className="fleet-command-chart" tabIndex={0} aria-label="Fleet command chart" data-armed={armed ? true : undefined} {...chartInteraction}>
      ...
      {flights.filter(f => f.active || f.airborne > 0).map(f => {
        const on = desk.selectedFlightIds.includes(f.id), carrier = ships.find(s => s.id === f.ownerId);
        const from: Vec3 = f.airborne > 0 ? f.position : carrier ? [carrier.x, 0, carrier.z] : f.position;
        const patrol = f.order.kind === 'patrol', searchArea = f.order.kind === 'search-area' ? f.order : undefined;
        ...
        return <g key={`air-${f.id}`} className={`fleet-command-air-course ${on ? 'selected' : ''} ${f.airborne === 0 ? 'planned' : ''}`} aria-label={`${f.name} course`}>
          {path.length > 1 && <path className="fleet-command-air-route" data-map-smooth="true" data-map-path={JSON.stringify(path)}/>}
          ...
          {searchArea && <path className="fleet-command-search-area" data-map-path={JSON.stringify(circlePoints(searchArea.center[0], searchArea.center[1], searchArea.radiusM))} data-closed="true"/>}
        </g>;
      })}
      ...
      {previewArmed(armed) && armed !== 'move' && movePoint && selectedFlights.length > 0 && <g className="fleet-command-move-preview air" aria-label="Air order preview">
        ...
        {armed === 'search' && <path data-map-path={JSON.stringify(circlePoints(movePoint[0], movePoint[1], search.radiusM))} data-closed="true"/>}
        {armed !== 'search' && <path data-map-path={JSON.stringify(circlePoints(movePoint[0], movePoint[1], LOITER_RADIUS_M))} data-closed="true"/>}
        <g data-map-position={JSON.stringify([movePoint[0], 0, movePoint[1]])}><circle r="9"/><path d="M-14 0H14M0-14V14"/><text x="16" y="-8">{armed === 'search' ? `Search · ${search.radiusM / 1000} km` : 'Loiter here'}</text></g>
      </g>}
    ...
    <div ref={anchors} className="fleet-command-anchors">
      {lead && !selectedFlights.length && <div className={`fleet-command-wheel-anchor ${flipAt(lead.x, lead.z)}`} data-map-position={JSON.stringify([lead.x, 0, lead.z])}>
        {armed ? chip(recipients.length > 1 ? `${recipients.length} ships` : lead.name) : <div className="fleet-command-wheel-block">
          <div className="fleet-command-wheel-title"><strong>{recipients.length > 1 ? `${recipients.length} ships` : lead.name}</strong><small>{recipients.length > 1 ? recipients.map(s => s.name).join(' · ') : `${Math.round(lead.integrity * 100)}% · ${view.speedKnOf(lead.id)} kn · ${view.standingOrderOf(lead.id)}`}</small></div>
      ...
      {view.stragglerTags.map(({ leader, straggler, ship }) => ship && <div key={`straggler-${straggler.shipId}`} className="fleet-command-straggler" data-map-position={JSON.stringify([ship.x, 0, ship.z])} role="group" aria-label={`${ship.name} straggling`}>
        ...
        {(['slow-for-stragglers', 'leave-stragglers'] as const).map(policy => <button key={policy} disabled={!actionable} aria-pressed={orders[leader.id]?.formationPolicy === policy} onClick={() => { issue({ kind: 'formation-policy', shipId: leader.id, policy }); setFeedback(`${leader.name} · Formation policy queued`); }}>{policy === 'slow-for-stragglers' ? 'Slow for stragglers' : 'Leave behind'}</button>)}
      </div>)}
        ...
        {armed ? chip(selectedFlights.length === 1 ? selectedFlights[0].name : `${selectedFlights.length} air groups`) : <div className="fleet-command-wheel-block air">
          ...
          <div className="fleet-command-wheel-row">
            ...
            <div className="fleet-command-wheel-actions">
              ...
              <button onClick={() => issue({ kind: 'follow-aircraft', id: planesOf(selectedFlights[0]).find(p => airborne(p))?.id ?? '' })} disabled={!planesOf(selectedFlights[0]).some(p => airborne(p))}><Icon name="camera" size={13}/>Follow lead</button>
            </div>
          </div>
          {!lineOpen && <div className="fleet-command-planes">{selectedFlights.map(f => <div key={f.id} className="fleet-command-plane-row" onMouseEnter={() => setHoverFlightId(f.id)} onMouseLeave={() => setHoverFlightId(undefined)}><b>{f.name}</b><span className="fleet-command-plane-bars">{planesOf(f).map(p => <i key={p.id} className={p.hp <= 0 || ['lost', 'withdrawn'].includes(p.phase) ? 'lost' : p.hp < 50 ? 'hurt' : ''} style={{ ['--hp' as string]: `${Math.max(0, Math.min(100, p.hp))}%` }} title={`${p.id.split('/').at(-1)} · ${Math.ceil(p.hp)}% · ${p.lossReason ?? p.phase}${p.role === 'fighter' ? p.ammo > 0 ? ' · armed' : ' · no ammunition' : p.payload ? ' · armed' : ' · no payload'}`}/>)}</span><span>{f.surviving}/{f.total}</span></div>)}</div>}
          {boundary && !lineOpen && <div className="fleet-command-buttons fleet-command-search">
            <label>Radius<Select value={search.radiusM} onValueChange={v => setSearch(s => ({ ...s, radiusM: Number(v) }))}>{[2000, 4000, 6000].map(v => <SelectOption key={v} value={v}>{v / 1000} km</SelectOption>)}</Select></label>
            <label>Altitude<Select value={search.altitude} onValueChange={v => setSearch(s => ({ ...s, altitude: v as SearchAltitude }))}><SelectOption value="low">Low · 200 m</SelectOption><SelectOption value="medium">Medium · 850 m</SelectOption><SelectOption value="high">High · 1,500 m</SelectOption></Select></label>
            <label>On contact<Select value={search.policy} onValueChange={v => setSearch(s => ({ ...s, policy: v as SearchPolicy }))}><SelectOption value="report">Report only</SelectOption><SelectOption value="shadow">Shadow and report</SelectOption><SelectOption value="strike">Search and strike</SelectOption></Select></label>
          </div>}
      ...
      {selectedContact && <div className="fleet-command-popover-anchor" {...(selectedReport ? { 'data-map-position': JSON.stringify(reportPosition(selectedReport, tick)), 'data-contact-marker': selectedReport.id } : { 'data-map-position': JSON.stringify([selectedContact.x, 0, selectedContact.z]) })}>
        <section className="fleet-command-popover" aria-label="Contact report">
          ...
          {selectedReport && view.healthOf(selectedReport) !== undefined && <p>{Math.round(view.healthOf(selectedReport)! * 100)}% HP remaining</p>}
          ...
          <div className="fleet-command-buttons">
            {selectedReport?.kind === 'aircraft' && interceptor && <button disabled={!actionable || selectedReport.status === 'stale'} onClick={() => { if (issue({ kind: 'squadron', flightId: interceptor.id, order: { kind: 'intercept', flightId: selectedReport.id } })) setFeedback(`${interceptor.name} · Intercept order queued`); else setFeedback(`${interceptor.name} is not available to intercept.`); }}><Icon name="aircraft" size={13}/>Intercept · {interceptor.name}</button>}
            <button disabled={!actionable || !selectedFlights.length && !flights.some(f => f.role !== 'fighter' || true)} onClick={() => { if (!current.current.view.selectedFlights.length) { setFeedback('Select air groups first, then search here.'); return; } arm('search'); }}><Icon name="target" size={13}/>Search here<kbd>S</kbd></button>
            <button onClick={() => { setContactId(undefined); issue({ kind: 'select-target', id: '' }); }}>Close<kbd>Esc</kbd></button>
          </div>
        </section>
      </div>}
    </div>
    <div className="fleet-command-recon"><ReconnaissanceLegend coverage={desk.frame.reconCoverage}/>{desk.frame.reconCoverage && <span>{coverageTick === undefined ? 'Point at water for search age' : `Here: observed ${observationAge(coverageTick, tick)}`}</span>}</div>
    {box && <div className="fleet-command-box" style={{ left: box.x, top: box.y, width: box.width, height: box.height }}/>}
    <SquadronLabels data={data} desk={desk} onPointerDown={e => {
    ...
    <header className="fleet-command-top">
      ...
      <nav aria-label="Fleet views">
        ...
        <button onClick={() => chart.fitAirMap()} title="Fit reported fleet"><Icon name="expand" size={14}/>Fit</button>
    ...
    {lineOpen && <FlightLine
      carriers={wings.map(({ owner, wing }) => ({ id: owner.motion.id, name: nameFor(owner.motion.id), hull: ships.find(s => s.id === owner.motion.id)?.integrity ?? 0,
        kn: Math.round(Math.abs((owner.motion.speed ?? 0) * KNOTS_PER_MPS)), order: view.standingOrderOf(owner.motion.id), wing }))}
      flights={flights} planesOf={planesOf} selectedIds={desk.selectedFlightIds} hoverId={hoverFlightId}
      armed={typeof armed === 'object' ? armed.kind : armed === 'search' ? 'search' : undefined} actionable={actionable} hasBoundary={!!boundary}
      search={{ radius: search.radiusM, altitude: search.altitude, policy: search.policy, setRadius: radiusM => setSearch(s => ({ ...s, radiusM })), setAltitude: altitude => setSearch(s => ({ ...s, altitude })), setPolicy: policy => setSearch(s => ({ ...s, policy })) }}
      onHover={setHoverFlightId} onSelect={selectAir} onVerb={onAirWheel}
      onFollowLead={f => issue({ kind: 'follow-aircraft', id: planesOf(f).find(p => airborne(p))?.id ?? '' })} canFollow={f => planesOf(f).some(p => airborne(p))}
      onCentre={f => chart.centerAirMapOn(f.position[0], f.position[2])}
      onService={service}
      onDeckPolicy={(id, policy) => { if (issue({ kind: 'deck-policy', carrierId: id, policy })) setFeedback(`${nameFor(id)} · Deck policy order queued`); }}
      onPrioritize={(id, request) => { if (issue({ kind: 'deck-priority', carrierId: id, requestId: request })) setFeedback(`${nameFor(id)} · Next deck task requested`); }}
      onCancelTask={(id, request) => { if (issue({ kind: 'deck-cancel', carrierId: id, requestId: request })) setFeedback(`${nameFor(id)} · Deck cancellation queued`); }}
      deckOpen={deckOpen} onToggleDeck={setDeckOpen}
