...
test('the sortie board answers the same four questions for every mode and marks the last one played', () => {
  ...
  expect(html.match(/<dt>You<\/dt>/g)).toHaveLength(4); expect(html.match(/<dt>Needs<\/dt>/g)).toHaveLength(4);
  ...
  expect(html.match(/aria-checked="false" tabindex="-1"/g)).toHaveLength(3);
});
