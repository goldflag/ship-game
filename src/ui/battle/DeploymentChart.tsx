...
interface Gesture {
  ...
  pointer: number;
  inverse: DOMMatrix;
  start: DeploymentPoint;
  client: [number, number];
  center: DeploymentPoint;
  units: ChartUnit[];
  ids: string[];
  moved: boolean;
}
interface Props {
  ...
  fit: number;
  onChange(units: ChartUnit[]): void;
  onCommit(before: ChartUnit[]): void;
  ...
  hover?: ChartSelection;
  onHover?(hover: ChartSelection): void;
  disabled?: boolean;
}
...
export function DeploymentChart({
  ...
  const [zoom, setZoom] = useState(1),
    [center, setCenter] = useState(deployment.focus);
  ...
  const focusX = deployment.focus.x,
    focusZ = deployment.focus.z;
  useEffect(() => {
    setZoom(1);
    setCenter({ x: focusX, z: focusZ });
  }, [fit, focusX, focusZ]);
  useEffect(() => {
  ...
  const span = fit / zoom,
    wide = size.w >= size.h;
  const halfW = wide ? span * (size.w / size.h) : span,
    halfH = wide ? span : span * (size.h / size.w);
  ...
  const units = deployment.units,
    active = selectedUnits(deployment, selection);
  ...
  const startGesture = (event: ReactPointerEvent<SVGElement>, kind: Gesture['kind'], ids: string[] = []) => {
    ...
    const inverse = svg.current?.getScreenCTM()?.inverse(),
      start = worldPoint(event.clientX, event.clientY, inverse);
    ...
    gesture.current = {
      ...
      center: kind === 'pan' ? center : unitsCenter(moving),
      units,
      ids,
      moved: false,
    };
  ...
  const pressGroup = (event: ReactPointerEvent<SVGElement>, groupId: string) => {
    ...
    startGesture(
      event,
      'move',
      units.filter((unit) => unit.groupId === groupId).map((unit) => unit.id),
    );
  };
  const pressRotate = (event: ReactPointerEvent<SVGElement>) => {
    event.stopPropagation();
    startGesture(
      event,
      'rotate',
  ...
  const keyDown = (event: KeyboardEvent<SVGSVGElement>) => {
    ...
    if (key === 'g' || key === 's') {
      event.preventDefault();
      event.stopPropagation();
      onScopeChange(key === 'g' ? 'group' : 'ship');
      return;
    }
  ...
  for (let n = Math.floor(Math.max(view.z, -radius) / gridStep) * gridStep; n <= Math.min(view.z + view.h, radius); n += gridStep)
    gridRows.push(n);
  const groups = deployment.groups
    .map((group) => ({ ...group, units: units.filter((unit) => unit.groupId === group.id) }))
  ...
  return (
    <svg
      ref={svg}
      className={`deploy-chart ${dragging ? 'is-dragging' : ''} ${dropActive ? 'is-drop-target' : ''}`}
      ...
      onPointerMove={(event) => {
        ...
        current.moved = true;
        if (current.kind === 'pan')
          setCenter({ x: current.center.x + current.start.x - point.x, z: current.center.z + current.start.z - point.z });
      ...
      onDragOver={(event) => {
        if (disabled || !event.dataTransfer.types.includes(DEPLOYMENT_DRAG)) return;
        event.preventDefault();
        event.stopPropagation();
    ...
    >
      <defs>
        {bounds.kind === 'circle' && (
          <clipPath id={clip}>
            <circle r={bounds.radius} />
          </clipPath>
        )}
      </defs>
      <g clipPath={bounds.kind === 'circle' ? `url(#${clip})` : undefined}>
        {bounds.kind === 'circle' ? (
          <circle r={bounds.radius} className="chart-water" />
        ) : (
          <rect x={view.x} y={view.z} width={view.w} height={view.h} className="chart-water" />
        )}
        {deployment.friendlyMinZ !== undefined && (
          <rect x={-radius} y={deployment.friendlyMinZ} width={radius * 2} height={radius * 2} className="chart-sector" />
        )}
        <path
        ...
        {deployment.labels.south && (
          <text x="0" y={radius * 0.86} className="chart-label" fontSize={px(11)}>
            {deployment.labels.south}
          </text>
        )}
      </g>
      ...
      {ring && focus && (
        <g className="chart-ring-group" transform={`translate(${focus.x} ${focus.z})`}>
          <circle
            className="chart-ring-hit"
            r={ring.r}
            style={{ strokeWidth: px(28) }}
          ...
          {Array.from({ length: 24 }, (_, i) => i * 15).map((angle) => {
            ...
            return (
              <line
                key={angle}
                className={`chart-tick ${major ? 'major' : ''}`}
                x1={Math.sin(rad) * ring.r}
                y1={-Math.cos(rad) * ring.r}
                x2={Math.sin(rad) * (ring.r + length)}
                y2={-Math.cos(rad) * (ring.r + length)}
              />
            );
          })}
          {(
          ...
          <g
            className="chart-knob"
            transform={`rotate(${headingDegrees(ring.heading)})`}
            role="button"
            tabIndex={disabled ? -1 : 0}
            aria-disabled={disabled}
            aria-label="Rotate the selection. Drag the heading knob, or press Q or E."
            onPointerDown={pressRotate}
            onClick={(event) => event.stopPropagation()}
          >
          ...
          <g
            ...
            transform={`translate(${Math.sin(ring.heading) * (ring.r + px(42))} ${-Math.cos(ring.heading) * (ring.r + px(42))})`}
            role="button"
            tabIndex={disabled ? -1 : 0}
            aria-disabled={disabled}
      ...
      {groups.map((group) => {
        const box = unitsBox(group.units),
          pad = px(26),
          x = box.x0 - pad,
          z = box.z0 - pad,
          w = box.x1 - box.x0 + pad * 2,
          h = box.z1 - box.z0 + pad * 2;
        const selected = selection?.kind === 'group' && selection.id === group.id,
          hovered = hover?.kind === 'group' && hover.id === group.id;
        ...
        const tagWidth = px(label.length * 6.4 + 16),
          tagHeight = px(18);
        return (
          <g key={group.id} className={`chart-group ${group.side} ${selected ? 'is-selected' : ''} ${hovered ? 'is-hovered' : ''}`}>
            <rect
              ...
              onPointerDown={(event) => pressGroup(event, group.id)}
              onClick={(event) => event.stopPropagation()}
              {...groupHover}
              onKeyDown={(event) => {
                if (event.key === 'Enter' || event.key === ' ') {
                  event.preventDefault();
                  event.stopPropagation();
                  if (!disabled) onSelect({ kind: 'group', id: group.id });
                }
              }}
            />
            <g
            ...
            >
              ...
              <text x={x + px(8)} y={z - tagHeight - px(4) + px(13)} fontSize={px(10)}>
                {label}
              </text>
            </g>
          </g>
      ...
      {units.map((unit) => {
        number++;
        const selected = active.some((other) => other.id === unit.id),
          own = selection?.kind === 'ship' && selection.id === unit.id;
        const hovered = hover?.kind === 'ship' ? hover.id === unit.id : hover?.kind === 'group' && hover.id === unit.groupId;
        const shipClass = shipClassOf(resolveShip(unit.presetId)),
          glyph = SHIP_GLYPHS[shipClass];
        return (
          <g
            key={unit.id}
            transform={`translate(${unit.spawn.x} ${unit.spawn.z})`}
            className={`chart-ship ${unit.side} ${selected ? 'is-selected' : ''} ${own ? 'is-focus' : ''} ${hovered ? 'is-hovered' : ''}`}
            role="button"
            tabIndex={disabled ? -1 : 0}
            aria-label={`Select ${unit.name}`}
            aria-pressed={own}
            onPointerDown={(event) => pressShip(event, unit)}
            onClick={(event) => event.stopPropagation()}
            onPointerEnter={() => onHover?.({ kind: 'ship', id: unit.id })}
            onPointerLeave={() => onHover?.(undefined)}
            onKeyDown={(event) => {
              if (event.key === 'Enter' || event.key === ' ') {
                event.preventDefault();
                event.stopPropagation();
                if (!disabled) onSelect(scope === 'ship' ? { kind: 'ship', id: unit.id } : { kind: 'group', id: unit.groupId });
              }
            }}
          >
            ...
            <g transform={`rotate(${headingDegrees(unit.spawn.heading)}) scale(${px(0.95)})`}>
              ...
              <path className="chart-mark" d={glyph.mark} />
            </g>
            {zoom >= SHIP_NUMBER_ZOOM && (
              <text x={px(14)} y={px(4)} fontSize={px(10)}>
                {number}
              </text>
            )}
          </g>
