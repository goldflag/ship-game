...
const Turn = ({ direction }: { direction: 'port' | 'starboard' }) => (
  <svg
    width="14"
    height="14"
    viewBox="0 0 24 24"
    ...
    aria-hidden="true"
    style={direction === 'starboard' ? { transform: 'scaleX(-1)' } : undefined}
  >
    <path d="M9 4 5 8l4 4M5 8h9a5 5 0 0 1 0 10h-3" />
  </svg>
);
const UndoIcon = () => (
  <svg
      ...
      {Array.from({ length: 12 }, (_, i) => i * 30).map((angle) => {
        ...
        return (
          <line
            ...
            x1={c + Math.sin(rad) * r}
            y1={c - Math.cos(rad) * r}
            x2={c + Math.sin(rad) * (r + (major ? 6 : 3))}
            y2={c - Math.cos(rad) * (r + (major ? 6 : 3))}
          />
        );
      })}
      <g transform={`rotate(${headingDegrees(heading)} ${c} ${c})`}>
        <path d={`M${c} ${c - r + 2} L${c + 5} ${c - r + 12} L${c} ${c - r + 9} L${c - 5} ${c - r + 12}Z`} />
        <line x1={c} y1={c} x2={c} y2={c - r + 10} />
      </g>
...
export function DeployScreen({ deployment, onChange, onReset, disabled, tools, fitKey, onFormation }: Props) {
  ...
  useEffect(() => {
    if (selection && !active.length) setSelection(firstGroup ? { kind: 'group', id: firstGroup.id } : undefined);
  }, [selection, active.length, firstGroup]);
  const commit = (before: ChartUnit[]) => setHistory((list) => [...list.slice(-29), before]);
  const change = (next: ChartUnit[]) => {
    if (!disabled) {
      commit(units);
      onChange(next);
    }
  ...
  const rosterDrag = (event: DragEvent<HTMLElement>, ids: string[], next: ChartSelection) => {
    ...
    event.dataTransfer.setData(DEPLOYMENT_DRAG, JSON.stringify(ids));
  };
  const scopeChange = (next: ChartScope) => {
    setScope(next);
    if (next === 'group' && selection?.kind === 'ship')
      setSelection({ kind: 'group', id: units.find((unit) => unit.id === selection.id)?.groupId ?? selection.id });
    if (next === 'ship' && selection?.kind === 'group') {
      const first = units.find((unit) => unit.groupId === selection.id);
      if (first) setSelection({ kind: 'ship', id: first.id });
    }
  };
  let number = 0;
  return (
    <div className="deploy-body">
      <aside className="deploy-side" aria-label="Deployment">
        ...
        <div className="deploy-scope" role="group" aria-label="Selection scope">
          <div className="battle-segmented">
            <button type="button" aria-pressed={scope === 'group'} onClick={() => scopeChange('group')}>
              Group <kbd>G</kbd>
            </button>
            <button type="button" aria-pressed={scope === 'ship'} onClick={() => scopeChange('ship')}>
              Ship <kbd>S</kbd>
            </button>
          </div>
          <p>
            {scope === 'group'
              ? 'Selecting a ship selects its whole group. Drag a frame or its tag to move the group.'
              : 'Selecting a ship selects only that ship. Drag it to move it alone.'}{' '}
            Drag the ring or the heading knob to rotate.
          </p>
        </div>
        <div className="deploy-list">
          {deployment.groups.map((group) => {
            const members = units.filter((unit) => unit.groupId === group.id),
              ids = members.map((unit) => unit.id);
            if (!members.length) return null;
            const groupSelected = selection?.kind === 'group' && selection.id === group.id,
              groupHovered = hover?.kind === 'group' && hover.id === group.id;
            return (
              <div key={group.id} className={`deploy-group ${group.side}`}>
                <button
                  type="button"
                  className={`deploy-group-row ${groupHovered ? 'is-hovered' : ''}`}
                  draggable={!disabled}
                  disabled={disabled}
                  aria-pressed={groupSelected}
                  onDragStart={(event) => rosterDrag(event, ids, { kind: 'group', id: group.id })}
                  onClick={() => setSelection({ kind: 'group', id: group.id })}
                  onPointerEnter={() => setHover({ kind: 'group', id: group.id })}
                ...
                <ul>
                  {members.map((unit) => {
                    number++;
                    const own = selection?.kind === 'ship' && selection.id === unit.id,
                      hovered = hover?.kind === 'ship' && hover.id === unit.id;
                    return (
                      <li key={unit.id}>
                        <button
                          type="button"
                          className={`deploy-ship-row ${hovered ? 'is-hovered' : ''}`}
                          draggable={!disabled}
                          disabled={disabled}
                          aria-pressed={own}
                          onDragStart={(event) => rosterDrag(event, [unit.id], { kind: 'ship', id: unit.id })}
                          onClick={() => setSelection({ kind: 'ship', id: unit.id })}
                          onPointerEnter={() => setHover({ kind: 'ship', id: unit.id })}
                          onPointerLeave={() => setHover(undefined)}
                        >
                          <span className="deploy-ship-number">{number}</span>
                          <ShipThumbnail presetId={unit.presetId} width={56} />
                          <span className="deploy-ship-name">{unit.name}</span>
                          <NationFlag nation={shipIdentity(unit.presetId).nation} />
                        </button>
                      </li>
                    );
                  })}
        ...
        <div className="deploy-tools">
          {onFormation && (
            <div className="deploy-formation">
              <label className="rail-field">
                <span>Formation{selectedGroup ? ` · ${selectedGroup.name}` : ''}</span>
                <Select
                  aria-label="Cruising formation"
                  value={formation}
                  disabled={disabled || !selectedGroup}
                  onValueChange={(value) => chooseFormation(value as Formation)}
                >
          ...
          <div className="deploy-heading">
            ...
            <div className="rail-field">
              ...
              <div className="rail-row">
                ...
                <Button
                  disabled={disabled || !active.length}
                  aria-label="Rotate 15 degrees to port"
                  title="Rotate 15° to port (Q)"
                  onClick={() => turn(-Math.PI / 12)}
                >
                  <Turn direction="port" />
                  15°
                </Button>
                <Button
                  disabled={disabled || !active.length}
                  aria-label="Rotate 15 degrees to starboard"
                  title="Rotate 15° to starboard (E)"
                  onClick={() => turn(Math.PI / 12)}
                >
                  <Turn direction="starboard" />
                  15°
                </Button>
              </div>
            </div>
      ...
      <div className="deploy-chart-wrap">
        <DeploymentChart
          ...
          disabled={disabled}
        />
        <div className="deploy-legend" aria-hidden="true">
          <span className="selected">Selected</span>
          <span className="friendly">Friendly</span>
          {deployment.groups.some((group) => group.side === 'enemy') && <span className="enemy">Enemy</span>}
