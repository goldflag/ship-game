...
export const nextGroupName = (request: PveRequest) =>
  `Group ${Math.max(0, ...request.groups.map((group) => Number(group.name.match(/^Group (\d+)$/)?.[1]) || 0)) + 1}`;
interface LanesProps {
  request: PveRequest;
  options: PveOptions;
  onChange(request: PveRequest): void;
...
export function PveLanes({ request, options, onChange, transfer, onTransfer, onError, nextId, disabled, rule = openFleet }: LanesProps) {
  ...
  return (
    <>
      <BudgetStrip
        label="Fleet allowances"
        items={[
          {
            label: 'Tonnage',
            reading: `${tonnes(totals.displacementKg)} / ${tonnes(budget.maxDisplacementKg)} t`,
            used: totals.displacementKg,
            max: budget.maxDisplacementKg,
          },
      ...
      <div className="battle-lanes" aria-label="Task groups">
        {request.groups.map((group) => {
          ...
          return (
            <FleetLane
              ...
              active={active}
              disabled={disabled}
              onPlace={() => place(group.id)}
              title={
                <Input
                  className="fleet-lane-name"
                  aria-label={`Task group name: ${group.name}`}
                  maxLength={32}
                  value={group.name}
                  disabled={disabled}
                  onChange={(event) =>
                    onChange({
                      ...request,
                      groups: request.groups.map((g) => (g.id === group.id ? { ...g, name: event.target.value } : g)),
                    })
                  }
                />
              }
              count={
                <>
                  {members.length} {members.length === 1 ? 'ship' : 'ships'} · {tonnes(fleetTotals(members).displacementKg)} t
                </>
              }
              extra={
                <Select
                  className="fleet-lane-station"
                  aria-label={`Station for ${group.name}`}
                  title="Front groups deploy ahead; rear groups deploy behind"
                  value={group.station}
                  disabled={disabled}
                  onValueChange={(value) =>
                    onChange({
                      ...request,
                      groups: request.groups.map((g) => (g.id === group.id ? { ...g, station: value as GroupStation } : g)),
                    })
                  }
                >
                  ...
                  <>
                    ...
                    {request.groups.length > 1 && (
                      <>
                        {' '}
                        <Button
                          className="fleet-lane-action"
                          disabled={disabled}
                          onClick={() => onChange({ ...request, groups: request.groups.filter((g) => g.id !== group.id) })}
                        >
                          Remove empty group
                        </Button>
                      </>
                ...
                {members.map((unit) => {
                  ...
                  return (
                    <ShipChip
                      ...
                      disabled={disabled}
                      draggable
                      onDragStart={(event) => dragUnit(event, unit.id)}
                      onDragEnd={() => onTransfer(undefined)}
                      pickLabel={`Move ${name} to another group`}
                      onPick={() => onTransfer(picked ? undefined : { kind: 'unit', id: unit.id })}
                      removeLabel={`Remove ${name}`}
                      onRemove={() => onChange({ ...request, ships: request.ships.filter((ship) => ship.id !== unit.id) })}
                    />
                  );
                })}
              </ul>
        ...
        {request.groups.length < MAX_TASK_GROUPS && (
          <FleetLane
            label="New task group"
            tone="ghost"
            title={
              <>
                <Icon name="plus" size={14} /> Group
              </>
            }
            active={active}
            disabled={disabled}
            onPlace={newGroup}
            hint={<span className="fleet-lane-vertical">Drop a ship to start {nextGroupName(request)}</span>}
          />
        )}
      </div>
    </>
...
export const pveInvalid = (request: PveRequest, options?: PveOptions) =>
  options
    ? budgetError(request.ships, options.rules.budget) ||
      (request.groups.some((group) => !group.name.trim()) ? 'Name every task group.' : '')
    : '';
    ...
    <>
      <RailBlock title="Waters" aside={map?.region}>
        <MapTiles
          ...
          value={request.mapId}
          disabled={disabled}
          onChange={(mapId: OceanMapId) => onChange({ ...request, mapId })}
        />
      </RailBlock>
      <RailBlock title="Conditions">
        <label className="rail-field">
          ...
          <Select
            ...
            disabled={disabled}
            onValueChange={(weather) => onChange({ ...request, weather })}
          >
        ...
        <div className="rail-field">
          <span id="pve-difficulty-label">Difficulty</span>
          <div className="battle-segmented" role="group" aria-labelledby="pve-difficulty-label">
            {(['easy', 'normal', 'hard'] as const).map((level) => (
              <button
                type="button"
                key={level}
                aria-pressed={request.difficulty === level}
                disabled={disabled}
                onClick={() => onChange({ ...request, difficulty: level })}
              >
      ...
      <RailBlock title="Mission seed">
        <div className="rail-row">
          <output className="rail-seed" aria-label="Mission seed">
            {request.seed.toString(16).toUpperCase().padStart(8, '0')}
          </output>
          <Button disabled={disabled} onClick={() => onChange({ ...request, seed: missionSeed() })}>
            New opponent
          </Button>
        </div>
...
export function pveBrief(request: PveRequest): string[] {
  const map = OCEAN_MAPS.find((entry) => entry.id === request.mapId),
    weather = WEATHER_PRESETS.find((entry) => entry.id === request.weather),
    totals = fleetTotals(request.ships);
  return [
    map?.name ?? request.mapId,
    `${weather?.name ?? request.weather} · ${request.difficulty[0].toUpperCase()}${request.difficulty.slice(1)}`,
    `${totals.ships} ${totals.ships === 1 ? 'ship' : 'ships'} in ${request.groups.length} ${request.groups.length === 1 ? 'group' : 'groups'} · ${tonnes(totals.displacementKg)} t`,
  ];
}
