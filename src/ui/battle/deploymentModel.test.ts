...
const setup: BattleSetup = {
  playerShipId: 'bismarck',
  friendlyBots: ['fletcher'],
  enemies: ['yamato', { shipId: 'fubuki', aiLevel: 'easy' }],
  spawnDistance: 6000,
...
test('PvE deployment mirrors placements and reports sector violations', () => {
  const briefing = {
    ...
    groups: [
      { id: 'front', name: 'Screen', station: 'front' },
      { id: 'rear', name: 'Carriers', station: 'rear' },
      { id: 'empty', name: 'Empty', station: 'rear' },
    ],
    assignments: [
      { id: 'dd', presetId: 'fletcher', groupId: 'front' },
      { id: 'cv', presetId: 'enterprise-cv6', groupId: 'rear' },
    ],
    totals: { displacementKg: 1, ships: 2, aircraft: 48 },
    eligiblePresets: ['fletcher', 'enterprise-cv6'],
    deploymentMinZ: 7000,
  } as PveBriefing;
...
test('a formation preset stations a group around its guide and leaves every other group alone', () => {
  ...
  const units = [
    unit('bb', 'bismarck', 'front', 1000, 12000, Math.PI / 2),
    unit('dd', 'fletcher', 'front', -4000, 19000),
    unit('ca', 'baltimore', 'front', 6000, 3000),
    unit('cv', 'enterprise-cv6', 'rear', 0, 16000),
  ];
  ...
  expect(column[2].spawn.x).toBeCloseTo(1000 - d);
  expect(column[2].spawn.z).toBeCloseTo(12000);
  expect(column[1].spawn.x).toBeCloseTo(1000 - d * 2);
  expect(column[1].spawn.heading).toBeCloseTo(Math.PI / 2);
  ...
  const doubledCa = doubled.find((ship) => ship.id === 'ca')!,
    doubledDd = doubled.find((ship) => ship.id === 'dd')!;
  expect(doubledCa.spawn.x).toBeCloseTo(1000);
  expect(doubledCa.spawn.z).toBeCloseTo(12000 + d);
  expect(doubledDd.spawn.x).toBeCloseTo(1000 - d);
  expect(doubledDd.spawn.z).toBeCloseTo(12000);
...
test('the first chart puts each group on its formation stations around the centre the worker chose', () => {
  ...
  const briefing = {
    ...
    setup: {
      ships: [
        ship('bb', 'bismarck', -400, 8000),
        ship('ca', 'baltimore', 400, 8000),
        ship('dd', 'fletcher', -400, 8700),
        ship('cv', 'enterprise-cv6', -400, 16500),
    ...
    groups: [
      { id: 'front', name: 'Screen', station: 'front' },
      { id: 'rear', name: 'Carriers', station: 'rear', formation: 'line-abreast' },
      { id: 'solo', name: 'Picket', station: 'front' },
    ],
    assignments: [
      { id: 'bb', presetId: 'bismarck', groupId: 'front' },
      { id: 'ca', presetId: 'baltimore', groupId: 'front' },
      { id: 'dd', presetId: 'fletcher', groupId: 'front' },
      { id: 'cv', presetId: 'enterprise-cv6', groupId: 'rear' },
      { id: 'cl', presetId: 'cleveland', groupId: 'rear' },
      { id: 'lone', presetId: 'fletcher', groupId: 'solo' },
    ],
    totals: { displacementKg: 1, ships: 6, aircraft: 48 },
    eligiblePresets: [],
    deploymentMinZ: 7000,
  } as unknown as PveBriefing;
  ...
  const d = roleInterval('battleship');
  expect(at('ca').x).toBeCloseTo(at('bb').x);
  expect(at('dd').x).toBeCloseTo(at('bb').x);
  expect(at('ca').z - at('bb').z).toBeCloseTo(d);
  expect(at('dd').z - at('ca').z).toBeCloseTo(d);
  ...
  const edge = {
    ...briefing,
    setup: { ...briefing.setup, ships: [ship('bb', 'bismarck', -400, edgeZ), ship('ca', 'baltimore', 400, edgeZ)] },
    assignments: briefing.assignments.slice(0, 2),
  } as PveBriefing;
  expect(
    pveDeployment(
      edge,
  ...
  const coast = {
    ...
    setup: {
      ...briefing.setup,
      ships: [
        ship('bb', 'bismarck', -400, south),
        ship('ca', 'baltimore', 400, south),
        ship('dd', 'fletcher', -400, south + 700),
        ship('cl', 'cleveland', 400, south + 700),
      ],
    },
    assignments: [
      { id: 'bb', presetId: 'bismarck', groupId: 'front' },
      { id: 'ca', presetId: 'baltimore', groupId: 'front' },
      { id: 'dd', presetId: 'fletcher', groupId: 'front' },
      { id: 'cl', presetId: 'cleveland', groupId: 'front' },
    ],
  } as PveBriefing;
  expect(
    pveDeployment(
