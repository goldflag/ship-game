...
export const mission = (f: FlightSummary) => f.order.kind === 'search-area' ? `${f.order.policy === 'strike' ? 'Search and strike' : f.order.policy === 'shadow' ? 'Shadow and report' : 'Search and report'} · ${f.order.radiusM / 1000} km radius · ${f.order.altitude} altitude`
