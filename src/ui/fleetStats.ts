...
export interface AirCluster { id: string; type: ObservedAircraftType; label: string; count: number; position: Vec3; heading: number; trackIds: string[]; smoking: number; lastObservedTick: number; stale: boolean }
...
export function airClusters(tracks: readonly ContactTrack[], tick: number, radiusM = 900): AirCluster[] {
  const aircraft = tracks.filter(t => t.kind === 'aircraft').map(t => ({ track: t, position: reportPosition(t, tick), type: observedAircraftType(t.classification) }));
  ...
  return clusters.map(({ members }) => {
    ...
    return { id: `air-${members.map(m => m.track.id).sort().join('+')}`, type: lead.type, label: aircraftTypeLabel(lead.type, n), count: n, position, heading: Math.atan2(vx, -vz),
