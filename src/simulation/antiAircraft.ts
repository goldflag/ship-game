...
export function updateAntiAircraft(actor: FleetActor, m: MountDefinition, state: MountState, ctx: AirContext, dt: number, candidates?: readonly Aircraft[]): boolean {
  ...
  const discipline = state.aaDiscipline ??= initialFireDiscipline();
  ...
  const aligned = updateMount(m, state, actor.definition, actor.motion, crewAim, dt, velocity);
  ...
  // Use the trajectory solved for the lead point. The distance to the plane's
  // current position gives a different time and can make inbound targets unhittable.
  const burstTime = state.aimCache?.time ?? flightTime;
  const shots = [];
  for (let barrel = 0; barrel < barrels; barrel++) {
    const position = muzzleWorld(m, state, barrel, actor.motion);
    const direction = dispersedDirection(shotDirection(m, state, actor.motion), AIR_GUNNERY.aaSpread(closest), ctx.seed ?? 0, ctx.nextId());
