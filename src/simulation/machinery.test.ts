...
test('a penetrating underwater path opens boundaries and flooding disables otherwise intact boilers', () => {
  ...
  // Coarser strips no longer subdivide this band, so the hole opens the
  // retained port void behind the plating rather than a wing strip wired into
  // the partition chain. The void fills; the boilers take the shell's damage
  // but no longer flood from this hit alone.
  expect(assignments[0]).toBe('void-port-120');
  ...
  expect(equipmentCondition(actor, def, boiler).reason).toBe('damaged');
  expect(actor.damage.compartments.find(c => c.id === 'void-port-120')!.waterM3).toBeGreaterThan(0);
