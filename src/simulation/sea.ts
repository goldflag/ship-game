...
export function createSeaState(mapId: OceanMapId, weather: WeatherId | undefined, seed: number): SeaState {
  const map = oceanMap(mapId), waves = battleEnvironment(map, 'map', weather ?? 'clear').waves;
  return { amplitudeM: weather === undefined ? 0 : waves.amplitude * 4,
    wavelengthM: waves.peakWavelength * 4, direction: map.water.windDirection * Math.PI / 180,
    windMps: weather === undefined ? 0 : waves.windSpeed, phase: seed / 0xffffffff * Math.PI * 2 };
}
