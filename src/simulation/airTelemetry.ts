...
export const AIR_STATUS_LABELS: Record<AirStatus, string> = { withdrawn: 'Unavailable', ready: 'Ready', launching: 'Launching', 'on-mission': 'On mission', returning: 'Returning', servicing: 'Servicing', lost: 'Lost', hangar: 'In hangar', handling: 'Handling' };
...
export function airWingTelemetry(actor: FleetActor, actors: FleetActor[]) {
  ...
  const endurance: EndurancePolicy = state.operatingRules?.endurance ?? state.deck?.endurance ?? { ...legacyAir.endurance, kind: 'timed' };
  ...
  const counts: Record<AirStatus, number> = { withdrawn: 0, ready: 0, launching: 0, 'on-mission': 0, returning: 0, servicing: 0, lost: 0, hangar: 0, handling: 0 };
  ...
  const groups: FlightSummary[] = squadronFlights(actor).flatMap(f => {
    ...
    const surviving = planes.filter(p => !terminalAircraft(p));
    ...
    const status = !surviving.length ? planes.some(p => p.phase === 'withdrawn') ? 'withdrawn' : 'lost' : surviving.some(p => airStatus(p) === 'launching') ? 'launching'
    ...
    const search = f.order.kind === 'search-area' ? lead.search : undefined;
    ...
    const destination: Vec3 = status === 'returning' || f.order.kind === 'return' ? home : target ? [target.motion.x, 0, target.motion.z]
      : f.order.kind === 'search-area' ? lead.navigationTarget ?? [f.order.center[0], 0, f.order.center[1]] : reportMission ? lead.navigationTarget ?? position : f.order.kind === 'patrol' ? [...f.order.point] : escort ? actors.flatMap(a => a.airWing?.planes ?? []).find(p => p.flightId === escort.id && airborne(p))?.position ?? home : home;
  ...
  return {
    ...
    flights: state.planes.map(p => ({ id: p.id, flightId: p.flightId, modelId: p.modelId, role: p.role, phase: p.phase, status: airStatus(p),
      hp: p.hp, payload: p.payload, ammo: p.ammo, location: p.phase === 'withdrawn' ? 'Unavailable' : p.phase === 'lost' ? 'Lost' : onFlightDeck(p) ? 'Deck' : airborne(p) ? 'Airborne' : 'Hangar',
      followable: !terminalAircraft(p) && (onFlightDeck(p) || airborne(p)), lossReason: p.lossReason,
