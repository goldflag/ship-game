...
| `--freeze <s>`, `--history <s>` | Hold the berth's ride, the waves, clouds, funnel smoke and the battle at that sea time, after replaying `--history` seconds (default 30) of foam, wake and smoke with every hull held where it stands. The sky stands at that time too, and its clouds stop marching once converged on the view (`SkyApi.hold`). Two port runs frozen at the same time render the same frame (99.5% of pixels identical, the rest one level apart): master against a branch differ only where the code does. In a battle the hulls stay where the simulation had them when it froze, which differs run to run |
...
| `freezeScene({ time, history })` | `Game.freezeScene`: hold every presentation clock at `time` seconds of sea after a `history`-second replay, then wait while the held sky converges; `false` lets them run. Place the camera first, since smoke detail follows it. Underway, the replay fades the wakes astern; `history: 0` keeps them and whatever foam the sea had |
