...
### Equipment authoring scope

Run `bun assets/ships/author-local-damage.ts <ship-id> [...]` for explicit local-profile regeneration, or `all` for the runtime roster. No arguments write nothing. Authored directors/generators suppress helper-owned proxies. `bun assets/ships/author-equipment.ts <ship-id> [...] | all` updates the supported equipment bindings and inventories without replacing existing compartment fire loads or local calibration. Both scripts validate blueprint/catalog data before publication through the standard ship build. Shared barrel/compiler changes invalidate the fleet: run `ship:check all` and rebuild every stale output.
