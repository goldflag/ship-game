...
Reference research is part of authoring, not a build dependency: `ship:reference`, `ship:slice` and `ship:hardpoints` ([reference workflow](reference-workflow.md)) read and write only `.build/`. Do not recreate ship `reports/` or `references/` directories.
...
Use Blender MCP or background Blender renders for the authoring loop below. Batch generation, export, thumbnails and fixed review renders continue through the existing pipeline commands: `BLENDER_BIN` overrides the executable, with the standard macOS application or `blender` on PATH as defaults. Those commands start isolated Blender processes and do not require MCP or use the open interactive scene.

### Blender MCP authoring loop

For a new ship, complete the [brief and reference approval](ship-pipeline.md#start-a-new-ship-collaboratively) before ship-specific geometry or paint authoring. The loop is required for ship geometry and articulation work. Blender MCP and background Blender with inspected renders are equal ways to run it: use MCP when it is running and your task owns the interactive scene, and background renders otherwise. One GUI Blender cannot serve several agents, so agents working on one ship in parallel always use background renders. Documentation-only, simulation-only and unchanged-asset checks do not require an interactive Blender session.
...
Without MCP, run the same edit–inspect–correct loop with local background Blender and renders you actually inspect; do not claim Blender was inspected through MCP. Summarize the actual tools used, inspected views/assemblies, remaining limitations and rebuild verification in the task response or PR. No additional tracked report is required.
