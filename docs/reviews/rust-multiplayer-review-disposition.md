...
Full Linux validation passed on `6402f452` ([run](https://github.com/goldflag/ship-game/actions/runs/34202887964)): native/WASM parity, the full TypeScript suite, real asset checks/production build, and release-server HTTP/WebSocket smoke. Its duplicate PR run exceeded one pre-existing 20-second bot-test timeout: 24.6 seconds versus 17.5 seconds on the passing runner. That 90-second simulated correctness test now allows 60 wall-clock seconds, retaining all assertions and a finite deadline.
