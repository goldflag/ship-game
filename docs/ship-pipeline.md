...
Collision coverage is separate from the armor schedule. All four historical presets enable `structuralPlating` with explicitly provisional ordinary-steel thicknesses: hull/structure 20/8 mm for Bismarck, 25/8 mm for Yamato, 16/6 mm for Baltimore and 15.875/6 mm for Enterprise. The CPU derives each complete hull shell from `hull.sections` and deckhouse shells from `structures[].footprint`; optional original `surface` vertices/triangles describe tapered towers, open funnel jackets and Enterprise's cambered deck/elevator slabs. The visual recipes consume those same surfaces. These cached, spatially bounded surfaces participate in swept hits, sight aiming and Armor inspection. AP can pass through thin structure while causing damage; only hull-shell penetrations can create sea breaches. Nearby exterior armor replaces nominal shell plating instead of charging both at the same location. Combat and the opaque inspection skin share a 1.5 m replacement allowance for reconstructed exterior plate offsets; inspection removes covered skin patches so belt colors and hover picks remain visible. The waterline splash check uses the hull interior, not armor inspection bounds. Broad end sections receive complete caps; zero-breadth air above a recurved bulb receives no fictitious centerline steel. Legacy definitions without this opt-in retain their previous behavior.

Bismarck uses spatially separated belt/backing/support, upper belt, decks, turtleback, bulkheads, barbettes and gunhouse plates. Yamato, Baltimore and Enterprise now have their own qualified fixed protection; Bismarck, Yamato and Baltimore's closed gunhouses use original per-facet protection. CPU collisions, sight picking, probes and inspection all use the same plate surfaces; mounted plates follow CPU yaw. A plate consumes resistance once per crossing, including coplanar seams and tick boundaries. Teak contributes no invented steel-equivalent resistance. KC, Wh, Ww and structural steel are identified but do not yet have distinct empirical penetration curves. Ricochet, AP budgets, module health, magazine events and flooding remain game approximations; AP fuzes, HE blast/spall, crew and mesh fracture are absent. The source/discrepancy registers distinguish corroborated thickness families from estimated boundaries and incomplete schedules.
