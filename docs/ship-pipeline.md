...
The impact ledger (`impact`) and renderer attachment evidence (`surfaceImpact`) are separate serializable fields. Surface positions, normals and incoming directions use ship-local coordinates or the struck gunhouse's local frame. Shell effects preserve ammunition type and incoming velocity even when a plate stops the projectile. A projectile spends one kinetic equipment-damage budget across its path; exhausting that budget does not remove its remaining speed or cancel an armed fuze.
