...
The sea used for gameplay is a flat Y=0 surface. GPU waves render the sea and animate buoys, while combat ship poses remain authoritative CPU values so muzzles, damage and visuals agree. A future shared CPU wave model can add synchronized heave and wave-driven roll/pitch. Ship contact/collision response, target AI, networking, and a shipbuilding UI are not implemented here.

Target inspection draws armor, modules, compartments and floodwater as an X-ray overlay, including spaces below the visible sea. Development builds expose the read-only `window.shipTrialDiagnostics()` check for the loaded muzzle positions and recent combat events. The regression test loads the exported joint hierarchy and checks it against simulation poses, including rear-facing turrets and recoil.
