...
**Measuring.** Do not write measurement scripts under `.build/`; every accuracy pass so far rewrote the same ones.

- **Hull lines:** `bun run ship:lines my-ship [--ref <vehicle>]` measures the cached reference hull and writes `authoring/lines.json` (when the ship has none; otherwise `.build/ships/<id>/lines/lines.json`, or the authoring file with `--replace`), and compares the result with the ship's existing lines at their own stations. It follows the starboard shell at each station from its widest point, so bilge keels, bossings and rails are not followed; takes the deck as the lowest plating that reaches the shell; finds deck breaks; and places control stations 0.5 m apart near the ends, 1 m within 22 m, 2 m elsewhere. The frame centres the hull on its length and the output records the shift (`zShift`, reference z + shift). `--format offsets` writes Hood's rows (`[z, keel, deck, half-breadths]` at the `LOW`/`HIGH`/`H0` levels, the format Hood, Hipper, Baltimore and Fubuki read); `--format sections` writes the `{station, points}` sections Fletcher reads and `author-blueprint.py --loft` of a `ship:new --legacy` scaffold takes. It keeps an existing file's format and levels. Check its guesses: `--breaks none` for a flush deck, `--deck-below` where deckhouse sides run flush with the hull, `--span bow,stern` when the ends are a judgement (a counter, a stern walk). It reproduces Hood's lines within 2 mm at 95 % of the levels.
- **Decks and deckhouses:** trace outlines with `ship:slice <ref> --plan <y> --sym --parts hull,misc,other`.
- **Painted detail:** see the windows, doors and markings a GameModels3D reference paints into its textures with `ship:reference <ref> --render` ([reference workflow](reference-workflow.md)), or next to our model with `ship:overlay --textured` (below).
...
- A GameModels3D reference is not centred on our midships (about 2.3 m on Iowa, 2.0 m on Alaska), and its waterline datum can differ from ours (Bismarck's reference sits 0.85 m low). `ship:overlay` fits both: fore and aft on the waterline half-breadths, then fore and aft and up and down on the side and top silhouettes (within 3 cm of the hand-measured values; 0.87 m on Bismarck against 0.85 m by hand). It prints the offset as `offsetFlag` with a hint when the vertical part is not negligible; pass it to any other comparison. `--align fore-aft` restores the fore-and-aft-only fit.
...
- `bun run ship:sweep <id>` poses every mount through its installed traverse, elevation and recoil against the fixed ship and the other mounts at rest, then neighbouring mounts against each other, and replays each contact through the simulation's interlock resolver. A contact the interlocks stop first is covered; one the mount can reach fails, unless `assets/ships/<id>/sweep-accepted.json` accepts it (`{ "accepted": [{ "mount", "against", "reason" }] }`, globs; `against` is the touched part's assembly ID or mesh name, or the other mount). Accepted contacts are listed apart and do not fail the run; an entry that no longer occurs is reported as stale, so remove it. A rotating base touching the structure it stands on (a part that trains but does not elevate, against a mesh met by vertical rays dropped around the pivot from just above the foot of the base to just below it) is a seat contact: counted in one line and listed under `seats` in `sweep-report.json`, never replayed. It is judged per mesh, so on a `--glb` sweep, where the export merges structure into large meshes, a base that also strikes another part of its seat's mesh is counted as seated. Reachable clashes print first. `--mounts` takes shell globs (`main-*`, `'*-aa-*'`, `'casemate-[ps]1'`, `'{main,ha}-?'`) and fails with the list of mount IDs when a pattern matches none. The interlock replay runs on every core (`--jobs N` to limit it) with the same report for any worker count; `--reuse` replays the last `sweep.json` without Blender while the model, definition and options are unchanged, for example after editing the accepted list. A whole ship takes a few minutes.
...
**Review.** The overlay comparison found most proportion errors on Hood and Alaska. `bun run ship:overlay my-ship` renders the published GLB and the ship's suggested reference (cached with `ship:reference` if needed; `--reference` picks another) from the same orthographic camera, ours-only red and reference-only blue, as side, top, front, bridge and aft shots with IoU in `summary.json`, all under `.build/ships/<id>/overlay/`. `--box x0,y0,z0,x1,y1,z1` adds views clipped to a region, and `--camera` takes a `ui:shot` pose (`bowQuarter`, `az,el[,m]`). Two additions replace the per-ship pairing and cut scripts:

- `--textured` also renders the textured reference (in its source paint, `--paint` for another) and our model under one light from each shot's exact camera, into `overlay/textured/<shot>-reference.png`, `-ours.png` and `-pair.png` (stacked for wide views, side by side otherwise). It adds about 25 s.
- `--sections z=-40,z=0,y=6.5,x=0` cuts both models by those planes, in the ship frame after the fitted offset: `z` stations (starboard right), `y` plans (bow right, port up) and `x` profiles (bow right). Each writes `overlay/section-<axis><value>.png`, reference blue and ours red on a metre grid (darker every 5 m, black through zero), and the summary gives the largest and mean half-breadth difference over all drawn parts (`--parts hull` for the hull alone, `--box` to clip).

Add close views of the bridge glazing and of the underwater stern (screws, rudders, shafts) to the fixed views; distant views hide both.
