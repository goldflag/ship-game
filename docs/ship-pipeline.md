...
Collision coverage is separate from the armor schedule. Bismarck enables `structuralPlating` with explicitly provisional ordinary-steel thicknesses. The CPU derives its complete hull shell from `hull.sections` and deckhouse shells from `structures[].footprint`; optional original `surface` vertices/triangles describe the tower taper and open funnel jacket. The visual recipe consumes those same surfaces. These cached, spatially bounded surfaces participate in swept hits, sight aiming and Armor inspection. AP can pass through thin structure while causing damage; only hull-shell penetrations can create sea breaches. Nearby exterior armor replaces nominal shell plating instead of charging both at the same location. Combat and the opaque inspection skin share a 1.5 m replacement allowance for reconstructed exterior plate offsets; inspection removes covered skin patches so belt colors and hover picks remain visible. The waterline splash check uses the hull interior, not armor inspection bounds. Other presets retain their existing collision definitions until structural plating is authored for them.
...
Hull structural HP is derived at simulation initialization from the blueprint’s `hull.massKg`: `300 + 1450 * sqrt(massKg / 70_000_000)`, rounded to the nearest 10 HP. The base allowance and diminishing size bonus keep small hulls viable while retaining greater endurance for larger ships. A hypothetical 2,000-tonne destroyer gets 550 HP and a 3,000-tonne destroyer gets 600 HP. The current authored masses produce Yamato 1,750 HP, Bismarck 1,450 HP, Enterprise 1,180 HP and Baltimore 1,020 HP. This replaces linear displacement scaling, which gave a 2,000-tonne hull only 50 HP. The curve is provisional gameplay calibration, not a historical survivability calculation. Armor resistance, module health and reserve buoyancy keep their own roles; armor thickness does not multiply HP. Added floodwater does not grant HP. Both teams and future custom definitions use the same rule. Damage state stores current and maximum integrity, telemetry normalizes against each actor’s maximum, and HUD numbers and score use absolute HP. Reset restores each hull’s own maximum.

Each shell has one internal module damage budget, initially its catalog damage. Damage spent on one module is unavailable to later modules, including across fixed ticks; exhausting the budget consumes the shell. This is a bounded gameplay impact model, not a physical fuze or energy calculation. Destroying a magazine disables linked mounts, opens a flooding breach and causes one 150 HP explosion. These penalties apply equally to players and bots. The earlier 450 HP explosion plus repeated full damage along a shell's path allowed Bismarck's opening salvo to sink Yamato at the default deployment before the player could react.
