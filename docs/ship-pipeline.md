...
The optional versioned stability profile declares dry CG, a uniform buoyancy calibration and default shell thickness. Hull-section clipping derives displacement and buoyancy centers at finite angles. Hydrostatic forces update at 2 Hz; CPU pose integration remains at 60 Hz. Added water uses the same horizontal waterplane for centroid, portal pressure, breach flow and equipment immersion, with a waterplane-area estimate between hydrostatic updates. Moving water supplies the free-surface moment without an extra penalty. The calibration reconciles authored hull volume with stated mass and targets an estimated initial GM; each ship's `reports/stability.json` records the mismatch and load. Neither these estimates nor capsize timing establish historical stability.
