...
A blueprint's optional `submarine` equipment defines submerged handling, ballast capacity and rates, neutral fill fraction, vertical speed limits, periscope eye/depth, depth and torpedo-launch limits, surface/submerged engine module IDs, and appendage joint IDs. The compiler validates finite bounded parameters, engine links, periscope placement and distinct joint IDs. Export checks require each declared appendage to remain an independent pivot empty with moving children. All five presets rebuild when shared compiler code changes; the Bismarck baseline remains untouched.

`Combatant.submarine` holds serializable depth order, ballast volume, emergency blow, plane order and trim. `motion.y` remains the one world-space hull height. `stepSubmarine` follows flooding each fixed tick: the ballast/plane controller adjusts buoyancy and vertical velocity, then combines commanded pitch with damage trim. Surface immersion restores buoyancy; damage water adds weight without being confused with ballast. Intentional depth does not trigger sinking. Sinking hulls continue downward from their current depth and cannot be revived with a depth order. Breach inflow for submarines uses actual depth pressure; exceeding the operating limit costs HP. These are bounded gameplay hydrostatics, not a pressure-hull strength calculation.
