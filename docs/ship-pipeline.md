...
Compartments retain separate bounded breach clusters and water volume. Explicit exterior regions map openings to flood spaces, splitting waterline openings across room heights. Closed boundaries prevent transfer until opened or penetrated; hydraulic-head transfer conserves water. The original `assets/ships/author-flood-spaces.ts` recipe preserves researched/legacy room IDs and adds estimated wing and end spaces inside the hull. These are inspectable game envelopes, not certified historical watertight subdivisions. Exact boundaries, capacities, partition thicknesses and unmodeled upper/deep spaces remain in the discrepancy registers. Equipment immersion disables availability separately from HP; draining cannot repair destroyed equipment. Explicit propulsion groups link boiler supply, drives and shafts, with provisional aggregation and routing documented per preset. Flooded magazines disable ammunition supply.

Added water still changes the simplified draft/list/trim model; integrity loss or exhausted reserve buoyancy initiates sinking, with an explicit final cause. Hull mass, waterplane area, reserve buoyancy and handling remain calibrated values until the stability milestone. The flat-sea/undispersed stock gunnery scenarios can leave openings entirely above water; controlled submerged shots validate the flooding chain without claiming historical damage behavior. Per-shell history exposes protection, local damage, breach assignments and terminal outcomes in Gunnery.
