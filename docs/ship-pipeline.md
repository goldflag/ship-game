...
A cached hull-shell query covers surfaces between explicit exterior plates, including deck, bottom and ends, using provisional 12 mm steel. A normal probe suppresses the fallback where an explicit outer plate already provides protection. Unmapped openings reach the nearest actual flood cell. This closes gameplay coverage while leaving detailed plating, downflooding apertures, underwater shock, trapped air and structural fracture unresolved. Hull-shell fallback contacts are labeled in the impact ledger; the port armor overlay continues to show explicitly authored protection.

Universal HP sinking is retired. Equipment condition is a summary of module and gun health. Exhausting derived buoyancy causes sinking; sustained finite-angle inversion with an outward righting arm causes capsize. Immobile ships can keep fighting. Ships with permanently lost guns/ammunition leave the battle's fighting strength while afloat; temporary immersion can recover. Every loss retains a cause.

The sea used for gameplay is a flat Y=0 surface. GPU waves render the sea and animate buoys, while combat ship poses remain authoritative CPU values. For smooth motion at arbitrary display rates, the browser interpolates hull and joint transforms between the last two fixed ticks, presenting ships one tick (16.7 ms) behind simulation time. The camera, wake and inspection geometry follow those same sampled ship poses; firing and hit resolution continue to use the current authoritative tick. Pause holds the interpolation fraction, and resets/port teleports clear pose history. GPU wave samples never alter these poses. A future shared CPU wave model can add synchronized heave and wave-driven roll/pitch. Custom battles use renderer-free bot helm and fire commands, team-aware targeting, per-mount ballistic leading and shared damage/flooding for every selected ship. Formation lines start 5 km apart. Bot engagement ranges and avoidance are provisional gameplay heuristics. Ship contact/collision response, aircraft operations, networking, and a shipbuilding UI are not implemented here.
