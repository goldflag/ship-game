...
Fleet harbor keeps the fleet carousel below the ship, commander and orders on the left, characteristics on the right, and the brass Battle command centered horizontally in the viewport within the transparent top bar, with a caret that opens a mode menu and a small “Last” reading beneath it. Its instruments are approximately 15% smaller than garage study A. The ship and illustrative 3D harbor remain visible. The comparison switcher and alternate garage layouts have been removed.
...
The left rail contains Ship designs, New design, Edit design and a scrolling saved-design list in the existing Barlow and maritime control styles. New design uses the brass primary button; Edit design and per-row edit actions stay secondary. A visible Delete action beside every saved design opens an inline confirmation naming the design and its retained revisions. The editor’s Designs menu uses the same control. Confirmation removes the design from storage and the local fleet; deleting the open design starts a new one-block design. Each saved source shows Ready for sea, Preparing preview, Draft or Needs recovery. Mint marks the inspected design. Valid local designs appear before historical ships in the fleet carousel, with a My designs filter; selecting one renders its compiled ship in the harbor. Unfinished drafts remain available for editing. The rail replaces commander, daily-order, currency and tier placeholders. At narrow widths, the design and statistics columns share the space without covering one another.
