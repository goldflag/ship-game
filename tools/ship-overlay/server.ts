...
import { isHullConfiguration, embeddedJson, sanitizeMaterials, texturePath, vehicleId, type ReferencePack, type Scheme } from './reference';
...
export async function loadReference(root: string, vehicle: string): Promise<ReferencePack> {
  ...
  const visual = embeddedJson(page, /scheme\s*:\s*/).visual;
  ...
  const pack: ReferencePack & { format: number } = { format: PACK_FORMAT, vehicle, name: metadata.name, url, fetchedAt: new Date().toISOString(), scheme, paints, models: {}, omitted: [] };
  const pending = [...paths];
