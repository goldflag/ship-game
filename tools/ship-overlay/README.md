...
The GameModels3D fetcher and its `.build/ship-overlay/` cache live in `download.ts`, shared with the construction [reference workflow](../../docs/reference-workflow.md) (`ship:reference`), so both read the same downloads once.
...
**Lighting.** The viewer renders through WebGPU, as the game does. **Game lighting** (the default in this page) lights models the way the port does: the game's sky bakes the environment the model reflects (the sky itself is not drawn), the port sun and hemisphere fill take the game's mesh shares (`VisualEnvironment`: `PORT_LIGHT`, `meshLightShares`), the sun casts soft shadows fitted to the model, ship paint gets the game's plating, teak and wear (`ShipMaterialPalette`, `ShipSurfaceDetail`), ship ambient occlusion runs on the viewer's orthographic camera (`ShipOcclusion`), and the frame goes through the game's display grade and AgX (`DisplayTransform`). **Sun bearing** turns the sun about the model (0° ahead, 90° to starboard) and re-bakes the sky. **Studio lighting** is the earlier even light for judging shape; carousel thumbnails and the Shipbuilder comparison page use it. There is no sea, wet band or cloud shadow. **Isometric / Perspective** beside the camera views switches the orthographic comparison camera (one metric scale at every depth, the default) for a perspective camera; views, fit, zoom and comparison panels work in both.
