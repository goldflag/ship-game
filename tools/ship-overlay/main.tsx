...
function App() {
  ...
  const [kind, setKind] = useState<LibraryKind>(initialPart ? 'component' : 'ship');
  ...
  const [input, setInput] = useState(suggestedVehicles[shipId] ?? ''), [pack, setPack] = useState<ReferencePack | null>(null);
  ...
  const subtitle = kind === 'component' ? part && `${part.family} · ${standalone ? 'Shared recipe' : 'Installed model preview'}` : referenceName && mode !== 'inspect' ? `Compared with ${referenceName}` : selected?.configuration;
  useEffect(() => {
  ...
  function chooseKind(next: LibraryKind) { clearComparison(); setKind(next); setInput(next === 'ship' ? suggestedVehicles[shipId] ?? '' : ''); }
  ...
  function save() {
    if (!pack) return;
    try { localStorage.setItem(`ship-overlay:${kind === 'ship' ? shipId : `part:${partId}`}:${pack.vehicle}`, JSON.stringify({ pose, hull, components, paint })); setNotice('Alignment saved in this browser.'); }
    catch { setError('Browser storage is unavailable. Keep this tab open to retain the alignment.'); }
  }
  const gridNote = kind === 'component' ? 'Grid 1 m · mount datum Y = 0' : 'Grid 10 m · waterline Y = 0';
    ...
    <main>
      ...
      <aside aria-label="Model controls">
        ...
        {mode !== 'inspect' && <section><h2>Reference model</h2><form onSubmit={e => { e.preventDefault(); void loadGame(); }}><label htmlFor="source">GameModels3D URL or vehicle ID</label><input id="source" value={input} onChange={e => setInput(e.target.value)} placeholder="Paste a WoWS vehicle page URL" spellCheck={false}/><button className="primary" type="submit" disabled={busy || !input.trim()}>{busy ? 'Downloading reference…' : 'Load from GameModels3D'}</button></form>
          ...
          <p className="subtle">{kind === 'ship' && selected && !suggestedVehicles[selected.id] ? `${selected.name} has no World of Warships counterpart; paste any suitable vehicle page. ` : ''}Downloads stay on this computer. WoWS references start at 15 m per viewer unit; verify scale and waterline before judging accuracy.</p>
        ...
        <section><h2>Visible model bounds</h2><table><thead><tr><th scope="col">Metres</th><th scope="col" className="our-key">Ours</th><th scope="col" className="ref-key">Reference</th></tr></thead><tbody>{(['length', 'beam', 'height'] as const).map(k => <tr key={k}><th scope="row">{k}</th><td>{(modelReady ? dimensions : null)?.ours[k].toFixed(2) ?? '—'}</td><td>{(modelReady ? dimensions : null)?.reference?.[k].toFixed(2) ?? '—'}</td></tr>)}</tbody></table><p className="subtle">Whole-model axis-aligned bounds, including fittings. {gridNote}. This is a visual comparison, not a historical accuracy score.</p></section>
      </aside>
    </main>
    <LibraryCarousel kind={kind} onKind={chooseKind} ships={ships} parts={parts} shipId={shipId} partId={partId} onShip={chooseShip} onPart={choosePart}/>
  </div>;
}
