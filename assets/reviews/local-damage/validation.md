...
Master then merged the carrier-command work (PR #46: squadron commands, AA combat, Bismarck's 10.5 cm, 3.7 cm and 2 cm AA mounts). That merge was integrated too: the recipe re-ran over master's Bismarck blueprint (now 86 local regions, since every AA mount gets its own small gunhouse budget as a secondary-battery mount), Bismarck was rebuilt with local Blender, and the comparison records of the five other reference ships were refreshed under the changed gun catalog. Regenerated renders and the Bismarck thumbnail again keep master's bytes where pixel-identical, so no image bytes change against master. `ship:check all` passes for all eleven presets on the merged tree; the test suite and production build results below were rerun on it.
