...
The [port sun](camera-port-sun.png) and [sailing sun](camera-sea-sun.png) are unedited 1705 × 1359 canvas captures from the same diagnostic in WebGPU, High quality, Atlantic sea, refreshed after integrating master at `45392d0`. The paused port camera uses azimuth -2.129301687433082 rad, elevation control -π/6 rad and distance 325 m, with camera y=40.972 m. The paused sailing camera faces the sun's azimuth with a 30° upward tilt and camera y=121.6 m. The sun disk is visible in both; no sky or sunlight parameters changed for this follow-up.

Validation: all 132 tests and `bun run build` passed after integrating master. The input-driven camera tests cover sun visibility within the 30° limit, returning to the normal orbit, mouse aiming, binocular aim preservation, and clearance through extreme input, zoom, narrow/wide aspect ratios and sinking across all six camera modes. Both final WebGPU views reported no diagnostic errors.
