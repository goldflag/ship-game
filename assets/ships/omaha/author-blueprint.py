"""Omaha blueprint authoring: a Blender-recipe preset scaffolded by `ship:new --legacy`.
...
example):
  ...
  python3 assets/ships/omaha/author-blueprint.py [--loft loft.json]
...
(see the README), then `bun run ship:build omaha`.
"""
import argparse
import json
import math
...
for id, name, z, bearing in MAIN:
    b['mounts'].append(dict(id=id, name=name, partId='us-6in53-mk16-twin', battery='main', position=[0, round(deck_y(z) + .6, 3), round(z, 3)],
...
print(f'Authored omaha: {len(sections)} sections, {round(volume * 1.025)} t at the design waterline, {len(b["mounts"])} mounts, '
