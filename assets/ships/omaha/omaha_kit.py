"""Shared vocabulary for the Omaha recipe: materials, primitives with ownership, merged wires.

Fitting positions are reference-frame datums read off the approved GameModels3D pasc005 model
...
ZSHIFT = 0.7523  # runtime z = reference z + ZSHIFT
...
COLORS = {'naval': (.160, .172, .175), 'hullgray': (.160, .172, .175), 'roof': (.034, .035, .034), 'deck': (.034, .035, .034),
          'antifouling': (.082, .047, .026), 'black': (.009, .009, .009), 'edge': (.045, .047, .048), 'painted-edge': (.110, .118, .121),
          'dark': (.012, .013, .014), 'glass': (.02, .04, .05), 'canvas': (.50, .48, .40), 'wood': (.19, .13, .075),
...
class Kit:
    def __init__(self, D, collection_names):
        ...
        for key, color in COLORS.items():
            m = bpy.data.materials.new('Omaha ' + key)
    ...
    def loft(self, assembly, col, label, rings, material='naval', cap_bottom=True, cap_top=True, smooth=True, top_material=None):
        ...
        if cap_top:
            ...
            caps.append(len(ff) - 1)
        ob = self.solid(self.tag(self.mesh(assembly + '.' + label, vv, ff, material, col, smooth), assembly))
    ...
    def roof(self, s):
        """Roof material of a measured block: dark grey painted steel, as the reference paints every roof."""
        return 'roof'

    # ------------------------------------------------------------ guns
    def gun_seat(self, mount, radius=None, col=None):
        """A pedestal from the supporting deck to a mount's datum when it stands above it."""
        x, y, z = R(mount['position'])
        floor = self.below(x, y, z + .3)
        gap = z - floor
        if gap > .03:
            r = radius or max(.35, mount['weapon']['barbetteRadius'] * .75)
            self.cylz(mount['id'], col or self.collections['Main and secondary batteries'], 'pedestal', (x, y, floor - .01), r, gap + .012, 'naval', 24)
    ...
    def beam(self, assembly, col, label, a, b, width, depth, material='naval', up=(0, 0, 1)):
        ...
        return self.tag(self.solid(ob), assembly)
