...
for a in auth['captures']:
    ...
    ours=labelled(paper(ap),'Original Fletcher revision 3 · '+definition['contentHash'][:12])
...
after=labelled(paper(source/'generated/review/profile.png'),'Corrected revision 3 · original hull and superstructure')
...
for name in ['starboard','top','bridge']:
    shape_html+='<figure><a href="shape-'+name+'.png"><img src="shape-'+name+'.png" loading="lazy" alt="Previous Fletcher, corrected Fletcher and reference '+name+' silhouettes"></a><figcaption>'+name.capitalize()+': previous revision 2 / corrected revision 3 / GameModels3D. Identical camera and scale; no image warping.</figcaption></figure>'
start=body.index('<figure><img src="before-after.png"')
end=body.index('</figure>',start)+len('</figure>')
body=body[:start]+shape_html+body[end:]
