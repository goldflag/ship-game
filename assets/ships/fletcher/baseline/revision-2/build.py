"""Fletcher revision 2: original reference-led, blueprint-driven ship authoring.
...
images, external meshes, attachment transforms or textures. See reports/refit.md.
"""
...
# Continuous station hull: rounded bilges, raked forefoot, flared bow, cruiser stern.
verts=[]
...
for side in [-1,1]:
    ...
    for z in [.55,1.8]:
        seam=[(s-half,side*(w-.035),min(z,deckz(s-half)-.2)) for s,w in h['halfBreadths'] if 5<s<103]
    ...
    if s['id'] in ['forward-deckhouse','aft-deckhouse','machinery-deckhouse']:
        ...
        for side in [-1,1]:
            for i in range(int((xmax-xmin)/3.2)):
                x=xmin+1.45+i*3.2;yy=side*(w+.028);base=s['baseY']+.12
...
# Two lower Oerlikon wings, rounded splinter shields and triangular supporting knees.
for side in [-1,1]:
    wing=outline_oval(24.5,side*3.5,1.8,1.38,40)
    prism('bridge.aa-wing',wing,5.60,5.78)
    bulwark('bridge.aa-shield',wing,5.78,.72)
    for x in [23.4,25.6]:
        mesh('bridge.wing-knee',[(x,side*3.2,5.65),(x,side*4.5,5.65),(x,side*3.2,4.65)],[(0,1,2)],materials['naval'])
    # Navigation bridge wings project aft of the curved pilot front.
    wing=outline_rect(16.6,22.7,side*4.7 if side<0 else 2.7, -2.7 if side<0 else 4.7,.55)
    prism('bridge.navigation-wing',wing,7.90,8.02)
    bulwark('bridge.navigation-shield',wing,8.02,.83)
    stairs('bridge.access', (13.1,side*3.5,3.12),(16.6,side*3.5,5.7))
    stairs('bridge.upper-access',(16.3,side*3.5,5.72),(19.0,side*3.5,7.98),.56)
# Round pilot windows around the forward arc; eyebrows, mullions and bridge visor.
for i in range(13):
    a=-math.pi*.47+i*math.pi*.94/12
    p=(20.9+2.82*math.cos(a),2.82*math.sin(a),9.26)
    portlight('bridge.navigation-window',p,(math.cos(a),math.sin(a),0),.205)
for side in [-1,1]:
    for x in [18.1,19.2]:portlight('bridge.side-window',(x,side*2.83,9.26),(0,side,0),.205)
    portlight('bridge.lower-port',(19.0,side*3.24,7.28),(0,side,0),.19)
upper=[(-z,-x) for x,z in structures['pilot-house']['footprint']]
prism('bridge.visor',[(20.7+(x-20.7)*1.06,y*1.075) for x,y in upper],9.79,9.90,materials['roof'])
# Open bridge behind the director, rather than another solid box.
bulwark('bridge.flying-shield',upper,9.9,.68)
for side in [-1,1]:
    cyl('bridge.pelorus',(18.25,side*1.9,10.28),.13,.74,materials['edge'])
    box('bridge.pelorus-head',(18.25,side*1.9,10.67),(.28,.25,.16),materials['naval'])
    rod('bridge.voice-pipe',(18.7,side*1.75,9.98),(18.7,side*1.75,10.6),.045,materials['edge'])
    # Signal searchlights with actual lens and yoke.
    x,y,z=17.25,side*4.0,8.94
...
for s in definition['structures']:
    ...
    def ring(t,scale=1,zoff=0):
        return [(cx-.15*height*t+rx*scale*math.cos(i*math.tau/n),ry*scale*math.sin(i*math.tau/n),base+height*t+zoff+.90*math.cos(i*math.tau/n)*t*t) for i in range(n)]
...
# Slender pole foremast (the early ships did not have a battleship tripod).
rod('mast.fore',(15.1,0,5.7),(13.82,0,23.9),.19,materials['naval'],r2=.075,vertices=20)
rod('mast.top',(13.82,0,23.9),(13.75,0,25.1),.057,materials['edge'],r2=.03)
rod('mast.yard',(14.2,-4.2,19.2),(14.2,4.2,19.2),.071,materials['edge'],r2=.071)
for side in [-1,1]:
    rod('mast.yard-brace',(14.07,0,20.55),(14.2,side*4.2,19.2),.031,materials['edge'])
    for y in [1.2,2.4,3.7]:
        tube_path('rigging.signal-halyard',[(14.2,side*y,19.2),(16.2,side*(2.6+y*.19),8.92)],.009,materials['rope'],sides=5)
    rod('rigging.fore-stay',(13.84,0,23.5),(30,side*2.65,5.85),.014,materials['dark'],vertices=6)
    rod('rigging.aft-stay',(13.84,0,23.5),(-21.3,side*.45,13.75),.014,materials['dark'],vertices=6)
ladder('mast.rungs',(14.86,0,7.4),(13.90,0,22.7),.34)
cyl('mast.lookout-platform',(14.53,0,14.5),.85,.12,materials['roof'],vertices=32)
rails('mast.lookout-rail',[(x,y,14.56) for x,y in outline_oval(14.53,0,.8,.8,20)],.80,True,1.4)
# Small early SC-style air-search array and separate SG element.
for z in [23.4,23.75,24.1]:rod('radar.sc-horizontal',(13.85,-1.6,z),(13.85,1.6,z),.023,materials['edge'],vertices=6)
for i in range(10):
    y=-1.6+i*3.2/9;rod('radar.sc-vertical',(13.85,y,23.4),(13.85,y,24.1),.022,materials['edge'],vertices=6)
rod('radar.sc-support',(13.9,0,22.9),(13.85,0,24.15),.048,materials['edge'])
rod('radar.sg-boom',(14.0,0,21.35),(14.95,0,21.35),.05,materials['edge'])
box('radar.sg-head',(14.95,0,21.75),(.22,.65,.18),materials['naval'])
# Aft pole and vertical wireless aerials.
rod('mast.aft',(-22.1,0,5.7),(-22.65,0,14.65),.075,materials['edge'],r2=.025)
rod('mast.aft-yard',(-22.55,-1.75,12.4),(-22.55,1.75,12.4),.037,materials['edge'])
for side in [-1,1]:
    rod('rigging.wireless',(13.90,side*.45,22.0),(-22.6,side*.5,14.3),.010,materials['dark'],vertices=6)
    rod('rigging.aft-downlead',(-22.6,side*.5,14.3),(-34,side*2.2,5.80),.011,materials['dark'],vertices=6)
...
for dx in [-1.65,1.65]:
    for y in [-1.65,1.65]:rod('aa-platform.stanchion',(aa_x+dx,y,5.7),(aa_x+dx,y,aa_z),.12,materials['naval'],vertices=12)
for side in [-1,1]:
    rod('aa-platform.diagonal',(aa_x-1.65,side*1.65,5.7),(aa_x+1.65,side*1.65,aa_z),.075,materials['edge'])
...
for side in [-1,1]:
    cx,cy,z=8.2,side*4.30,5.75;L=7.25;B=1.90
    ...
    for dx in [-2.3,2.3]:
        y=side*3.62;x=cx+dx
        tube_path('boats.davit',[(x,y,3.0),(x,y,6.95),(x,side*3.75,7.35),(x,side*4.55,7.4)],.075,materials['naval'],sides=12)
        rod('boats.fall',(x,side*4.55,7.4),(x,cy,z+.32),.015,materials['rope'],vertices=6)
        cyl('boats.davit-base',(x,y,3.15),.23,.30,materials['edge'],vertices=20)
...
for side in [-1,1]:
    for x in [16.8,20.0,28.5]:raft('lifesaving.forward-floats',x,side*3.54,4.40,side)
    ...
    bulwark('deck.boat-bulwark',[(4.2,side*5.73),(12.1,side*5.48)],2.98,.95,closed=False)
...
for side in [-1,1]:
    ...
    xx=52.6;yy=side*(width(xx)+.03);zz=3.7
...
rod('rigging.jackstaff',(56.15,0,4.9),(56.15,0,7.1),.028,materials['edge'],r2=.018)
rod('rigging.ensign',(-56.5,0,2.85),(-57.1,0,6.0),.035,materials['edge'],r2=.021)
...
for side in [-1,1]:
    ...
    for x,w in [(-28,0),(-23,.48),(24,.48),(30,0)]:
        y=side*width(x)*.77;vv += [(x,y,-2.7),(x,y+side*w,-2.92)]
    ...
    for j,d in enumerate('445'):
        for stroke in digits[d]:
            ...
            for u,v in stroke:
                ...
                z=2.15+v*.62;points.append((x,side*(hull_breadth_at(x,z)+.026),z))
...
print('FLETCHER REVISION 2',len(col.objects),'original objects',flush=True)
