...
The [comparison page](index.html) shows revision 2, revision 3 and the reference with identical cameras, followed by runtime captures and all matching views. Raw probes and limits are retained in [shape-measurements.json](shape-measurements.json). See [runtime summary](runtime/summary.json) for final simulation, build and runtime evidence.
