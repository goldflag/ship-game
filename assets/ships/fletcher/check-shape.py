...
assert auth['capturePlanSha256'] == reference['capturePlanSha256'], 'Current camera plans differ'
# New component closeups may be added without changing the historical probes.
for name in ['starboard','top','bow','bridge']:
    records=[next(c for c in m['captures'] if c['id']==name) for m in [auth,reference,previous]]
    for field in ['position','target','spanM','resolution','projection','cameraMatrixWorld','visibility']:
        assert records[0][field] == records[1][field] == records[2][field], 'Probe camera changed: '+name+' '+field
...
for view in ['starboard', 'top', 'bridge']:
    panels = []
    for folder, label in [(old, 'Previous revision 2'), (current, 'Current revision 4'), (ref, 'GameModels3D reference — comparison only')]:
