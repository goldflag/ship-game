...
IY=-36.25*.3048
# Island and funnel half-widths measured on GameModels3D pasa518 (the refit kept the island width).
HW=2.075;FHW=2.06
...
for x in [-118,-113.7,-109.4,-90.4,-86.1,-81.8,-77.5,-73.2,-68.9,-64.6]:
 ...
 for y in [-9,-4.5,0,4.5,9]:rod('Arrestor wire spring',(x-.28,y,FLIGHT),(x,y,FLIGHT+.07),.025,M['edge'],COL['Deck equipment'],vertices=6)
for x in [-57,-52,-47]:
...
for x in range(-121,115,5):
 if all(abs(x-(-sum(p[1] for p in S[id]['footprint'])/len(S[id]['footprint'])))>8 for id in ['elevator-forward','elevator-middle','elevator-aft']):
  box('Deck centerline',(x,0,FLIGHT+.010),(2.5,.12,.018),M['line'],COL['Deck equipment'])
...
# Original detail vocabulary and the plated 5-inch sponsons (see details.py, sponsons.py).
for part in ['details','sponsons']:exec((Path(__file__).parent/(part+'.py')).read_text(),globals())
# Edge girders, catwalks and steel galleries keep the hangar open and inspectable.
def gun_gaps(a,b,y,clear=1.6):
 # Rail runs along x from a to b at side y, broken where a 20 mm mount stands so its
 # shoulder rests and shield sweep clear (1942 galleries left the gun positions open).
 guns=sorted(-m['position'][2] for m in D['mounts'] if 'oerlikon' in m['partId'] and abs(-m['position'][0]-y)<1.2)
 runs=[];x=a
 for g in guns:
  if g+clear<=a or g-clear>=b:continue
  if g-clear>x+.3:runs.append((x,g-clear))
  x=max(x,g+clear)
 if b>x+.3:runs.append((x,b))
 return runs
...
for side,y in [('port',10.1),('starboard',-10.1)]:
 ...
 for a,b,yedge in [(-69,-34,14.2),(31,67,14.2),(-121,-110,13.1)]:
  ...
  for c,d in gun_gaps(a,b,yy):railing([(c,yy+(1 if yy>0 else -1)*.75),(d,yy+(1 if yy>0 else -1)*.75)],FLIGHT-.68,'AA gallery',col)
 for x in range(-82,72,6):
  # Flared brackets beneath the flight-deck cantilever.
  yy=y*1.18;rod('Cantilever bracket',(x,y,FLIGHT-.46),(x,yy,15.5569),.09,M['naval'],col)
...
for m in D['mounts']:
 ...
 if m['partId'].startswith('us-5'):
  pass  # Plated sponsons, gun decks and bulwarks: sponsons.py
...
for c,d in gun_gaps(-14,35,-15.5):railing([(c,-16.45),(d,-16.45)],FLIGHT,'Island gallery',COL['Island'])
...
for key in ['communications-walkway','flag-walkway','navigation-wings','bridge-roof','pilot-roof','fighting-platform','fighting-roof','searchlight-platform-port','searchlight-platform-starboard']:
 ...
 for x,y in points:
  if abs(y-IY)>2.7:
   rod('Platform web support',(x,IY+math.copysign(HW,y-IY),z-1.4),(x,y,z-.12),.065,M['naval'],COL['Island'])
...
for x in [frame(102.5),frame(89.7)]:
 for y in [IY-FHW-.12,IY+FHW+.12]:rod('Steam pipe',(x,y,FLAG),(x,y,STACK+.45),.10,M['naval'],COL['Island'],vertices=12)
...
for x,z in [(frame(82),level(174)),(AX,level(165))]:rod('Aerial spreader',(x,IY-1.05,z),(x,IY+1.05,z),.04,M['naval'],COL['Island'],vertices=8)
...
for id,x,z in [('forward',frame(71.5),PILOT_ROOF),('aft',frame(110.7),ROOF)]:
 base=FLIGHT if id=='aft' else COMM  # the aft trunk stands on the flight deck abaft the island
 cyl('Director support trunk',(x,IY,(base+z)/2),.38,z-base,M['naval'],COL['Island'],24)
...
for id,x in [('forward',frame(75.5)),('aft',frame(109.5))]:
 ...
 rod('Crane head axle',end+Vector((0,-.30,0)),end+Vector((0,.30,0)),.07,M['naval'],COL['Island'])
...
deck_support=SupportSurface([*COL['Hull'].objects,*COL['Hangar and galleries'].objects,*COL['Flight deck'].objects])
...
for side,sgn in [('port',1),('starboard',-1)]:
 for x,length in [(-79,10.7),(-45,8.5),(0,12.2),(14,9.1)]:
  ...
  for xx in [x-length*.32,x+length*.32]:
   rod('Boat davit',(xx,y*.89,deck_support.below(xx,y*.89,MAIN+.05)),(xx,y*.89,MAIN+4.4),.09,M['naval'],COL['Deck equipment'])
   rod('Davit head',(xx,y*.89,MAIN+4.4),(xx,y,MAIN+4.4),.09,M['naval'],COL['Deck equipment'])
   # Fall, sheave and bridle use the same longitudinal station as the davit.
   fitboat=Fittings(dict(mesh=mesh,cyl=cyl,rod=rod,box=box),M,COL['Deck equipment'])
   fitboat.ring('Davit block',(xx,y,MAIN+4.25),.12,.035,'y',segments=12)
   rod('Davit block hanger',(xx,y,MAIN+4.4),(xx,y,MAIN+4.25),.035,M['naval'],COL['Deck equipment'])
   rod('Boat fall',(xx,y,MAIN+4.25),(xx,y,MAIN+1.65),.021,M['edge'],COL['Deck equipment'],vertices=6)
   for offset in [-.76,.76]:rod('Boat lifting bridle',(xx,y,MAIN+1.65),(xx,y+offset,MAIN+1.16),.025,M['edge'],COL['Deck equipment'],vertices=6)
# Carley floats on edge in exterior racks under the flight-deck overhang: one oval tube and
# grating per float, two cradle arms to the gallery wall. Kept clear of gallery webs and sponsons.
rafts=Kit('Carley floats',COL['Deck equipment'],'carley-floats')
spans=[(-69,-34),(31,67)]
xs=[x for a,b in spans for x in range(a+3,b-1,8)]+[x for x in range(-86,72,9) if not any(a-2<x<b+2 for a,b in spans)]
for x in xs:
 for sign in [-1,1]:
  y=sign*12.4;z=FLIGHT-2.2
  carley(rafts,x,y,z,2.4,1.05,0)
  for dx in [-.6,.6]:
...
for side,sign in [('port',1),('starboard',-1)]:
 ...
 for kind,y,fr in [('inner',3.6,175),('outer',7.5,161)]:
  ...
  hull_skin=SupportSurface([o for o in COL['Hull'].objects if o.get('nodeId')=='hull.surface'])
  for dy in [-1.0,1.0]:
   foot=hull_skin.along((x+1.4,y+dy,z+.3),(0,0,1),15)
   rod('Shaft bracket',foot,(x+1,y,z),.14,M['naval'],COL['Underwater'])
  ...
  for blade in range(3):
   a=blade*2*math.pi/3;points=[(.10,.06),(.55,-.06),(.94,.05),(1,.28),(.76,.40),(.20,.14)]
...
stock=rod('Rudder stock',(0,0,3.0),(0,0,7.3),.24,M['edge'],COL['Underwater'],vertices=20);stock.parent=rudder;stock['assemblyId']='rudder'
...
for sign in [-1,1]:
 ...
 fit.ladder('Funnel external ladder',(frame(96),IY+sign*(FHW+.04),ROOF),(frame(96),IY+sign*(FHW+.04),STACK),.64)
 fit.ladder('Fighting top access',(MX-1,IY+sign*.9,ROOF),(MX-1,IY+sign*.9,TOP_FLOOR),.54)
 for x in [frame(96)-1.65,frame(96)+1.65]:fit.knee('Searchlight gallery pierced knee',x,IY+sign*FHW,IY+sign*4.2,ROOF-.2,1.4)
 for x in [frame(72),frame(75)]:fit.knee('Navigation wing pierced knee',x,IY+sign*HW,IY+sign*5.0,NAV-.18,1.45)
...
for sign in [-1,1]:
 ...
 for a,b,outer in [(-69,-34,14.95),(31,67,14.95),(-121,-110,13.85)]:
  for x in range(a+1,b,4):
   fit.knee('AA gallery supporting web',x,sign*10.10,sign*outer,15.5569,1.65)
   rod('AA gallery deck hanger',(x,sign*10.10,15.5569-1.65),(x,sign*10.10,FLIGHT-.05),.09,M['naval'],COL['Hangar and galleries'])
 for a,b in [(-69,-34),(31,67)]:
  for x in range(a+2,b,3):
   # Raised rolled shutters retain separate vertical slats and guide tracks.
   box('Shutter roll rib',(x,sign*10.21,FLIGHT-2.646),(.035,.075,.47),M['naval'],COL['Hangar and galleries'])
  for x in [a,b]:
...
reels=Kit('Mooring reels',COL['Deck equipment'],'mooring-reels')
for sign in [-1,1]:
 ...
 for x in [104,95,-109,-119]:reel(reels,x,sign*4.0,interpolate(H['deckHeights'],x+H['length']/2)+.01,.42,1.2)
 for x in [50.5,60.5,-43.5,-58.5]:
  box('20 mm magazine locker',(x,sign*13.4,15.7869+.35),(.9,.42,.7),M['naval'],COL['Deck equipment'])
  box('Magazine locker lid',(x,sign*13.4,15.7869+.72),(.96,.49,.07),M['roof'],COL['Deck equipment'])
reels.emit()
