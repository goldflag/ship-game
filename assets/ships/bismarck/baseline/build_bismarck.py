...
LOA=250.50; BEAM=36.0; DRAFT=9.33; DEPTH=15.0
ST=[(0,0),(2,2.9),(5,4.7),(10,6.6),(20,9.5),(30,11.5),(40,12.6),(50,14.0),(60,15.2),(70,16.3),(80,17.1),(90,17.65),(100,17.93),(110,18.0),(120,18.0),(130,17.72),(140,17.3),(150,16.6),(160,15.7),(170,14.6),(180,13.25),(190,11.65),(200,9.75),(210,7.65),(220,5.55),(230,3.60),(240,1.8),(247,0.63),(250.5,0)]
def interp(table,t):
...
def deckz(x):return interp([(0,6.5),(35,5.82),(90,5.67),(150,5.67),(190,6.0),(220,7.0),(250.5,8.8)],x+LOA/2)
def bottom(s):return interp([(0,5.7),(3,1.8),(7,-1.1),(13,-2.8),(21,-3.7),(31,-6.7),(42,-8.8),(56,-9.33),(205,-9.33),(229,-9.33),(242.8,-9.0),(247,-2.2),(250.5,8.8)],s)
N=360
...
turret_objects=[]
def turret(name,x,y,zbase,bearing=0,scale=1,primary=True,rangefinder=True):
    col=MAIN if primary else SECOND
    r=4.95 if primary else 2.50
    cyl(name+' • armored barbette',(x,y,(deckz(x)+zbase)/2),r,zbase-deckz(x),hullgray,col,64)
    cyl(name+' • roller race',(x,y,zbase+.13),r+.10,.26,edge,col,64)
    L,W,H=(13.4,11.0,4.1) if primary else (6.5,5.2,2.6)
    ...
    fc=[tuple(reversed(range(8))),tuple(range(8,16))]+[(i,(i+1)%8,(i+1)%8+8,i+8) for i in range(8)]
    ...
    turret_objects.append(ob)
    def pt(a,b,z):return (x+a*math.cos(bearing)-b*math.sin(bearing),y+a*math.sin(bearing)+b*math.cos(bearing),z)
    spacing=3.75 if primary else 1.75
    exposed_start=5.8 if primary else 2.75
    muzzle=19.63 if primary else 8.25
    bore=.19 if primary else .075
    gunz=zbase+(1.85 if primary else 1.15)
    elev=math.radians(1.0)
    for k in (-1,1):
        gy=k*spacing/2
        rad=.69 if primary else .30
        ...
        for (a,ra),(b,rb) in zip(sections,sections[1:]):
            rod(name+' • 38 cm barrel' if primary else name+' • 15 cm barrel',pt(a,gy,gunz+(a-exposed_start)*math.tan(elev)),pt(b,gy,gunz+(b-exposed_start)*math.tan(elev)),ra,edge,col,rb,24)
    ...
    ob['battery']='38 cm twin' if primary else '15 cm twin'
    return ob
main_specs=[('Anton',192.55,6.8,0),('Bruno',174.35,10.7,0),('Cäsar',64.35,10.6,PI),('Dora',46.15,6.4,PI)]
for name,station,z,angle in main_specs:
    ob=turret(name,station-122.5,0,z,angle,primary=True,rangefinder=name!='Anton')
    ob['historical_station_m']=station
for side in (-1,1):
    for i,(x,y) in enumerate([(27.0,9.6),(8.1,14.1),(-24.4,14.0)]):
        # Forward/mid turrets train forward; after turrets aft, as in the reference plan.
        turret(('Port' if side==1 else 'Starboard')+f' 15 cm {i+1}',x,side*y,6.15,0 if i<2 else PI,primary=False,rangefinder=i==1)
...
for path in [Path(__file__),OUT/'README.md']:
    if path.exists():
...
assert abs(dims[0]-250.5)<.001
assert abs(dims[1]-36)<.001
assert abs(min(p.z for p in bounds)+9.33)<.001
