"""Shared vocabulary for the New Orleans recipe: materials, primitives with ownership, merged wires.

Fitting positions are reference-frame datums read off the approved GameModels3D pasc107 B_Hull model
...
ZS = 0.6112  # runtime z = reference z + ZS (authoring/lines.json `zShift`: the hull centred on its length)
...
# Linear-RGB interpretations of the reference's default texture; appearance.json binds the named paints.
COLORS = {'naval': (.137, .180, .196), 'hullgray': (.137, .180, .196), 'roof': (.130, .171, .187), 'deck-blue': (.026, .034, .040),
          'wood_deck': (.026, .034, .040), 'antifouling': (.085, .075, .046), 'boot': (.018, .018, .019), 'black': (.014, .014, .015),
          'edge': (.040, .047, .052), 'painted-edge': (.100, .130, .142), 'dark': (.012, .013, .014), 'glass': (.02, .04, .05),
          'canvas': (.30, .31, .29), 'wood': (.19, .13, .075), 'white': (.52, .53, .52), 'red': (.40, .03, .02),
          'green': (.03, .30, .06), 'bronze': (.36, .27, .12), 'brass': (.45, .33, .12), 'rust': (.16, .07, .035)}
...
class Kit:
    ...
    def barbette(self, mount):
        """The armour model's 2.96 m barbette from the deck below to the gunhouse sole, with a coaming ring."""
        x, y, z = R(mount['position'])
        floor = self.below(x, y, z - .05)
        col = self.collections['Main and secondary batteries']
        aid = mount['id']
        self.cylz(aid, col, 'barbette', (x, y, floor - .02), 2.96, z - floor + .025, 'naval', 72)
        self.cylz(aid, col, 'barbette coaming', (x, y, floor), 3.02, .1, 'edge', 72)
    ...
    def gun_seat(self, mount):
        ...
        gap = z - floor
        if gap > .03:
            r = max(.35, mount['weapon']['barbetteRadius'] * .75)
            col = self.collections['Light AA' if mount['battery'] != 'main' and not mount['id'].startswith('secondary') else 'Main and secondary batteries']
            self.cylz(mount['id'], col, 'pedestal', (x, y, floor - .01), r, gap + .012, 'naval', 20)
    ...
    def lattice(self, assembly, col, a, b, w, h, n=None, chord=.06, web=.035, material='naval'):
        a, b = Vector(a), Vector(b)
        d = (b - a)
        length = d.length
        d.normalize()
        ...
        for s in (-1, 1):
            for u in (-1, 1):
                self.member(assembly, col, a + side * s * w / 2 + up * u * h / 2, b + side * s * w / 2 + up * u * h / 2, chord, material, 6)
        for i in range(n):
            p, q = a.lerp(b, i / n), a.lerp(b, (i + 1) / n)
            for s in (-1, 1):
                self.member(assembly, col, p + side * s * w / 2 - up * h / 2, q + side * s * w / 2 + up * h / 2, web, material, 5)
                self.member(assembly, col, p + side * s * w / 2 + up * h / 2, q + side * s * w / 2 - up * h / 2, web, material, 5)
            self.member(assembly, col, p - side * w / 2 + up * h / 2, p + side * w / 2 + up * h / 2, web, material, 5)
            self.member(assembly, col, p - side * w / 2 - up * h / 2, p + side * w / 2 - up * h / 2, web, material, 5)
