...
definition['structures'] = [s for s in definition['structures']
                            if not s['id'].startswith('fixed-gallery-')]
for name, (points, triangles) in surfaces.items():
    ...
    definition['structures'].append(dict(
        id='fixed-gallery-'+name.replace('.', '-'), name=name.replace('.', ' ').replace('-', ' '),
...
print(f'Synchronized {len(surfaces)} original fixed gallery surfaces.')
