...
def gunhouse(quad):
    ...
    if quad:
        floor=[(-6.9,0),(-6.75,-1.5),(-6.3,-3),(-5.65,-4.4),(-4.85,-5.42),(.5,-6.15),(4.9,-5.48),(4.9,5.48),(.5,6.15),(-4.85,5.42),(-5.65,4.4),(-6.3,3),(-6.75,1.5)]
        ridge=.5; rear=-6.9; front=4.9; rear_z=3.22; front_z=3.38
    else:
        floor=[(-5.1,0),(-4.98,-1.15),(-4.64,-2.3),(-4.1,-3.4),(-3.45,-4.15),(-1.0,-4.72),(3.9,-4.32),(3.9,4.32),(-1.0,4.72),(-3.45,4.15),(-4.1,3.4),(-4.64,2.3),(-4.98,1.15)]
        ridge=-1.0; rear=-5.1; front=3.9; rear_z=3.57; front_z=3.40
    def roof_z(x):
        return rear_z+(3.7-rear_z)*(x-rear)/(ridge-rear) if x<=ridge else 3.7+(front_z-3.7)*(x-ridge)/(front-ridge)
    roof=[(x,y*(.978 if x==front else .95),roof_z(x)) for x,y in floor]
    n=len(floor)
    ...
    for label,polygon in [('rear',[0,1,2,3,4,5,8,9,10,11,12]),('front',[5,6,7,8])]:
    ...
    for i in range(n):
        ...
        label='front' if i==6 else f'wall-{i}'
        mm=324 if i==6 else 225 if i in [4,5,7,8] else 174
        tri(label+'-a',[a,b,c],mm);tri(label+'-b',[a,c,d],mm)
    return dict(version=1,vertices=vertices,faces=faces,provenance=dict(sourceId='rn-mkiii-plan',basis='inferred',note='Revision 3: upright face, continuous sloping sides, rounded rear and shallow roof crown from RN plates 2/62 and IWM photographs. Local dimensions and plate boundaries remain approximate; no game geometry.'))

catalogPath=ROOT/'assets/parts/guns.json';catalog=json.loads(catalogPath.read_text())
main=dict(kind='gun',barbetteRadius=6.1,gunhouseSize=[11.8,12.3,3.7],pivotHeight=1.85,trunnionForward=4.5,muzzleForward=17.1,
...
parts=[dict(main,id='bl-14-mkvii-quad',name='14-inch Mk VII quadruple',massKg=1582000,barrelCount=4,gunhouseMesh=gunhouse(True)),
       dict(main,id='bl-14-mkvii-twin',name='14-inch Mk VII twin',massKg=915000,barrelCount=2,barbetteRadius=4.7,gunhouseSize=[9.0,9.44,3.7],rangefinderWidth=9.14,rangefinderForward=-2.8,trunnionForward=3.6,muzzleForward=16.2,gunhouseMesh=gunhouse(False)),
