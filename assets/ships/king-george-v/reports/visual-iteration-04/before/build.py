...
for mount in D['mounts']:
 ...
 if main:
  ...
  radius=.74;zc=1.44;port_top=front_z-.105
  for i,gy in enumerate(axes):
   ...
   panel(gy-radius,gy+radius,port_top,front_z)
   aperture=arc+[(gy+radius,port_top),(gy-radius,port_top)]
   idx=len(vv);vv.extend([(front-inset,y,base+z) for inset in [0,.48] for y,z in aperture]);count=len(aperture)
   for k in range(count):ff.append([idx+k,idx+(k+1)%count,idx+count+(k+1)%count,idx+count+k]);mi.append(0)
 ...
 if main:
  ...
  rx=spec['rangefinderForward'];root_y=W*.39;tip_y=spec['rangefinderWidth']/2+.47
  for side in [-1,1]:
   outline=[(rx-.66,side*root_y),(rx+.66,side*root_y),(rx+1.14,side*(tip_y-.14)),(rx+1.05,side*tip_y),(rx-1.12,side*tip_y),(rx-1.22,side*(tip_y-.14))]
   ...
   box('Rangefinder forward glass',(rx+1.135,side*(tip_y-.23),z),(.035,.30,.25),'glass')
   ...
   hx=-5.0 if quad else -3.75;hy=side*(1.5 if quad else 1.05)
...
up_launcher('up-main-b',-1.8,0,3.69,yawB)
up_launcher('up-main-y',-2.0,2.25,3.56,yawY)
up_launcher('up-waist-1',-2.0,-2.25,3.56,yawY)
