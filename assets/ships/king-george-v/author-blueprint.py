...
def gunhouse(quad):
    # Original interpretation of RN gunhouse plan plate 2, flashproofing plate 62
    # and IWM A3655/A3905. The front is vertical, close to the trunnions. There
    # is no upper shoulder bevel: continuous sides meet two shallow roof planes.
    # Revision 4 separates the roof crown from the widest plan station. Relative
    # spans are checked against retained raster measurements, not game meshes.
    if quad:
        floor=[(-8.3,0),(-8.14,-1.5),(-7.67,-3),(-6.92,-4.4),(-6.05,-5.42),(.5,-6.15),(4.9,-5.48),(4.9,5.48),(.5,6.15),(-6.05,5.42),(-6.92,4.4),(-7.67,3),(-8.14,1.5)]
        ridge=-2.8; rear=-8.3; front=4.9; rear_z=2.72; front_z=2.87; peak=3.12
    else:
        floor=[(-7.0,0),(-6.83,-1.15),(-6.32,-2.3),(-5.49,-3.4),(-4.5,-4.15),(-1.0,-4.72),(4.9,-4.32),(4.9,4.32),(-1.0,4.72),(-4.5,4.15),(-5.49,3.4),(-6.32,2.3),(-6.83,1.15)]
        ridge=-1.0; rear=-7.0; front=4.9; rear_z=3.10; front_z=2.87; peak=3.23
    # Split a side at the crown only when required. Both rings have the same
    # stations, so the roof can be two genuinely planar polygons.
    split=[]
    for a,b in zip(floor,floor[1:]+floor[:1]):
        split.append(a)
        if min(a[0],b[0])<ridge<max(a[0],b[0]):
            t=(ridge-a[0])/(b[0]-a[0]);split.append((ridge,a[1]+t*(b[1]-a[1])))
    floor=split
    def roof_z(x):
        return rear_z+(peak-rear_z)*(x-rear)/(ridge-rear) if x<=ridge else peak+(front_z-peak)*(x-ridge)/(front-ridge)
    roof=[(x,y*.86,roof_z(x)) for x,y in floor]
    n=len(floor)
    vertices=[[x,y,.25] for x,y in floor]+[list(p) for p in roof]
    faces=[]
    def tri(label,idx,mm,finish='naval'):faces.append(dict(id=label,indices=idx,thicknessMm=mm,material='steel',finish=finish))
    for i in range(1,n-1):tri('floor-'+str(i),[0,i+1,i],25)
    for label,polygon in [('rear',[i for i,(x,y) in enumerate(floor) if x<=ridge]),('front',[i for i,(x,y) in enumerate(floor) if x>=ridge])]:
        for i in range(1,len(polygon)-1):tri(f'roof-{label}-{i}',[n+polygon[0],n+polygon[i],n+polygon[i+1]],149,'roof')
    for i in range(n):
        j=(i+1)%n;a=i;b=j;c=b+n;d=a+n
        is_front=floor[i][0]==front and floor[j][0]==front
        label='front' if is_front else f'wall-{i}'
        mm=324 if is_front else 225 if max(floor[i][0],floor[j][0])>=ridge else 174
        tri(label+'-a',[a,b,c],mm);tri(label+'-b',[a,c,d],mm)
    return dict(version=1,vertices=vertices,faces=faces,provenance=dict(sourceId='rn-mkiii-plan',basis='inferred',note='Revision 4: longer rear bodies, lower and narrower roofs; quad crown separated from widest plan station. RN plates 2/62 and IWM photographs guide form; manually measured GameModels3D raster proportions guide the comparison. Historical versus game dimensional disagreement remains in visual-iteration-04/measurements.json; no game geometry.'))

catalogPath=ROOT/'assets/parts/guns.json';catalog=json.loads(catalogPath.read_text())
main=dict(kind='gun',barbetteRadius=6.1,gunhouseSize=[13.2,12.3,3.12],pivotHeight=1.55,trunnionForward=4.45,muzzleForward=15.8,
...
parts=[dict(main,id='bl-14-mkvii-quad',name='14-inch Mk VII quadruple',massKg=1582000,barrelCount=4,gunhouseMesh=gunhouse(True)),
       dict(main,id='bl-14-mkvii-twin',name='14-inch Mk VII twin',massKg=915000,barrelCount=2,barbetteRadius=4.9,gunhouseSize=[11.9,9.44,3.23],rangefinderWidth=9.14,rangefinderForward=-4.3,gunhouseMesh=gunhouse(False)),
...
structure('tower-base','Forward tower lower platform',[(27.2,-3.7),(24.2,-8),(14,-8),(14,8),(24.2,8),(27.2,3.7)],DECK+3.05,3.05)
structure('tower-middle','Forward tower signal deck',[(27.2,-3.7),(24.6,-7.8),(15.6,-7.8),(9,-3.9),(9,3.9),(15.6,7.8),(24.6,7.8),(27.2,3.7)],DECK+6.1,3.05)
...
for mount in b['mounts'][:3]:
    radius=6.1 if mount['id']!='main-b' else 4.9;x=-mount['position'][2];top=mount['position'][1]+.12
    for i in range(24):
