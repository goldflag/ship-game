"""Takao blueprint authoring: a Blender-recipe preset scaffolded by `ship:new --legacy`.
...
example):
  ...
  python3 assets/ships/takao/author-blueprint.py [--loft loft.json]
...
(see the README), then `bun run ship:build takao`.
"""
import argparse
import json
import math
...
for id, name, z, bearing in MAIN:
    b['mounts'].append(dict(id=id, name=name, partId='type3-203-furutaka-twin', battery='main', position=[0, round(deck_y(z) + .6, 3), round(z, 3)],
...
print(f'Authored takao: {len(sections)} sections, {round(volume * 1.025)} t at the design waterline, {len(b["mounts"])} mounts, '
