"""Shared vocabulary for the Takao recipe: materials, primitives with ownership, merged wires.

Fitting positions are reference-frame datums read off the approved GameModels3D pjsc708 model
...
COLORS = {'naval': (.089, .090, .108), 'hullgray': (.089, .090, .108), 'roof': (.066, .069, .085), 'linoleum': (.070, .027, .016),
          'antifouling': (.100, .050, .025), 'black': (.012, .012, .013), 'edge': (.035, .036, .040), 'painted-edge': (.074, .077, .092),
          'dark': (.012, .013, .014), 'glass': (.02, .04, .05), 'canvas': (.50, .48, .40), 'wood': (.19, .13, .075),
          'white': (.52, .53, .52), 'bronze': (.36, .27, .12), 'red': (.40, .03, .02), 'brass': (.45, .33, .12)}
...
class Kit:
    def __init__(self, D, collection_names):
        ...
        for key, color in COLORS.items():
            m = bpy.data.materials.new('Takao ' + key)
    ...
    def roof(self, s):
        """Roof material of a measured block: the aircraft deck and the shelter deck round the
        funnels are laid with linoleum, as the reference shows; other roofs are grey steel."""
        xs = [p[0] for p in s['footprint']]
        zs = [p[1] + ZC for p in s['footprint']]
        top = s['baseY'] + s['height']
        if 5.8 < top < 6.4 and min(zs) > 22 and max(zs) < 52:
            return 'linoleum'
        return 'roof'
    ...
    def gun_seat(self, mount):
        ...
        if gap > .03:
            ...
            col = self.collections['Light AA' if mount['partId'].startswith('type96') else 'Main and secondary batteries']
