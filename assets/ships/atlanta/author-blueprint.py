"""Atlanta blueprint authoring: a Blender-recipe preset scaffolded by `ship:new --legacy`.
...
example):
  ...
  python3 assets/ships/atlanta/author-blueprint.py [--loft loft.json]
...
(see the README), then `bun run ship:build atlanta`.
"""
import argparse
import json
import math
...
for id, name, z, bearing in MAIN:
    b['mounts'].append(dict(id=id, name=name, partId='us-5in38-mk32-mod12', battery='main', position=[0, round(deck_y(z) + .6, 3), round(z, 3)],
...
print(f'Authored atlanta: {len(sections)} sections, {round(volume * 1.025)} t at the design waterline, {len(b["mounts"])} mounts, '
