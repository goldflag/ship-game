"""Fusō blueprint authoring: a Blender-recipe preset scaffolded by `ship:new --legacy`.
...
example):
  ...
  python3 assets/ships/fuso/author-blueprint.py [--loft loft.json]
...
(see the README), then `bun run ship:build fuso`.
"""
import argparse
import json
import math
...
b = dict(schemaVersion=1, id='fuso', name='Fusō',
...
print(f'Authored fuso: {len(sections)} sections, {round(volume * 1.025)} t at the design waterline, {len(b["mounts"])} mounts, '
