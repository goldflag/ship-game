"""Shared vocabulary for the Pensacola recipe: materials, primitives with ownership, merged wires.

Fitting positions are reference-frame datums read off the approved GameModels3D pasc106 model
...
# Linear-RGB interpretations of the reference's texture swatches (median texels of its hull atlas);
# appearance.json binds the named paints.
COLORS = {'naval': (.138, .185, .203), 'hullgray': (.138, .185, .203), 'roof': (.024, .039, .050), 'deck': (.024, .038, .048),
          'antifouling': (.109, .036, .018), 'black': (.012, .012, .013), 'edge': (.098, .128, .140), 'painted-edge': (.118, .156, .171),
          'dark': (.012, .013, .014), 'glass': (.02, .04, .05), 'canvas': (.2, .21, .19), 'wood': (.19, .13, .075),
          'white': (.52, .53, .52), 'bronze': (.36, .27, .12), 'red': (.40, .03, .02), 'green': (.02, .30, .04), 'brass': (.45, .33, .12),
          'floatblue': (.041, .067, .112)}
...
class Kit:
    def __init__(self, D, collection_names):
        ...
        for key, color in COLORS.items():
            m = bpy.data.materials.new('Pensacola ' + key)
    ...
    def disc_ring(self, n, r, center=(0, 0, 0), rx=None, phase=0.0):
        cx, cy, cz = center
    ...
    def roof(self, s):
        """Roof material of a measured block: painted steel in the reference's deck blue."""
        return 'roof'

    # ------------------------------------------------------------ guns
    def barbette(self, mount):
        """Measured barbettes (armour model and plan cuts, 2.66 m radius) from the deck or roof under
        the gunhouse to its sole, with a coaming ring where they meet it."""
        x, y, z = R(mount['position'])
        top = z
        floor = self.below(x, y, top - .05)
        col = self.collections['Main and secondary batteries']
        aid = mount['id']
        self.cylz(aid, col, 'barbette', (x, y, floor - .03), 2.66, top - floor + .035, 'naval', 72)
        self.cylz(aid, col, 'barbette coaming', (x, y, floor), 2.74, .08, 'edge', 72)
    ...
    def gun_seat(self, mount):
        ...
        if gap > .03:
            ...
            col = self.collections['Light AA' if mount['partId'].startswith(('us-11in', 'us-20mm')) else 'Main and secondary batteries']
