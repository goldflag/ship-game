"""Hyūga blueprint authoring: a Blender-recipe preset scaffolded by `ship:new --legacy`.
...
example):
  ...
  python3 assets/ships/hyuga/author-blueprint.py [--loft loft.json]
...
(see the README), then `bun run ship:build hyuga`.
"""
import argparse
import json
import math
...
b = dict(schemaVersion=1, id='hyuga', name='Hyūga',
...
print(f'Authored hyuga: {len(sections)} sections, {round(volume * 1.025)} t at the design waterline, {len(b["mounts"])} mounts, '
