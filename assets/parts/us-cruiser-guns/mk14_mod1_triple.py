"""Original 8-inch/55 Mk 14 Mod 1 triple mount, the Indianapolis main battery.

Visual proportions follow the approved GameModels3D pasc507 (Indianapolis) A1
artillery, `agm020_8in55_mk14_mod1`: a turtle-back gunhouse whose guns work
...
WING = [(1.575, .02), (2.946, .02), (2.946, 1.85), (2.600, 2.10), (1.575, 2.15)]
...
def create_mount(mount, col, helpers, materials):
    ...
    for mirror in [False, True]:
        prism('wing', WING, 1.55, 2.766, naval, mirror)
        sign = -1 if mirror else 1
        put(box(name + '.wing-rim', (2.24, sign * 2.20, 2.30), (.54, .11, .53), painted, col))
        put(box(name + '.wing-cleat', (2.82, sign * 2.92, .645), (.14, .10, .41), painted, col))
    put(box(name + '.glacis', (2.197, 0, .470), (1.14, 3.15, .900), naval, col))
    # The deck sits 5 cm lower than the approved model's, so the guns keep it
    # clear at full depression.
    put(box(name + '.recess-deck', (2.197, 0, .930), (1.14, 3.15, .06), naval, col))
    ...
    for sign in [1, -1]:
        ...
        put(box(name + '.rangefinder-collar', (-5.645, sign * 2.70, 2.67), (1.29, .80, .82), naval, col))
        put(box(name + '.rangefinder-hood', (-5.645, sign * 3.50, 2.67), (1.29, 1.60, .82), naval, col))
        put(box(name + '.rangefinder-window', (-4.985, sign * 3.90, 2.70), (.05, .70, .32), glass, col))
        put(box(name + '.rangefinder-cap', (-6.25, sign * 3.90, 2.67), (.06, .78, .60), painted, col))

        # Rung ladder up the flank, its rungs stood off the plate.
        for i in range(7):
            z = .24 + i * .33
            put(rod(name + '.ladder-rung', (-4.34, sign * 2.93, z), (-3.92, sign * 2.93, z), .026, painted, col, vertices=4))
        for x in [-4.34, -3.92]:
            put(rod(name + '.ladder-stringer', (x, sign * 2.93, .22), (x, sign * 2.93, 2.22), .024, painted, col, vertices=4))
        # Grab rail on the shoulder above the ladder head.
        for x in [-4.13, -2.40]:
            put(rod(name + '.shoulder-rail-post', (x, sign * 2.72, top_z(x, sign * 2.72) - .02),
                    (x, sign * 2.72, top_z(x, sign * 2.72) + .18), .022, painted, col, vertices=4))
        put(rod(name + '.shoulder-rail', (-4.13, sign * 2.72, top_z(-4.13, sign * 2.72) + .17),
                (-2.40, sign * 2.72, top_z(-2.40, sign * 2.72) + .17), .022, painted, col, vertices=4))

    # Sight hood and a short vent on the crown.
    put(box(name + '.crown-hood', (-4.40, -2.20, top_z(-4.40, -2.20) + .12), (.31, .28, .30), naval, col))
    put(box(name + '.crown-hood-window', (-4.26, -2.20, top_z(-4.40, -2.20) + .14), (.03, .18, .14), glass, col))
    ...
    for side, y, _ in barrel_layout(spec):
        ...
        profile = [(.20, SLEEVE), (3.68, SLEEVE), (3.70, .275), (5.60, .262), (length, .255)]
        count = 16
