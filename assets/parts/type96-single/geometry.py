...
def create_mount(m,col,helpers,materials):
 ...
 cyl=lambda name,loc,radius,depth,mat,vertices=16,r2=None:raw[1](name,loc,radius,depth,mat,col,vertices,r2)
 ...
 lc(cyl(name+'.foundation',(0,0,.07),sp['barbetteRadius'],.14,materials['edge'],vertices=16),base)
 ...
 lc(cyl(name+'.pedestal',(0,0,.48 if kongo else .45),.16,.70 if kongo else .64,materials['naval'],vertices=16,r2=.105))
 ...
 for side in [-1,1]:
  ...
  lc(rod(name+'.bearing',(trunnion,side*.10,pivot),(trunnion,side*bearing_end,pivot),bearing_radius,materials['edge'],vertices=12))
 ...
 lc(rod(name+'.barrel',(0,0,0),(length,0,0),.041,materials['edge'],r2=.025,vertices=12),rec)
 for j in range(12):
  o=rod(name+'.cooling-ring',(.14+j*.04,0,0),(.155+j*.04,0,0),.047,materials['naval'],vertices=8);lc(o,rec)
 lc(rod(name+'.flash-hider',(length-.12,0,0),(length,0,0),.03,materials['edge'],r2=.055,vertices=12),rec)
 lc(rod(name+'.bore',(length-.002,0,0),(length+.002,0,0),.0125,materials['dark'],vertices=12),rec)
 for s in [-1,1]:
 ...
 lc(tube_path(name+'.ring-sight',[(.13,.17+.10*math.cos(j*math.tau/16),.31+.10*math.sin(j*math.tau/16)) for j in range(16)],.009,materials['edge'],closed=True),elev)
