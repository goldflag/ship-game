"""Original 20 cm/50 3rd Year Type twin, Model C with turret rangefinder (Aoba).

Visual proportions follow the approved GameModels3D pjsc007 AB1 artillery
(`jgm014_203mm50_type_c_rf`). It shares the Model C gunhouse, barrels and
bloomers of `jgm013_203mm50_type_c` and adds a 6.85 m rangefinder housing
lying athwartships on the rear crown, with its optical ends projecting past
both flanks; the perimeter rail is broken either side of it. No reference
geometry is loaded here. The catalog owns the closed armour shell and the
weapon data; this recipe draws that shell and adds the gun ports, canvas
bloomers, sliding barrels, roof furniture, railing and ladders.
...
# Railing stations on the roof knuckle, as (x, y, z, stanchion). The
# rangefinder housing occupies the crown between them, so the rail is two open
# runs that die into its end faces instead of the single ring of the plain
# Model C mounting.
RAIL_AFT = [
    (-4.450, 2.42, 1.996, True), (-4.900, 1.40, 2.190, True),
    (-5.000, 1.01, 2.190, True), (-5.070, .00, 2.190, True),
]
RAIL_FWD = [
    (-2.660, 2.70, 1.930, False), (-1.750, 2.83, 1.900, True),
    (-0.690, 3.02, 1.810, True), (0.170, 3.02, 1.760, True),
    (0.960, 3.02, 1.690, True), (1.390, 3.02, 1.650, True),
    (2.170, 2.245, 1.980, True), (2.295, 1.73, 2.120, True),
    (2.295, 1.34, 2.130, True), (2.295, .57, 2.130, True),
]
# Rangefinder housing section in (x, z), extruded athwartships.
RF_SECTION = [
    (-3.90, 1.707), (-4.06, 1.933), (-4.06, 2.552), (-3.90, 2.778),
    (-2.78, 2.778), (-2.62, 2.552), (-2.62, 1.933), (-2.78, 1.707),
]
...
def create_mount(mount, col, helpers, materials):
    ...
    # Rangefinder: one armoured housing across the rear crown, its optical
    # ends standing proud of both flanks, with the hoods and end steps the
    # reference carries there.
    half = spec.get('rangefinderWidth', 6.85) / 2
    k = len(RF_SECTION)
    points = [(x, y, z) for y in (-half, half) for x, z in RF_SECTION]
    ...
    for sign in (1, -1):
        y = sign * half
        put(box(name + '.rangefinder-objective', (-3.34, y + sign * .02, 2.24), (.72, .03, .46), glass, col))
        for z in (1.925, 2.215, 2.505):
            put(box(name + '.rangefinder-end-step', (-3.93, y + sign * .05, z), (.21, .10, .09), painted, col))
        for x in (-3.78, -3.50, -3.22, -2.94):
            put(box(name + '.rangefinder-crown-rib', (x, y - sign * .06, 2.665), (.13, .27, .21), roof, col))
        # Sighting hood standing on the forward face at each optical end.
        put(box(name + '.rangefinder-hood', (-2.44, sign * 3.12, 2.52), (.56, .62, 1.20), naval, col))
        put(box(name + '.rangefinder-hood-window', (-2.18, sign * 3.12, 2.86), (.03, .30, .22), glass, col))

    # Perimeter railing standing on the roof knuckle, in two runs.
    def railing(run):
