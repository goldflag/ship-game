"""Original 20 cm/50 Type 3 Model E twin with turret rangefinder (Furutaka, Takao class).

Visual proportions follow the approved GameModels3D pjsc005 AB1 artillery
(`jgm147_203mm50_type_e_rf`); Atago (pjsc038) and Maya (pjsc517) fit the same
mounting as `jgm024_203mm50_type_e_rf`. It shares the Model E gunhouse,
barrels and bloomers of `jgm146_203mm50_type_e` and adds a low 6.6 m
rangefinder bar lying athwartships over the rear roof, its optical ends
projecting past both flanks; the rail runs only forward of it. It is a
different gunhouse from the
...
# The rangefinder replaces the after railing: the side run dies into its
# forward face instead of carrying on round the stern.
RAIL_SIDE = [
    (1.46, 2.175, ROOF_Z, True), (-.305, 2.375, ROOF_Z, True), (-1.695, 2.53, ROOF_Z, True),
    (-3.02, 2.56, ROOF_Z, False),
]
# Rangefinder bar section in (x, z), extruded athwartships.
RF_SECTION = [
    (-4.30, 1.66), (-4.38, 1.80), (-4.38, 2.40), (-4.30, 2.53),
    (-2.97, 2.53), (-2.89, 2.40), (-2.89, 1.80), (-2.97, 1.66),
]
...
def create_mount(mount, col, helpers, materials):
    ...
    # Rangefinder: one low armoured bar across the rear roof, its optical ends
    # standing proud of both flanks, with the hoods the reference carries on
    # its forward face.
    half = spec.get('rangefinderWidth', 6.9) / 2 - .14
    k = len(RF_SECTION)
    ...
    for sign in (1, -1):
        put(box(name + '.rangefinder-end', (-3.575, sign * (half + .07), 2.175), (1.07, .14, .69), naval, col))
        put(box(name + '.rangefinder-objective', (-3.30, sign * (half + .15), 2.18), (.46, .03, .34), glass, col))
    # The two sighting hoods are not a mirrored pair on the reference.
    put(box(name + '.rangefinder-hood', (-2.73, 3.025, 2.12), (.40, .45, .48), naval, col))
    put(box(name + '.rangefinder-hood', (-2.755, -2.98, 2.05), (.69, .62, .78), naval, col))
