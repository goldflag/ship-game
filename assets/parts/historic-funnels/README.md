...
The attachment socket is the flat sole at runtime Y=0. The inlet and one metre of below-deck occupancy fit inside the supporting hull. Measured bounds include ladders, pipes, rails and platforms; the default envelope conservatively encloses those fittings. Because that box is far larger than the casing inside it, the compiler treats a funnel as no collision body at all: hull pieces and other fittings may occupy it in either source order, and the below-deck uptake raises no intersects-plating, wall, load or water fault. Attachment/support, mass, CG, exhaust capacity, the sealed flooding opening it cuts and the runtime gun-arc obstruction are unchanged. The outlet follows the raked cap's centre. Original root metadata gives the renderer the actual mouth width and length; the existing smoke renderer reads the installed, rotated `exhaust-out` socket. Machinery simulation and propulsion formulas are unchanged. Starter designs fit the Clemson package, the smallest of these funnels, now that the RN corvette funnel is retired. Mass and exhaust capacity are game estimates comparable to existing catalog packages, not historical measurements.
