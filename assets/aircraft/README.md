...
The [independent comparator](../../scripts/aircraft/compare.py) reads actual exported GLB triangles and composed transforms, then projects them onto the original side/top rasters without fitting the model to the drawing. NumPy and Pillow are required. Per-aircraft `generated/comparison/` retains overlays, silhouettes, source crops and hashed measurements; ignored `.build/reviews/aircraft/schematic-comparisons.json` indexes the collection. `--allow-stale` produces explicitly labeled diagnostic overlays during rebuilding. Fresh comparisons still require visual review.

Consult the [source register](references/sources.json), individual shape/reference notes, and [discrepancy register](discrepancies.md) for evidence and known limitations. Overview sheets (`review_sheets.py`), browser captures (`browser_review.py`) and per-aircraft six-view sets are generated evidence, not historical approval; sheets and captures are written to ignored `.build/reviews/aircraft/`.

## Scope and remaining approximations

Neutral wings stay extended; F4F-4 and TBD-1 have independent folding rigs, while the SBD has fixed wings. Gear travel and linkage clearance remain approximate. The neutral model combines a level engine-shaft attitude with extended gear; the six combat aircraft use CPU resting poses, with fitted contact on crowned carrier decks still under review. Source payload sockets remain empty. Combat attaches simple payload visuals and provides approximate takeoff, landing, aircraft combat and damage; crews and aerodynamic flight dynamics remain absent. See [air operations](../../docs/air-operations.md).

The shared arrestor hook stows along each aircraft's authored fuselage underside. Its hinge pin, fork shank and shoe retain the stable `arrestor.hook` joint; fixed mounting lugs attach to the fuselage. The fork provides physical tail-wheel clearance, but its shape and travel are original mechanical approximations, not historically verified variant mechanisms. After hook/gear changes, run the pipeline's published-geometry [hook clearance check](../../scripts/aircraft/check_hook_clearance.py) and inspect intermediate poses as well as stowage.
