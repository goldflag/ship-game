...
- Use `ship:new`, `ship:compile`, `ship:build`, `ship:check` and `ship:review`. For construction-backed ships, follow [agent construction authoring](../docs/construction-authoring.md) and inspect the exact exported model through its fixed views and native articulation checks. For reusable component geometry and Blender-recipe ships, follow the [authoring loop](../docs/ship-build-reference.md#blender-mcp-authoring-loop) through Blender MCP or background Blender renders: inspect before and after meaningful changes and preserve accepted edits in durable inputs before a clean `ship:build`. A successful Python command is not visual review.
