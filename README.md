...
Every visible battle ship has an overhead name, live hull HP bar and current HP number, floating directly over the scene without a background box, connector or team text. Mint bars identify friendly ships and salmon bars identify enemies. Nearby labels separate to remain readable and keep the instruments clear. Labels follow the displayed hull poses, hide outside the camera view, and follow the H instrument toggle. A sinking ship keeps its remaining structural HP reading, since flooding can sink a hull before its structure reaches zero.
