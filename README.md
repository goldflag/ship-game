...
Naval fleet battles using **Rust, Bun, TypeScript, React, Three.js WebGPU with the game's own ocean, and Sky Pro 2.2.0**. Custom battles run the shared Rust simulation in a WebAssembly worker; 1v1 multiplayer uses the same simulation on an authoritative Rust server. Historical ships use a reproducible Blender asset pipeline and versioned blueprints. Reusable equipment is discovered with `bun run part:list` and built from original shared recipes; `bun run model:viewer` browses ships, standalone components and reference comparisons. See [shared components](docs/shared-components.md).
...
- `src/game/Game.ts`: scene, ocean and Sky Pro integration and lifecycle. Combat ship poses come from CPU simulation; GPU waves animate the sea and buoys.
...
- `vendor/threejs-sky-pro/`: the supplied proprietary Sky Pro runtime bundle, license and patch record. Its original terms remain in force.
