...
| 1 / 2 / 3 | Select main AP / secondary AP / torpedoes (when fitted) |
...
Friendly and enemy bots have an overhead name, live hull HP bar and current HP number, floating directly above their ships without a background box, connector or team text. Your own ship has no overhead label. Mint bars identify friendly ships and salmon bars identify enemies. Labels stay at their projected ship positions and may overlap each other or the instruments. They follow the displayed hull poses, hide outside the camera view, and follow the H instrument toggle. A sinking ship keeps its remaining structural HP reading, since flooding can sink a hull before its structure reaches zero. Hull hits show the HP lost beside the name, with the lost section of the bar briefly held in gold before fading. Hits arriving together combine into a single salvo number. Your own hull hits show a gold HP-loss readout above the helm and a brief red screen-edge cue. Feedback freezes while paused. The upper-right Damage and Frags counters track your actual enemy hull damage and sinkings for the current battle; friendly fire and damage to sinking wrecks do not add to the score. The last opponent to damage a hull or open a breach earns its frag, including delayed flooding losses. Returning to port resets both counters.

Hull HP uses a base allowance plus a square-root bonus from the displacement authored in each ship blueprint: **Yamato 1,750; Bismarck 1,450; Enterprise 1,180; Baltimore 1,020**. The shared rule is `300 + 1,450 × sqrt(displacement tonnes / 70,000)`, rounded to the nearest 10 HP. This keeps smaller hulls viable: a hypothetical 2,000-tonne destroyer gets 550 HP and a 3,000-tonne destroyer gets 600 HP. Armor determines penetration separately, magazines can disable guns, and flooding can sink a ship with HP remaining. These are provisional gameplay values.
...
Combat is an accessible simulation prototype: approximate AP penetration through physical plates (Bismarck) or legacy armor volumes, internal module failures, magazine events, compartment flooding, and sinking. The internal layout and performance values are provisional gameplay data. Networking, player shipbuilding, aircraft, crew, full physical hydrostatics, detailed HE/AP fuzes, and hull fracture remain future work. Submarine dive planes, propellers and rudders animate from CPU motion through the blueprint’s retained joints; other non-gun fittings remain visual.
