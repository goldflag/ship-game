...
**Type VIIC** is an early-war 1941 submarine preset with surface and submerged operation. It has an articulated 8.8 cm deck gun, a 2 cm platform gun, four bow tubes and one stern tube, with 14 torpedoes. Select **3** for torpedoes (selected initially on this preset), aim within ±15° of the bow or stern, then click / Q / Fire for one tube or hold to launch loaded tubes in sequence. The torpedo instrument shows each tube’s readiness, remaining rounds and reload, plus range and arming distance. G → Aim at → Target waterline supplies a lead; manual aiming requires you to lead moving ships. G7a torpedoes run straight at 44 kn, at 2 m depth, for up to 5 km; they arm after 300 m and tubes reload in 45 seconds. A water wake shows each run; hull contact creates a water plume, damage and flooding. Allies and sinking wrecks can be hit. Bots use the same tubes, lead solver and damage rules. Type VIIC has 450 hull HP from the shared displacement curve; two armed G7a hits exhaust it, while flooding remains a separate sinking route. The **Depth** instrument orders Surface, Periscope (7 m), or 50 m; **Z / X** change the order in 10 m steps up to 150 m. **B / Emergency blow** empties ballast faster and orders the surface. Tanks fill or blow over time, and dive planes help while underway. Depth is measured below the original surfaced waterline, so keel depth includes another 4.74 m. Electric motors supply submerged propulsion at up to 7.6 kn; diesels resume near the surface. Guns are secured below 0.5 m; torpedo launches are allowed down to 12 m and settle onto their 2 m running depth. Deep hulls pass below surface ships and shallow torpedoes. Damage flooding is separate from ballast and enters faster at depth; pressure beyond 150 m opens a persistent hull breach. Submarine bots dive on torpedo approaches and surface between attacks while their forward-facing tubes reload. **C** cycles to the periscope view; **Shift** magnifies it. A scope taken deeper stays underwater. Ballast/depth rates are tuned gameplay values; battery charge, finite compressed air, oxygen, sonar and depth charges are not simulated. See [Type VIIC configuration and limitations](assets/ships/type-viic/README.md).
...
Choose **USS Enterprise** in Custom battle. Hold **Ctrl** to use **Air group** on the left. Launch up to three Wildcats (fighters), Dauntlesses (dive bombers), or Devastators (torpedo bombers) per click, from six of each aboard. Select an enemy ship before launching a strike. Orders retain that target; fighters automatically intercept hostile aircraft and patrol halfway toward the selected enemy. Add an enemy Enterprise to fight opposing air groups; bot carriers launch automatically.

All 18 aircraft are visible parked on Enterprise in port and battle. Launch orders taxi planes onto the flight lane for a visible takeoff run. Survivors return, touch down, roll to a stop, taxi back to their spots, and rearm for 35 seconds. One plane at a time uses the flight lane, with about 16–22 seconds between launches and 27–32 seconds per recovery including taxi time; larger groups wait their turn. **Recall all** cancels queued launches and returns surviving aircraft. **Aircraft · select to follow** lists each surviving plane with its phase, condition, ammunition and payload. Hold Ctrl and choose **Follow** beside a plane to ride behind it through deck operations, combat and landing. **Return to ship**, C (Change camera), or R (Recenter) restores the ship view; losing the tracked plane returns automatically. Following preserves the ship’s existing aim and does not pilot the aircraft; the chart marks aircraft as crosses in their team's color. Losses remain until the next battle. Flight operations stop if the service magazine is unavailable or the ship is too deeply immersed or heavily listed.
