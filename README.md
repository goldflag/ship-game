...
You start in port with the Bismarck moored. Drag to inspect the ship, then choose **Custom battle** to configure both fleets. Choose your own ship, add up to 29 friendly bots, and select one to 30 enemy bots, for up to 30 ships per side. All six registered presets can appear on either team, including duplicates. **Spawn distance** sets the separation between formations from 1–20 km in 0.5 km steps (5 km by default). **Start battle** loads the chosen models and deploys both teams facing each other, with 650 m between adjacent ships. **Return to port** in the pause menu ends the battle and resets every ship. The selected [Fleet harbor garage](docs/garage-mockups/README.md) uses compact panels and a transparent top bar. The fleet carousel lists the registered ship presets: Bismarck, Yamato, Baltimore, Enterprise, Type VIIC, and Fletcher. Select a card to switch ships in place for inspection and sailing; the harbor stays loaded, your orbit direction is preserved, and camera distance adjusts to keep the same relative zoom for the new hull. Currency, refits, and commanders are illustrative, with temporary state only.
...
Planes take off at three-second intervals, fight, release weapons, return, and rearm for 35 seconds. **Recall all** cancels queued launches and returns surviving aircraft. **Inspect flights** shows phase, aircraft condition, ammunition and payload; the chart marks aircraft as crosses in their team's color. Losses remain until the next battle. Flight operations stop if the service magazine is unavailable or the ship is too deeply immersed or heavily listed.
