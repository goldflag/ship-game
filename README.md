...
- `src/game/FleetShipDraws.ts`, `FleetBatch.ts`, `SubtreeLayers.ts`: the fleet's batched surfaces. Batched originals stay in each ship's hierarchy, masked by layers, for picking, inspection and scars; `SubtreeLayers` indexes those hierarchies (structure through three's child events, layers as the batches report them) so three's scene projection and the caster passes' walks skip subtrees with nothing on the camera's layers. Batches keep each part's world bounds from its last pose, so the main pass (and the occlusion prepass, when three draws it) only test them. Each frame `FleetShipDraws` walks its surfaces through flat arrays in batch order and forms a drawn surface's pose itself, leaving the surface's own matrix undone (only the batch reads it). Switches for comparison, each leaving output unchanged: `subtreePruning(renderer).enabled`, `FleetShipDraws.flat`, `FleetBatch.keepBounds`, `FleetBatch.directPoses`, `FleetBatch.flatSpheres`, `ShipPoseMatrices.flat`, `jointTurns.direct` (joint Eulers and quaternions written as three computes them, in `jointTurns.ts`) and `ShipRigView.shareFrustum`.
