...
Naval fleet battles using **Rust, Bun, TypeScript, React, Three.js WebGPU with the game's own ocean and sky**. Custom battles run the shared Rust simulation in a WebAssembly worker; 1v1 multiplayer uses the same simulation on an authoritative Rust server. Historical ships use a reproducible Blender asset pipeline and versioned blueprints. Reusable equipment is discovered with `bun run part:list` and built from original shared recipes; `bun run model:viewer` browses ships, standalone components and reference comparisons. See [shared components](docs/shared-components.md).
...
Open http://localhost:5173. The game requires WebGPU: use a current Chrome, Edge or Safari with hardware acceleration on. Without WebGPU, start-up stops on an error screen instead of falling back to WebGL. Initial startup compiles the ocean and cloud shaders, which can take a moment.
...
Sound uses an [original naval set](assets/audio/naval/README.md): ElevenLabs-generated mechanical UI clicks, engine telegraph and reload cues, a departure horn, procedural caliber-based gunfire, armor hits, ricochets, splashes and magazine explosions. Click or press a key to enable browser audio. **Esc → Settings → Sound** adjusts master, effects and interface levels or mutes everything; changes apply immediately and persist without restarting the battle. Pause stops combat tails; background tabs are silent. `bun run audio:build` rebuilds the processed assets locally from the retained originals and prompts; `bun run audio:synth` renders the gun reports from their presets.
...
- `src/game/ShipView.ts`: binds simulation state to exported joints (`ShipRenderView.ts` draws the pose through `shipJoints.ts`; `ShipView` adds aiming and muzzle checks). `ShipInspection.ts` renders shared armor/module/compartment inspection geometry; `src/ships/inspection.ts` supplies both its geometry and the port list. `CombatEffects.ts` expands projectile instances as needed and uses bounded pools for cosmetic particles.
- `src/game/Game.ts`: scene, ocean and sky integration and lifecycle. Combat ship poses come from CPU simulation; GPU waves animate the sea and buoys.
- `src/game/FocusShadowNode.ts`, `ShadowCasterPass.ts`: the sun's near, wide and view shadow maps. The caster pass draws them with WebGPU directly instead of a three scene pass per map: it collects casters once a frame, culls each batched part and instance per map and submits depth-only draws from three's own buffers, leaving casters it cannot reproduce (skinned, one-sided, custom vertex or discard nodes) to three. `game.shadowCasters.enabled = false` restores three's passes for comparison.
- `src/game/ShipOcclusion.ts`: ship-on-ship ambient occlusion from a half-resolution depth prepass of the ships alone, which a second caster pass (`DepthCasterPass` in `ShadowCasterPass.ts`) draws the same way (`game.occlusion.casters.enabled = false` hands it back to three).
- `src/game/FleetShipDraws.ts`, `FleetBatch.ts`, `SubtreeLayers.ts`: the fleet's batched surfaces. Batched originals stay in each ship's hierarchy, masked by layers, for picking, inspection and scars; `SubtreeLayers` indexes those hierarchies (structure through three's child events, layers as the batches report them) so three's scene projection and the caster passes' walks skip subtrees with nothing on the camera's layers. Batches keep each part's world bounds from its last pose, so the main pass (and the occlusion prepass, when three draws it) only test them. Switches for comparison, each leaving output unchanged: `subtreePruning(renderer).enabled`, `FleetBatch.keepBounds` and `ShipPoseMatrices.flat`.
- `src/game/ocean/`: the game's own ocean behind one facade (`Ocean.ts`): waves, surface, wake field, fog, screen-space ship reflections and the underwater view. See [its README](src/game/ocean/README.md) and [ocean configuration](docs/ocean-configuration.md).
...
- `src/game/sky/`: the game's own sky (atmosphere, celestial bodies, volumetric clouds, weather, the sea's environment) behind `SkyApi`; see its README.
- `vendor/threejs-sky-pro/`: the supplied proprietary Sky Pro runtime bundle the game's sky replaced, kept only as a lazily loaded comparison (Graphics `skyRenderer`), with its license and patch record. Its original terms remain in force.
...
Press **I** in battle, or use **Ship damage** beside the hull readout, to inspect your ship's live internals. The X-ray and report show damaged or destroyed equipment, fires, flooded compartments, open breaches and crew work. Select a named part to isolate it; **All parts** includes healthy equipment and dry spaces. Drag to orbit and scroll to zoom. Combat continues while you inspect. Press **I** again or **Esc** to return. The shortcut is rebindable in **Settings → Keybindings**.
