...
use naval_sim::{
    construction::{self, ConstructionCompiler},
...
fn main() {
    ...
    let sources: Vec<ConstructionSource> =
    ...
    for source in sources {
        ...
        assert_eq!(
            construction::to_json(&cached).unwrap(),
            construction::to_json(&fresh).unwrap(),
            "{}",
            source.revision
        );
        println!(
            "{}",
            serde_json::json!({ "revision": source.revision, "warmMs": warm_ms, "freshMs": fresh_ms,
