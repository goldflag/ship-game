...
fn primitive(kind: &str) -> ConstructionPrimitive {
    serde_json::from_value(
        json!({"id":"hull", "kind":kind, "size":[4,2,6], "position":[0,0,0], "rotationDeg":0}),
    )
    .unwrap()
}
fn tilt(pitch: f64, roll: f64) -> ConstructionPrimitiveTilt {
    ConstructionPrimitiveTilt {
        version: 1.,
        pitch_deg: pitch,
        roll_deg: roll,
    }
...
fn native_shape_paths_rotate_rigidly_without_baking_the_source() {
    ...
    rounded.shaping = Some(
        serde_json::from_value(
            json!({"version":1,"edges":[0,1,2,3],"radius":0.15,"style":"round"}),
        )
        .unwrap(),
    );
    ...
    for p in primitives {
        ...
        for (pitch, roll) in [(90., 0.), (0., 90.)] {
            ...
            turned.tilt = Some(tilt(pitch, roll));
            let after = construction::primitive_cells(&turned).unwrap();
            assert!(
                (cg::total(&before).volume - cg::total(&after).volume).abs() < 1e-6,
                "{} volume",
                p.kind
            );
                ...
                .flat_map(|f| &f.vertices)
            {
                assert!(
                    expected
                        .iter()
                        .any(|p| (0..3).all(|k| (v[k] - p[k]).abs() < 1e-7)),
                    "{} rotated point {v:?}",
                    p.kind
                );
            }
...
fn saved_orientation_compiles_with_face_assignments_and_rejects_bad_tilt() {
    ...
    assert!(
        serde_json::to_value(&source).unwrap()["construction"]["primitives"][0]
            .get("tilt")
            .is_none(),
        "Legacy sources omit absent tilt"
    );
    source.construction.primitives[0].tilt = Some(tilt(90., 0.));
    let restored: ConstructionSource =
    ...
    assert!(
        top.iter()
            .all(|s| s.paint == "deck-gray" && s.thickness_mm == 25. && s.normal[2] > 0.999)
    );
