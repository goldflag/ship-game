use naval_sim::{damage::Combatant, definition::ShipDefinition, geometry::radians, torpedoes::train_launchers};

#[test]
fn launcher_stops_reverse_through_neutral_and_clamp_unreachable_aim() {
    let def: ShipDefinition = serde_json::from_str(include_str!("../../../public/models/yukikaze.json")).unwrap();
    ...
    let aim = |angle: f64| Some([launcher.position[0] + 1000.0 * angle.sin(), 0.0, launcher.position[2] - 1000.0 * angle.cos()]);
...
fn absent_travel_stops_preserve_wrapped_shortest_path() {
    let mut def: ShipDefinition = serde_json::from_str(include_str!("../../../public/models/yukikaze.json")).unwrap();
    def.torpedo_launchers.as_mut().unwrap()[0].traverse_limits_deg = None;
    let launcher = &def.torpedo_launchers.as_ref().unwrap()[0];
    let mut actor = Combatant::new("player", &def);
    actor.launcher_trains.insert(launcher.id.clone(), radians(110.0));
    let target = -radians(110.0);
    train_launchers(&mut actor, &def, &|_| Some([1000.0 * target.sin(), 0.0, launcher.position[2] - 1000.0 * target.cos()]), 1.0 / 60.0, None);
