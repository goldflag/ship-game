...
fn coatings_round_trip_without_changing_surfaces_mass_or_hydrostatics() {
    ...
    assert!(serde_json::to_value(source.construction.primitives[0].custom_hull.as_ref().unwrap()).unwrap().get("paintBands").is_none());
    let original=construction::compile(&source,&catalog);
    assert!(original.definition.is_some(),"{:?}",original.diagnostics);
    source.construction.primitives[0].custom_hull.as_mut().unwrap().paint_bands=Some(serde_json::from_value(bands()).unwrap());
    let saved=serde_json::to_value(&source).unwrap();
    assert_eq!(saved["construction"]["primitives"][0]["customHull"]["paintBands"],bands());
    ...
    assert_eq!(serde_json::to_value(&original.surfaces).unwrap(),serde_json::to_value(&painted.surfaces).unwrap());
    let original=original.definition.unwrap(); let painted=painted.definition.unwrap();
    assert_eq!(serde_json::to_value(original.hull).unwrap(),serde_json::to_value(painted.hull).unwrap());
    assert_eq!(serde_json::to_value(original.loading).unwrap(),serde_json::to_value(painted.loading).unwrap());
}
...
fn compiler_rejects_invalid_band_versions_order_identity_and_paints() {
    ...
    let mut bad=vec![];
    for (field,value) in [("id",json!("lower")),("upperY",json!(-0.5)),("upperY",json!(-1)),("upperY",json!(501)),("paint",json!(""))] {
        let mut b=bands();b["bands"][1][field]=value;bad.push(b);
    }
    let mut version=bands();version["version"]=json!(2);bad.push(version);
    bad.push(json!({"version":1,"bands":(0..9).map(|i|json!({"id":format!("b{i}"),"upperY":i,"paint":"red-oxide"})).collect::<Vec<_>>()}));
    for value in bad {
        let mut invalid=source.clone();
        invalid.construction.primitives[0].custom_hull.as_mut().unwrap().paint_bands=Some(serde_json::from_value(value).unwrap());
        let result=construction::compile(&invalid,&catalog);
        assert!(result.definition.is_none(),"Invalid band settings were accepted");
