...
fn terrain_horizon_and_weather_clip_search_history() {
    let rules = VisualRules::default();
    let low = observer();
    // A long ridge, 180 m high and 1.4 km wide, lying north-south through (0, 500).
    let field = Heightfield::from_fn(401, 601, 20.0, [-4000.0, -6000.0], 0.25, |x, z| {
        let r = (x / 700.0).hypot(z / 2500.0);
        if r < 1.0 {
            1.0 + 179.0 * (1.0 - r * r)
        } else {
            -(2.5 + 0.12 * (r - 1.0) * 700.0).min(160.0)
        }
    });
    let ridge = Terrain::placed(std::sync::Arc::new(field), [0.0, 500.0]);
    ...
    surface.update(0, &[low], &ridge, clear(), &rules);
    air.update(0, std::slice::from_ref(&high), &ridge, clear(), &rules);
...
fn unseen_damage_and_sinking_do_not_change_condition_or_coverage() {
    ...
    for tick in [300, 600, 6000] {
        a.update(tick, &[target()], &Terrain::open_sea(), clear(), &rules);
        b.update(
            tick,
    ...
    b.update(6600, &[], &Terrain::open_sea(), clear(), &rules);
    assert_eq!(
...
fn a_visible_wreck_cannot_keep_observing_for_its_former_team() {
    ...
    sensors.update(0, &[wreck, target()], &Terrain::open_sea(), clear(), &rules);
...
fn a_large_spread_of_observers_keeps_coverage_storage_bounded() {
    ...
    for tick in [0, COVERAGE_INTERVAL, COVERAGE_INTERVAL * 2] {
        sensors.update(tick, &observers, &Terrain::open_sea(), clear(), &rules);
    }
    let coverage = sensors.coverage(TeamId::A);
