...
use crate::{
    definition::{
        AuthoredStructure, MountDefinition, ShipDefinition,
        ShipDefinitionMountClearanceMountsItem as Entry, Vec3,
    },
    geometry::*,
...
pub fn mount_pose_clear(
    ...
    let Some(entry) = profile.mounts.iter().find(|e| e.mount_id == m.id) else {
        return true;
    };
    let margin = profile.margin_m + extra_margin;
    let own = barrels(m, pose, entry, margin);
    if own.iter().any(|c| {
        profile.structures.iter().any(|p| {
            capsule_prism(
                *c,
                d.structures
                    .as_ref()
    ...
    for pair in &profile.neighbors {
        ...
        let oe = profile.mounts.iter().find(|e| &e.mount_id == id).unwrap();
...
pub fn move_mount_with_clearance(
    ...
    if !d
        .mount_clearance
        .as_ref()
        .is_some_and(|p| p.mounts.iter().any(|e| e.mount_id == d.mounts[index].id))
    {
        s.train = target.0;
    ...
    let steps = (train.abs().max(elevation.abs()) / radians(0.25))
        .ceil()
        .max(1.0) as usize;
    ...
    for i in 1..=steps {
        ...
        if !mount_pose_clear(d, index, next, states, margin) {
            return false;
        }
        s.train = next.0;
        s.elevation = next.1;
