...
impl GeometryCache {
    ...
    pub fn subtract_all<'a>(
        &mut self,
        cells: Vec<cg::Cell>,
        cutters: impl IntoIterator<Item = &'a cg::Cell>,
    ) -> Result<Vec<cg::Cell>, String> {
        if cells.is_empty() {
            return Ok(cells);
        }
        let bounds = crate::structure::bounds(
            cells
                .iter()
        ...
        let cutters: Vec<_> = cutters
            .into_iter()
            .filter(|b| {
                let (center, size) = cg::bounds(b);
                (0..3).all(|i| {
                    (center[i] - bounds.0[i]).abs() <= (size[i] + bounds.1[i]) * 0.5 + cg::EPS * 4.
                })
            })
            .collect();
        if cutters.is_empty() {
...
mod tests {
    use super::*;
    fn equal(a: Vec<cg::Cell>, b: Vec<cg::Cell>) {
        assert_eq!(
    ...
    fn reuse_is_exact_and_ignores_only_disjoint_cutters() {
        ...
        equal(
            cache.subtract_all(vec![a.clone()], [&b, &remote]).unwrap(),
            expected.clone(),
        );
        cache.begin();
        ...
        equal(
            cache.subtract_all(vec![a.clone()], [&moved]).unwrap(),
            cg::subtract_all(vec![a], [&moved]).unwrap(),
        );
        assert_eq!(cache.hits, 1);
    }
    #[test]
    fn union_matches_stateless_geometry_through_add_remove_and_move() {
        ...
        for offset in [1., 0.5, 1., 10., 1.] {
            let cells = vec![
                cg::box_cell([0.; 3], [4.; 3]),
                cg::box_cell([offset, 0., 0.], [2.; 3]),
            ];
            let neighbors = vec![vec![1], vec![0]];
            cache.begin();
            equal(
                cache.union_near(&cells, &neighbors).unwrap(),
                cg::union_near(&cells, &neighbors).unwrap(),
            );
        }
        cache.begin();
        cache.begin();
